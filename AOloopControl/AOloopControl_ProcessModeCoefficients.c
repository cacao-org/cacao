...
imageID __attribute__((hot)) AOloopControl_ProcessModeCoefficients(long loop)
{
    ...
    imageID IDout;
    imageID IDmodeval; // WFS measurement

    long modeval_bsize = 20; // circular buffer size (valid for both DM and PF)

    imageID IDmodevalDM_C; // DM correction, circular buffer to include history
    long    modevalDMindexl = 0;
    long    modevalDMindex  = 0; // index in the circular buffer

    imageID IDmodevalDM;        // DM correction at WFS measurement time
    imageID IDmodevalDMcorr;    // current DM correction
    imageID IDmodevalDMnow;     // DM correction after predictiv control
    imageID IDmodevalDMnowfilt; // current DM correction filtered
    float   alpha;
    imageID IDmodevalPFsync;

    imageID IDmodeARPFgain; // predictive filter mixing ratio per gain
                            // (0=non-predictive, 1=predictive)
    // imageID IDmodevalPF; // predictive filter output
    imageID IDmodevalPFres; // predictive filter measured residual (real-time)
    imageID IDmodeWFSnoise; // WFS noise
    imageID
        IDmodevalPF_C; // modal prediction, circular buffer to include history
    ...
    imageID IDatlimbcoeff;

    long     IDautogain  = -1; // automatic gain input
    uint64_t autogainCnt = 0;
    ...
    int modeval_semwaitindex =
        ImageStreamIO_getsemwaitindex(&data.image[IDmodeval], 4);
    ...
    uint32_t NBmodes = data.image[IDmodeval].md[0].size[0];
    ...
    fprintf(fptestmPF,
            "%4d  aoloopcontrol_var.aoconfID_modevalPF =  %ld\n",
            __LINE__,
            aoloopcontrol_var.aoconfID_modevalPF); // TEST
    ...
    if (aoloopcontrol_var.aoconfID_modevalPF == -1)
    {
        ...
        if (aoloopcontrol_var.aoconfID_modevalPF != -1)
        {
            ...
            for (uint64_t ii = 0;
                 ii < data.image[aoloopcontrol_var.aoconfID_modevalPF]
                              .md[0]
                              .size[0] *
                          data.image[aoloopcontrol_var.aoconfID_modevalPF]
                              .md[0]
                              .size[1];
                 ii++)
            {
                data.image[aoloopcontrol_var.aoconfID_modevalPF].array.F[ii] =
                    0.0;
            }
    ...
    // load/create aol_modeval_dm_now (current modal DM correction after mixing
    // with predicitiv control)
    ...
    // load/create aol_modeval_dm (modal DM correction at time of currently
    ...
    // predictive mode values) 0: adopt non-predictive value 1: adopt predictive
    // value
    //
    // Set values to 0 when predictive filter is off
    // set to 1 (or intermediate value) when predictive filter for corresponding
    // mode is on
    //
    ...
    if (aoloopcontrol_var.aoconfID_modeARPFgainAuto == -1)
    {
        ...
        for (uint32_t m = 0; m < NBmodes; m++)
        {
            data.image[aoloopcontrol_var.aoconfID_modeARPFgainAuto].array.F[m] =
                1.0;
        }
    }
    ...
    if (sprintf(imname,
                "aol%ld_modeval_dm_C",
                loop) < 1) // modal DM correction, circular buffer
    {
        PRINT_ERROR("sprintf wrote <1 char");
    }
    ...
    data.image[IDblknb].md[0].write = 1;
    {
        ...
        while (m < NBmodes)
        {
            ...
            uint32_t n = data.image[ID].md[0].size[2];
            printf(" -> found %2u modes\n", n);
            blockNBmodes[blk] = n;
    ...
    framelatency = AOconf[loop].AOtiminginfo.hardwlatency_frame +
    ...
    for (uint32_t m = 0; m < NBmodes; m++)
    {
        ...
        for (modevalDMindex = 0; modevalDMindex < modeval_bsize;
             modevalDMindex++)
        {
            data.image[IDmodevalDM_C].array.F[modevalDMindex * NBmodes + m] = 0;
        }
        for (modevalPFindex = 0; modevalPFindex < modeval_bsize;
             modevalPFindex++)
        {
    ...
    blockstatcnt = 0;
    for (uint32_t block = 0; block < AOconf[loop].AOpmodecoeffs.DMmodesNBblock;
         block++)
    {
    ...
    while (loopOK == 1)
    {
        ...
        if (loopOK == 1)
        {
            ...
            {
                while (cnt == data.image[IDmodeval]
                                  .md[0]
            ...
            {
                ImageStreamIO_semwait(&data.image[IDmodeval],
                                      modeval_semwaitindex);
            }
            ...
            /*        while(sem_trywait(data.image[IDmodeval].semptr[4])==0) {
                              printf("WARNING %s %d  : sem_trywait on
             modeval\n",
            ...
            {
                printf(
                    "WARNING %s %d  : [%d] sem_trywait on "
                    "data.image[IDmodeval]\n",
                    __FILE__,
                    __LINE__,
                    semval - semcnt);
                fflush(stdout);
            ...
            tdiffv = timespec_diff_double(
                ...
                tnow);
            data.image[aoloopcontrol_var.aoconfID_looptiming].array.F[3] =
                tdiffv;
            ...
            //
            //  current state =   modemult   x   ( last state   - modegain *
            //  WFSmodeval  )
            //
            ...
            for (uint32_t m = 0; m < NBmodes; m++)
            {
                data.image[IDmodevalDMcorr].array.F[m] =
                    modemult[m] *
                    (data.image[IDmodevalDM_C]
                         .array.F[modevalDMindexl * NBmodes + m] -
                     modegain[m] * data.image[IDmodeval].array.F[m]);
            }
            COREMOD_MEMORY_image_set_sempost_byID(IDmodevalDMcorr, -1);
            data.image[IDmodevalDMcorr].md[0].cnt1 =
                LOOPiter; // modevalPFindex;
                          // //TBC
            data.image[IDmodevalDMcorr].md[0].cnt0++;
            data.image[IDmodevalDMcorr].md[0].write = 0;
            ...
            tdiffv = timespec_diff_double(
                data.image[aoloopcontrol_var.aoconfID_looptiming].md[0].atime,
                tnow);
            data.image[aoloopcontrol_var.aoconfID_looptiming].array.F[4] =
                tdiffv;

            AOloopControl_RTstreamLOG_update(loop,
            ...
            if (AOconf[loop].aorun.ARPFon == 1)
            {
                ...
                else
                {
                    ...
                    // if waiting for modevalPF, uncomment this line, and the
                    // "drive semaphore to zero" line after the next loop
                    // sem_wait(data.image[IDmodevalPF].semptr[3]);

                    //
                    // prediction is mixed here with non-predictive output of WFS
                    // minus sign required to apply correction on DM (correction
                    // should be opposite of WFS measurement) note that second
                    // term (non-predictive) does not have minus sign, as it was
                    ...
                    for (uint32_t m = 0; m < NBmodes; m++)
                    {
                        ...
                        mixratio = AOconf[loop].aorun.ARPFgain *
                                   data.image[IDmodeARPFgain].array.F[m] *
                                   data.image[aoloopcontrol_var
                                                  .aoconfID_modeARPFgainAuto]
                                       .array.F[m];
                        data.image[IDmodevalDMnow].array.F[m] =
                            -mixratio *
                                data.image[aoloopcontrol_var.aoconfID_modevalPF]
                                    .array.F[m] +
                            (1.0 - mixratio) *
                    ...
                    for (uint32_t m = 0; m < NBmodes; m++)
                    {
                        data.image[IDmodevalPF_C]
                            .array.F[modevalPFindex * NBmodes + m] =
                            data.image[aoloopcontrol_var.aoconfID_modevalPF]
                                .array.F[m];
                    }
                    COREMOD_MEMORY_image_set_sempost_byID(IDmodevalPF_C, -1);
                    data.image[IDmodevalPF_C].md[0].cnt1 =
                        modevalPFindex; // NEEDS TO CONTAIN WRITTEN SLICE ?
                    data.image[IDmodevalPF_C].md[0].cnt0++;
                    data.image[IDmodevalPF_C].md[0].write = 0;
                    ...
                    for (uint32_t m = 0; m < NBmodes; m++)
                    {
                        ...
                        if (data.image[IDmodeARPFgain].array.F[m] >
                            0.5) // if mode is predictive-controlled
                        {
                            if (data.image[IDmodevalPFres].array.F[m] *
                                    data.image[IDmodevalPFres].array.F[m] <
            ...
            if (ARPF_ok == 0)
            {
                memcpy(data.image[IDmodevalDMnow].array.F,
                       data.image[IDmodevalDMcorr].array.F,
            ...
            tdiffv = timespec_diff_double(
                data.image[aoloopcontrol_var.aoconfID_looptiming].md[0].atime,
                tnow);
            data.image[aoloopcontrol_var.aoconfID_looptiming].array.F[5] =
                tdiffv;

            data.image[IDmodevalDMnowfilt].md[0].write = 1;
            // FILTERING MODE VALUES
            // THIS FILTERING GOES TOGETHER WITH THE SECONDARY WRITE ON DM TO KEEP
            // FILTERED AND ACTUAL VALUES IDENTICAL
            for (uint32_t m = 0; m < NBmodes; m++)
            {
                data.image[IDmodevalDMnowfilt].array.F[m] =
                    data.image[IDmodevalDMnow].array.F[m];
            }

            if (AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_ON ==
                1) // automatically adjust modal limits
            {
                data.image[IDatlimbcoeff].md[0].write = 1;
                for (uint32_t block = 0;
                     block < AOconf[loop].AOpmodecoeffs.DMmodesNBblock;
                     block++)
                {
                    limitblockarray[block] = 0.0;
                }

                data.image[aoloopcontrol_var.aoconfID_LIMIT_modes].md[0].write =
                    1;
                ...
                for (uint32_t m = 0; m < NBmodes; m++)
                {
                    uint32_t block = data.image[IDblknb].array.UI16[m];

                    if (fabs(AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_mcoeff *
                             data.image[IDmodevalDMnowfilt].array.F[m]) >
                        modelimit[m])
                    {
                        data.image[aoloopcontrol_var.aoconfID_LIMIT_modes]
                            .array.F[m] *=
                            (1.0 +
                             AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_delta);
                    }
                    else
                    {
                        data.image[aoloopcontrol_var.aoconfID_LIMIT_modes]
                ...
                data.image[aoloopcontrol_var.aoconfID_LIMIT_modes].md[0].cnt1 =
                    LOOPiter;
                data.image[aoloopcontrol_var.aoconfID_LIMIT_modes].md[0].write =
                    0;
                ...
                for (uint32_t block = 0;
                     ...
                     block++)
                {
                    data.image[IDatlimbcoeff].array.F[block] =
                        limitblockarray[block] / blockNBmodes[block];
                    coeff = (1.0 +
                             (data.image[IDatlimbcoeff].array.F[block] - 1.0) *
                                 AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_delta *
                                 0.1);
                    if (coeff <
                        1.0 - AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_delta)
                    {
                        coeff =
                            1.0 - AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_delta;
                    }
                ...
                data.image[aoloopcontrol_var.aoconfID_limitb].md[0].cnt1 =
                    LOOPiter;
            ...
            if (AOconf[loop].AOAutoTune.AUTOTUNE_GAINS_ON == 1)
            {
                ...
                if (IDautogain != -1)
                {
                    if (data.image[IDautogain].md[0].cnt0 != autogainCnt)
                    {
                        ...
                        printf(
                            "[%ld %s] Updated autogain [%12lu  %12lu] -> "
                            "applying gains\n",
                            IDautogain,
                            data.image[IDautogain].md[0].name,
                            autogainCnt,
                            data.image[IDautogain].md[0].cnt0);
                        fflush(stdout);
                        ...
                        sprintf(command,
                                "echo \"%6.4f\" > conf/param_loopgain.txt",
                        ...
                        // Set block gain to max gain within block, scaled to
                        // global gain
                        for (uint32_t block = 0;
                             block < AOconf[loop].AOpmodecoeffs.DMmodesNBblock;
                             block++)
                        {
                            ...
                            for (uint32_t m = 0; m < NBmodes; m++)
                                if (data.image[IDblknb].array.UI16[m] == block)
                                    if (data.image[IDautogain].array.F[m] >
                                        maxGainVal)
                                    {
                                        maxGainVal =
                                            data.image[IDautogain].array.F[m];
                                    }

                            printf("Set block %2u gain to  %f\n",
                                   block,
                                   maxGainVal / globalgain);

                            data.image[aoloopcontrol_var.aoconfID_gainb]
                                .array.F[block] =
                                maxGainVal / AOconf[loop].aorun.gain;

                            sprintf(command,
                                    "echo \"%6.4f\" > conf/param_gainb%02u.txt",
                                    data.image[aoloopcontrol_var.aoconfID_gainb]
                                        .array.F[block],
                                    block);
                            if (system(command) != 0)
                ...
                /*			float alphagain = 0.1;
                                      ...
                                      data.image[aoconfID_GAIN_modes].md[0].write
                 = 1; data.image[aoconfID_gaiinb].md[0].write = 1;
                          ...
                          for(m=0; m<NBmodes; m++)
                          {
                              ...
                              data.image[aoconfID_GAIN_modes].array.F[m] =
                 (1.0-alphagain)*data.image[aoconfID_GAIN_modes].array.F[m] +
                 alphagain*data.image[autogain].array.F[m];

                              gainblockarray[block] +=
                 data.image[aoconfID_GAIN_modes].array.F[m];
                          }
                          COREMOD_MEMORY_image_set_sempost_byID(aoconfID_GAIN_modes,
                 -1); data.image[aoconfID_GAIN_modes].md[0].cnt0++;
                          data.image[aoconfID_GAIN_modes].md[0].write = 0;

                                      // update block gains to drive average
                 gain coefficients to 1 data.image[IDatgainbcoeff].md[0].write =
                 1; for(block=0;
                 block<AOconf[loop].AOpmodecoeffs.DMmodesNBblock; block++)
                          {
                              data.image[IDatlimbcoeff].array.F[block] =
                 limitblockarray[block] / blockNBmodes[block]; coeff = ( 1.0 +
                 (data.image[IDatlimbcoeff].array.F[block]-1.0)*AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_delta*0.1
                 ...
                 * coeff;
                          ...
                          COREMOD_MEMORY_image_set_sempost_byID(IDatlimbcoeff,
                 -1); data.image[IDatlimbcoeff].md[0].cnt0++;
                          data.image[IDatlimbcoeff].md[0].write = 0;


                          COREMOD_MEMORY_image_set_sempost_byID(aoloopcontrol_var.aoconfID_limitb,
                 -1);
                 data.image[aoloopcontrol_var.aoconfID_limitb].md[0].cnt0++;
                          data.image[aoloopcontrol_var.aoconfID_limitb].md[0].write
                 = 0;
                          */
            }
            ...
            if (FILTERMODE == 1)
            {
                for (uint32_t m = 0; m < NBmodes; m++)
                {
                    ...
                    if (data.image[IDmodevalDMnowfilt].array.F[m] >
                        modelimit[m])
                    {
            ...
            COREMOD_MEMORY_image_set_sempost_byID(IDmodevalDMnow, -1);
            data.image[IDmodevalDMnow].md[0].cnt1 =
                LOOPiter; // modevalDMindex; //TBC
            data.image[IDmodevalDMnow].md[0].cnt0++;
            data.image[IDmodevalDMnow].md[0].write = 0;
            ...
            else
            {
                ...
                COREMOD_MEMORY_image_set_sempost_excl_byID(IDmodevalDMnowfilt,
                                                           2);
            }
            data.image[IDmodevalDMnowfilt].md[0].cnt1 =
                LOOPiter; // modevalDMindex; //TBC
            ...
            {
                data.image[IDmodevalDM_C]
                    .array.F[modevalDMindex * NBmodes + m] =
                    data.image[IDmodevalDMnowfilt].array.F[m];
            }
            COREMOD_MEMORY_image_set_sempost_byID(IDmodevalDM_C, -1);
            ...
            tdiffv = timespec_diff_double(
                data.image[aoloopcontrol_var.aoconfID_looptiming].md[0].atime,
                tnow);
            data.image[aoloopcontrol_var.aoconfID_looptiming].array.F[9] =
                tdiffv;
            ...
            for (uint32_t m = 0; m < NBmodes; m++)
            {
                data.image[IDmodevalDM].array.F[m] =
                    (1.0 - alpha) *
                        data.image[IDmodevalDM_C]
                            .array.F[modevalDMindex0 * NBmodes + m] +
                    alpha * data.image[IDmodevalDM_C]
                                .array.F[modevalDMindex1 * NBmodes + m];
                COREMOD_MEMORY_image_set_sempost_byID(IDmodevalDM, -1);
            }
            data.image[IDmodevalDM].md[0].cnt0++;
            ...
            tdiffv = timespec_diff_double(
                data.image[aoloopcontrol_var.aoconfID_looptiming].md[0].atime,
                tnow);
            data.image[aoloopcontrol_var.aoconfID_looptiming].array.F[10] =
                tdiffv;
            ...
            if (AOconf[loop].aorun.ARPFon == 1)
            {
                ...
                for (uint32_t m = 0; m < NBmodes; m++)
                {
                    data.image[IDmodevalPFsync].array.F[m] =
                        (1.0 - alpha) *
                            data.image[IDmodevalPF_C]
                                .array.F[modevalPFindex0 * NBmodes + m] +
                        alpha * data.image[IDmodevalPF_C]
                                    .array.F[modevalPFindex1 * NBmodes + m];
                }
            ...
            tdiffv = timespec_diff_double(
                data.image[aoloopcontrol_var.aoconfID_looptiming].md[0].atime,
                tnow);
            data.image[aoloopcontrol_var.aoconfID_looptiming].array.F[11] =
                tdiffv;
            ...
            tdiffv = timespec_diff_double(
                data.image[aoloopcontrol_var.aoconfID_looptiming].md[0].atime,
                tnow);
            data.image[aoloopcontrol_var.aoconfID_looptiming].array.F[12] =
                tdiffv;
            ...
            for (uint32_t m = 0; m < NBmodes; m++)
            {
                ...
                blockavePFresrms[block] +=
                    data.image[IDmodevalPFres].array.F[m] *
                    data.image[IDmodevalPFres].array.F[m];
                blockaveOLrms[block] +=
                    data.image[IDout].array.F[m] * data.image[IDout].array.F[m];
                blockaveCrms[block] += data.image[IDmodevalDMnow].array.F[m] *
                                       data.image[IDmodevalDMnow].array.F[m];
                blockaveWFSrms[block] += data.image[IDmodeval].array.F[m] *
                                         data.image[IDmodeval].array.F[m];

                blockaveWFSnoise[block] +=
                    data.image[IDmodeWFSnoise].array.F[m];
            }
            ...
            if (blockstatcnt == AOconf[loop].AOpmodecoeffs.AveStats_NBpt)
            {
                for (uint32_t block = 0;
                     ...
                     block++)
                {
                    AOconf[loop].AOpmodecoeffs.blockave_PFresrms[block] =
                        sqrt(blockavePFresrms[block] / blockstatcnt);
                    AOconf[loop].AOpmodecoeffs.blockave_OLrms[block] =
                        sqrt(blockaveOLrms[block] / blockstatcnt);
                    AOconf[loop].AOpmodecoeffs.blockave_Crms[block] =
                        sqrt(blockaveCrms[block] / blockstatcnt);
                    AOconf[loop].AOpmodecoeffs.blockave_WFSrms[block] =
                        sqrt(blockaveWFSrms[block] / blockstatcnt);
                    AOconf[loop].AOpmodecoeffs.blockave_WFSnoise[block] =
                        sqrt(blockaveWFSnoise[block] / blockstatcnt);
                    AOconf[loop].AOpmodecoeffs.blockave_limFrac[block] =
                        (blockavelimFrac[block]) / blockstatcnt;
                ...
                AOconf[loop].AOpmodecoeffs.ALLave_OLrms =
                    sqrt(allaveOLrms / blockstatcnt);
                AOconf[loop].AOpmodecoeffs.ALLave_Crms =
                    sqrt(allaveCrms / blockstatcnt);
                AOconf[loop].AOpmodecoeffs.ALLave_WFSrms =
                    sqrt(allaveWFSrms / blockstatcnt);
                AOconf[loop].AOpmodecoeffs.ALLave_WFSnoise =
                    sqrt(allaveWFSnoise / blockstatcnt);
                AOconf[loop].AOpmodecoeffs.ALLave_limFrac =
                    allavelimFrac / blockstatcnt;
            ...
            tdiffv = timespec_diff_double(
                data.image[aoloopcontrol_var.aoconfID_looptiming].md[0].atime,
                tnow);
            data.image[aoloopcontrol_var.aoconfID_looptiming].array.F[13] =
                tdiffv;
