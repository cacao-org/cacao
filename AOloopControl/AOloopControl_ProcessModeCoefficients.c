...
imageID __attribute__((hot)) AOloopControl_ProcessModeCoefficients(long loop)
{
    ...
    if (aoloopcontrol_var.aoconfID_modeARPFgainAuto == -1)
    {
        ...
        create_image_ID(imname, 2, sizeout, _DATATYPE_FLOAT, 1, 0, 0, &(aoloopcontrol_var.aoconfID_modeARPFgainAuto));
        COREMOD_MEMORY_image_set_createsem(imname, 10);
        // initialize the gain to zero for all modes
        for (uint32_t m = 0; m < NBmodes; m++)
        {
    ...
    while (loopOK == 1)
    {
        ...
        if (loopOK == 1)
        {
                ...
                else
                {
                    ...
                    for (uint32_t m = 0; m < NBmodes; m++)
                    {
                        ...
                        data.image[IDmodevalDMnow].array.F[m] =
                            -mixratio * data.image[aoloopcontrol_var.aoconfID_modevalPF].array.F[m] +
                ...
                if (IDautogain != -1)
                {
                    if (data.image[IDautogain].md[0].cnt0 != autogainCnt)
                    {
                        ...
                        printf("[%ld %s] Updated autogain [%12lu  %12lu] -> applying gains\n", IDautogain,
                               data.image[IDautogain].md[0].name, autogainCnt, data.image[IDautogain].md[0].cnt0);
                        fflush(stdout);
            ...
            if ((AOconf[loop].aorun.DMfilteredWriteON == 1) && (AOconf[loop].DMctrl.DMMODE == 1))
            {
                ...
                memcpy(data.image[aoloopcontrol_var.aoconfID_dmC].array.F, data.image[IDmodevalDMnowfilt].array.F,
                       sizeof(float) * NBmodes);
            ...
            if (AOconf[loop].aorun.ARPFon == 1)
            {
                ...
                for (uint32_t m = 0; m < NBmodes; m++)
                {
                    data.image[IDmodevalPFsync].array.F[m] =
                        (1.0 - alpha) * data.image[IDmodevalPF_C].array.F[modevalPFindex0 * NBmodes + m] +
