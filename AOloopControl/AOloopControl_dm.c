...
errno_t set_DM_modes(long loop)
{
        ...
        for (unsigned long i = 0; i < AOconf[loop].DMctrl.sizeDM; i++)
            for (unsigned long k = 0; k < AOconf[loop].AOpmodecoeffs.NBDMmodes; k++)
            {
                arrayf[i] += data.image[aoloopcontrol_var.aoconfID_cmd_modes].array.F[k] *
                             data.image[aoloopcontrol_var.aoconfID_DMmodes].array.F[k * AOconf[loop].DMctrl.sizeDM + i];
            }
    ...
    else
    {
        ...
        GPU_loop_MultMat_execute(1, &AOconf[loop].AOtiminginfo.status, &AOconf[loop].AOtiminginfo.GPUstatus[0], 1.0,
                                 0.0, 1, 30);
#endif
    }
                                                 ...
                                                 int GPUindex, long loop, int offloadMode)
{
    ...
    GPU_loop_MultMat_setup(GPUMATMULTCONFindex, DMmodes_name, modecoeffs_name, out_name, GPUcnt, GPUsetM, orientation,
                           use_sem, initWFSref, 0);
...
imageID AOloopControl_dm2dm_offload(const char *streamin, const char *streamout, float twait, float offcoeff,
                                    float multcoeff)
{
    imageID IDin, IDout;
    long cnt = 0;
    ...
    while (1)
    {
        ...
        for (ii = 0; ii < xysize; ii++)
        {
            data.image[IDout].array.F[ii] =
                multcoeff * (data.image[IDout].array.F[ii] + offcoeff * data.image[IDin].array.F[ii]);
        }
