    ...
    if (AOconf[loop].AOcompute.GPU1 == 0)
        {
            ...
            memcpy(
                data.image[aoloopcontrol_var.aoconfID_dmC].array.F, arrayf, sizeof(float) * AOconf[loop].DMctrl.sizeDM);
    ...
    else
        {
            ...
            GPU_loop_MultMat_execute(
                1, &AOconf[loop].AOtiminginfo.status, &AOconf[loop].AOtiminginfo.GPUstatus[0], 1.0, 0.0, 1, 30);
#endif
        }
...
imageID
AOloopControl_dm2dm_offload(const char *streamin, const char *streamout, float twait, float offcoeff, float multcoeff)
{
    imageID IDin, IDout;
    long cnt = 0;
