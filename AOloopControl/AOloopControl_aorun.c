...
int AOloopControl_aorun_FPCONF()
{
    ...
    */
    ...
    function_parameter_add_entry(&fps, ".semwaitindex", "input semaphore index", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT,
                                 &semwaitindex_default, &fpi_semwaitindex);

    long WFSrefON_default[4] = {0, 0, 1, 0};
    long fpi_WFSrefON = 0;
    function_parameter_add_entry(&fps, ".wfsrefON", "Use WFS reference", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT,
                                 &WFSrefON_default, &fpi_WFSrefON);
    ...
    function_parameter_add_entry(&fps, ".sn_wfs", "WFS stream name", FPTYPE_STREAMNAME, FPFLAG, "NULL",
                                 &fpi_streamname_wfs);

    FPFLAG = FPFLAG_DEFAULT_INPUT_STREAM | FPFLAG_STREAM_RUN_REQUIRED;
    long fpi_streamname_cmat = 0;
    function_parameter_add_entry(&fps, ".sn_cmat", "Control Matrix", FPTYPE_STREAMNAME, FPFLAG, "NULL",
                                 &fpi_streamname_cmat);

    // required to get DM size
    FPFLAG = FPFLAG_DEFAULT_INPUT_STREAM | FPFLAG_STREAM_RUN_REQUIRED;
    long fpi_streamname_DMout = 0;
    function_parameter_add_entry(&fps, ".sn_DMout", "output stream", FPTYPE_STREAMNAME, FPFLAG, "NULL",
    ...
    long fpi_loopgain = 0;
    function_parameter_add_entry(&fps, ".loopgain", "Main loop gain", FPTYPE_FLOAT64, FPFLAG, &loopgaindefault,
                                 &fpi_loopgain);
    ...
    FPFLAG |= FPFLAG_WRITERUN;
    long fpi_loopmult = 0;
    function_parameter_add_entry(&fps, ".loopmult", "Main loop mult coeff", FPTYPE_FLOAT64, FPFLAG, &loopmultdefault,
                                 &fpi_loopmult);
    ...
    function_parameter_add_entry(&fps, ".loopNBstep", "Loop number steps", FPTYPE_INT64, FPFLAG, &loopNBstep_default,
                                 &fpi_loopNBstep);
...
int AOloopControl_aorun_RUN()
{
    ...
    while (loopOK == 1)
    {
        ...
        if (processinfo_compute_status(processinfo) == 1)
        {
            ...
            if (wfsrefON == 1) // if WFS reference is NOT zero
            {
                for (long ii = 0; ii < sizeWFS; ii++)
                {
                    data.image[IDimWFS2].array.F[ii] =
                        data.image[IDimWFS1].array.F[ii] - normfloorcoeff * data.image[IDwfsref].array.F[ii];
                }
            }
            else
            {
            ...
            ControlMatrixMultiply(data.image[ID_CM].array.F, data.image[IDimWFS2].array.F, sizeDM, sizeWFS,
...
errno_t __attribute__((hot)) AOloopControl_aorun()
{
    ...
    if (vOK == 1)
    {
        ...
        while (AOconf[loop].aorun.kill == 0)
        {
            ...
            sprintf(pinfomsg, "WAITING (on=%d   loopOK=%d) [%d]", AOconf[loop].aorun.on, loopOK,
                    AOconf[loop].aorun.CMMODE);
            ...
            while ((AOconf[loop].aorun.on == 1) && (loopOK == 1))
            {
                ...
                else // 1 step: WFS -> DM act
                {
                    if (AOconf[loop].aorun.DMprimaryWriteON == 1) // if Writing to DM
                    {
                        ...
                        clock_gettime(CLOCK_REALTIME, &tnow);
                        tdiffv =
                            timespec_diff_double(data.image[aoloopcontrol_var.aoconfID_looptiming].md[0].atime, tnow);
                        ...
                        if (doComputation == 1)
                        {
                            for (unsigned long ii = 0; ii < AOconf[loop].DMctrl.sizeDM; ii++)
                            {
                                data.image[aoloopcontrol_var.aoconfID_dmC].array.F[ii] -=
                                    ...
                                    data.image[aoloopcontrol_var.aoconfID_meas_act].array.F[ii];
