...
int AOloopControl_aorun_ProcessInit() { return 0; }
...
int AOloopControl_aorun_FPCONF()
{
    ...
    long fpi_loop = 0;
    function_parameter_add_entry(&fps, ".loop", "Loop index", FPTYPE_INT64, FPFLAG, &loopindex_default, &fpi_loop);

    /*
      int64_t RTpriority_default[4] = { 90, 0, 99, 90 };
      FPFLAG = FPFLAG_DEFAULT_INPUT | FPFLAG_MINLIMIT | FPFLAG_MAXLIMIT;
      long fpi_RTpriority = 0;
          function_parameter_add_entry(&fps, ".RTpriority", "Real Time
     priority", FPTYPE_INT64, FPFLAG, &RTpriority_default &fpi_RTpriority);
  */
    ...
    function_parameter_add_entry(&fps,
                                 ...
                                 &semwaitindex_default,
                                 &fpi_semwaitindex);

    long WFSrefON_default[4] = {0, 0, 1, 0};
    long fpi_WFSrefON = 0;
    function_parameter_add_entry(
        &fps, ".wfsrefON", "Use WFS reference", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, &WFSrefON_default, &fpi_WFSrefON);
    ...
    function_parameter_add_entry(
        &fps, ".sn_wfs", "WFS stream name", FPTYPE_STREAMNAME, FPFLAG, "NULL", &fpi_streamname_wfs);

    FPFLAG = FPFLAG_DEFAULT_INPUT_STREAM | FPFLAG_STREAM_RUN_REQUIRED;
    long fpi_streamname_cmat = 0;
    function_parameter_add_entry(
        &fps, ".sn_cmat", "Control Matrix", FPTYPE_STREAMNAME, FPFLAG, "NULL", &fpi_streamname_cmat);

    // required to get DM size
    FPFLAG = FPFLAG_DEFAULT_INPUT_STREAM | FPFLAG_STREAM_RUN_REQUIRED;
    long fpi_streamname_DMout = 0;
    function_parameter_add_entry(
        &fps, ".sn_DMout", "output stream", FPTYPE_STREAMNAME, FPFLAG, "NULL", &fpi_streamname_DMout);
    ...
    long fpi_loopgain = 0;
    function_parameter_add_entry(
        &fps, ".loopgain", "Main loop gain", FPTYPE_FLOAT64, FPFLAG, &loopgaindefault, &fpi_loopgain);
    ...
    long fpi_loopmult = 0;
    function_parameter_add_entry(
        &fps, ".loopmult", "Main loop mult coeff", FPTYPE_FLOAT64, FPFLAG, &loopmultdefault, &fpi_loopmult);

    double maxlimdefault[4] = {1.0, 0.0, 1000.0, 1.0};
    FPFLAG = FPFLAG_DEFAULT_INPUT | FPFLAG_MINLIMIT | FPFLAG_MAXLIMIT; // required to enforce the min and max limits
    FPFLAG |= FPFLAG_WRITERUN;
    long fpi_maxlim = 0;
    function_parameter_add_entry(&fps, ".maxlim", "Maximum limit", FPTYPE_FLOAT64, FPFLAG, &maxlimdefault, &fpi_maxlim);

    FPFLAG = FPFLAG_DEFAULT_INPUT;
    FPFLAG |= FPFLAG_WRITERUN;
    long fpi_loopON = 0;
    function_parameter_add_entry(&fps, ".loopON", "loop ON/OFF", FPTYPE_ONOFF, FPFLAG, pNull, &fpi_loopON);
    ...
    function_parameter_add_entry(
        &fps, ".loopNBstep", "Loop number steps", FPTYPE_INT64, FPFLAG, &loopNBstep_default, &fpi_loopNBstep);

    FPFLAG = FPFLAG_DEFAULT_INPUT;
    FPFLAG |= FPFLAG_WRITERUN;
    long fpi_loopZERO = 0;
    function_parameter_add_entry(&fps, ".loopZERO", "Zero correction", FPTYPE_ONOFF, FPFLAG, pNull, &fpi_loopZERO);
...
int AOloopControl_aorun_RUN()
{
  ...
  interface\n", fpsname); return RETURN_FAILURE;
    ...
    // This parameter value will be tracked during loop run, so we create a
    // pointer for it The corresponding function is
    // functionparameter_GetParamPtr_<TYPE>
    //
    ...
    int RTpriority = functionparameter_GetParamValue_INT64(&fps, ".procinfo.RTprio");
    ...
    processinfo->RT_priority = RTpriority; // RT_priority, 0-99. Larger number =
    ...
    imageID IDimWFS2;
    create_image_ID(imWFS2sname, naxis, imsizearray, _DATATYPE_FLOAT, 1, 10, 0, &IDimWFS2);
    free(imsizearray);
    ...
    imageID ID_MVMout;
    create_image_ID(outMVMsname, naxis, imsizearray, _DATATYPE_FLOAT, 1, 10, 0, &ID_MVMout);
    free(imsizearray);
                    ...
                    if (semval > 0)
                        {
                            if (semval > 1)
                                {
                                    printf("\n\033[31;1m WARNING [%d] WFS SEMAPHORE already "
                                           "posted - Missed "
                                           "frame\033[0m\n",
                                           semval);
                                }
            ...
            if (processinfo_compute_status(processinfo) == 1)
                {
                    ...
                    if (wfsrefON == 1) // if WFS reference is NOT zero
                        {
                            for (long ii = 0; ii < sizeWFS; ii++)
                                {
                                    data.image[IDimWFS2].array.F[ii] =
                                        data.image[IDimWFS1].array.F[ii] -
                                        normfloorcoeff * data.image[IDwfsref].array.F[ii];
                                }
                        }
                    else
                        {
                    ...
                    ControlMatrixMultiply(data.image[ID_CM].array.F,
                                          data.image[IDimWFS2].array.F,
                                          sizeDM,
                                          sizeWFS,
                                          data.image[ID_MVMout].array.F);
                    ...
                    if (*loopONflag & FPFLAG_ONOFF)
                        {
                            ...
                            for (long ii = 0; ii < sizeDM; ii++)
                                {
                                    float tmpval = data.image[ID_DMout].array.F[ii] -
                                                   (*loopgain) * data.image[ID_MVMout].array.F[ii];
                                    tmpval *= (*loopmult);
                                    if (tmpval > *maxlim)
                                        {
                                            tmpval = (*maxlim);
 ...
 * Then, the correction is applied by calling set_DM_modes() if modal
 * correction, or by direct write to the DM otherwise.\n
 *
 *
 * ## Details
 *
    ...
    if (aoloopcontrol_var.COMPUTE_PIXELSTREAMING == 1)
        {
            ...
            for (unsigned int ii = 0; ii < xsize * ysize; ii++)
                {
                    if (data.image[aoloopcontrol_var.aoconfID_pixstream_wfspixindex].array.UI16[ii] >
                        aoloopcontrol_var.PIXSTREAM_NBSLICES)
                        {
                            aoloopcontrol_var.PIXSTREAM_NBSLICES =
                                data.image[aoloopcontrol_var.aoconfID_pixstream_wfspixindex].array.UI16[ii];
                        }
                }
            aoloopcontrol_var.PIXSTREAM_NBSLICES++;
    ...
    if (vOK == 1)
        {
            ...
            while (AOconf[loop].aorun.kill == 0)
                    ...
                    while ((AOconf[loop].aorun.on == 1) && (loopOK == 1))
                        {
...
#ifdef _PRINT_TEST
                            printf("[%s] [%d]  AOloopControl_aorun: Starting AOcompute, "
                                   "AOconf[%d].WFSim.WFSnormalize "
                                   "= %d\n",
                                   __FILE__,
                                   __LINE__,
                                   loop,
                            ...
                            // CTRLval = 5 will disable computations in loop (usually for
                            // testing)
                            int doComputation = 1;
                            if (data.processinfo == 1)
                            ...
                            //
                            // note: processinfo_exec_start is launched in Read_cam_frame() in
                            // AOcompute()
                            //
                            ...
                            tdiffv = timespec_diff_double(data.image[aoloopcontrol_var.aoconfID_looptiming].md[0].atime,
                                                          tnow);
                                    ...
                                    if (AOconf[loop].aorun.DMprimaryWriteON == 1) // if Writing to DM
                                        {
                                            ...
                                            for (unsigned long ii = 0; ii < AOconf[loop].DMctrl.sizeDM; ii++) // TEST
                                                {
                                                    if (isnan(data.image[aoloopcontrol_var.aoconfID_meas_act]
                                                                  .array.F[ii]) != 0)
                                                        {
                                                            printf("image aol2_meas_act  element %ld is "
                                                                   "NAN -> "
                                                                   "replacing by 0\n",
                                                                   ii);
                                            ...
                                            if (doComputation == 1)
                                                {
                                                    for (unsigned long ii = 0; ii < AOconf[loop].DMctrl.sizeDM; ii++)
                                                        {
                                                            data.image[aoloopcontrol_var.aoconfID_dmC].array.F[ii] -=
                                                            ...
                                                            if (data.image[aoloopcontrol_var.aoconfID_dmC].array.F[ii] >
                                                                AOconf[loop].aorun.maxlimit)
                                                                {
                                                                    data.image[aoloopcontrol_var.aoconfID_dmC]
                                                                        .array.F[ii] = AOconf[loop].aorun.maxlimit;
                                                                }
                                                            if (data.image[aoloopcontrol_var.aoconfID_dmC].array.F[ii] <
                                                                -AOconf[loop].aorun.maxlimit)
                                                                {
                                            ...
                                            // inform dmdisp that new command is ready in one of the
                                            // channels
                                                ...
                                                if (data.image[aoloopcontrol_var.aoconfID_dmdisp].md[0].sem > 1)
                                                    {
                                                        ...
                                                        if (semval < SEMAPHORE_MAXVAL)
                                                            {
                                                                sem_post(data.image[aoloopcontrol_var.aoconfID_dmdisp]
                                                                             .semptr[1]);
                                                            }
                                                    }
              ...
              if(tdiffv > 30.0e-6)
              {
                      printf("TIMING WARNING: %12.3f us  %10ld
              AOloopControl_aorun()
              - excluding AOcompute\n", tdiffv*1.0e6, (long)
              AOconf[loop].aorun.LOOPiteration); fflush(stdout);
              }*/
              ...
              if(tdiffv > 600.0e-6)
              {
                      printf("TIMING WARNING: %12.3f us  %10ld
              AOloopControl_aorun()\n", tdiffv*1.0e6, (long)
              AOconf[loop].aorun.LOOPiteration); printf("    AOcompute() read
              cam : %12.3f us \n", tdiffv00*1.0e6); printf("    AOcompute() post
              read cam   : %12.3f us \n", tdiffv01*1.0e6); printf("
              AOloopControl_aorun()    post-AOcompute  : %12.3f us \n",
              tdiffv02*1.0e6);
