    ...
    if ((AOconf[loop].RTSLOGarray[rtlindex].ENABLE == 1) && (AOconf[loop].RTSLOGarray[rtlindex].INIT == 0))
        {
            ...
            create_image_ID(
                imname, 2, imsize, _DATATYPE_UINT64, 1, 0, 0, &(AOconf[loop].RTSLOGarray[rtlindex].IDbuffinfo0));
            ...
            create_image_ID(
                imname, 2, imsize, _DATATYPE_UINT64, 1, 0, 0, &(AOconf[loop].RTSLOGarray[rtlindex].IDbuffinfo1));
    ...
    if (aoloopcontrol_var.RTSLOGarrayInitFlag[rtlindex] == 1) // ensure local ownership
        {
            if ((AOconf[loop].RTSLOGarray[rtlindex].ENABLE == 1) && (AOconf[loop].RTSLOGarray[rtlindex].ON == 1) &&
                (AOconf[loop].RTSLOGarray[rtlindex].INIT = 1))
                {
                    ...
                    data.image[IDinfo].array.UI64[AOconf[loop].RTSLOGarray[rtlindex].frameindex * 5 + 1] =
                        (long)tnow.tv_sec;
                    data.image[IDinfo].array.UI64[AOconf[loop].RTSLOGarray[rtlindex].frameindex * 5 + 2] =
                        (long)tnow.tv_nsec;
                    ...
                    if (AOconf[loop].RTSLOGarray[i].save == 1)
                        {
                            if (AOconf[loop].RTSLOGarray[i].memcpToggle != 0)
                                {
                                    sprintf(
                                        SAstring, "\033[1;31m ON[%1d]\033[0m", AOconf[loop].RTSLOGarray[i].memcpToggle);
                                }
                            else
                                {
                                    sprintf(
                                        SAstring, "\033[1;32m ON[%1d]\033[0m", AOconf[loop].RTSLOGarray[i].memcpToggle);
                                }
                        }
                    else
                        {
    ...
    while (loopOK == 1)
        {
            ...
            printw("  s: Save single    t: Add/remove to/from set   S: Save set ON "
                   "  U: Save set OFF\n");
            printw("  o: ON/OFF single  O: Set ON   F: Set OFF      Z: Zero index "
                   "set\n");
            attroff(A_BOLD);
...
int AOloopControl_RTstreamLOG_saveloop(int loop, char *dirname)
{
    ...
    int InBuffIndex[MAX_NUMBER_RTLOGSTREAM];  // input buffer index. Usually there
                                              // are two such files, so value is 0
                                              // or 1
    int OutBuffIndex[MAX_NUMBER_RTLOGSTREAM]; // large file buffer index. Usually
                            ...
                            if (AOconf[loop].RTSLOGarray[rtlindex].memcpToggle != 0) // Input buffer ready
                                {
                                    ...
                                    InBuffIndex[rtlindex] =
                                        AOconf[loop].RTSLOGarray[rtlindex].memcpToggle - 1; // buffindex to save
                                            ...
                                            if (zsizesave > AOconf[loop].RTSLOGarray[rtlindex].SIZE)
                                                {
                                                    ...
                                                    printf("     zsizesave                             "
                                                           "   = %d\n",
                                                           zsizesave);
                                                    printf("     AOconf[loop].RTSLOGarray[rtlindex].SIZE  "
                                                           "= %d\n",
                                            ...
                                            if (IDout == -1) // create large buffer if it does not exist
                                                {
                                                    ...
                                                    create_image_ID(
                                                        OutBuffIm, 3, imsize, datatype, SHARED, 0, 0, &IDout);
                                                    free(imsize);
                                                }
                                                    ...
                                                    if (tOK[rtlindex] == 1)
                                                        {
                                                            ...
                                                            if (pthread_tryjoin_np(thread_savefits[rtlindex], NULL) ==
                                                                EBUSY)
                                                                {
                                                                    if (VERBOSE > 0)
                                                                        {
                                                                            printf("%5d  PREVIOUS SAVE THREAD NOT "
                                                                                   "TERMINATED -> waiting\n",
                                                                                   __LINE__);
                                                                            fflush(stdout);
                                                                        }
                                                                    ...
                                                                    if (VERBOSE > 0)
                                                                        {
                                                                            printf("%5d  PREVIOUS SAVE THREAD NOW "
                                                                ...
                                                                {
                                                                    printf("%5d  PREVIOUS SAVE THREAD ALREADY "
                                                                           "COMPLETED -> OK\n",
                                                                           __LINE__);
                                                                    fflush(stdout);
                                                                }
