...
errno_t AOloopControl_InitializeMemory(int mode)
{
    ...
    AOconf = (AOLOOPCONTROL_CONF *)mmap(
        0, sizeof(AOLOOPCONTROL_CONF) * NB_AOloopcontrol, PROT_READ | PROT_WRITE, MAP_SHARED, SM_fd, 0);
            ...
            if ((aoloopcontrol_var.aoconfIDlogdata = image_ID(cntname)) == -1)
                {
                    ...
                    sizearray[1] = 1;
                    create_image_ID(
                        cntname, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &(aoloopcontrol_var.aoconfIDlogdata));
                    free(sizearray);
                }
