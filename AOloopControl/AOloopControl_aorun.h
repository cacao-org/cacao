...
typedef struct
{
    ...
    char             name[80];
    int
    CMMODE;                     /**< Combined matrix. 0: matrix is WFS pixels -> modes, 1: matrix is WFS pixels -> DM actuators */


    /* STATUS ******************************************************** */

    int
    init;                       /**< Has the structure been initialized ? */
    uint64_t
    cnt;                        /**<  loop step counter, set to zero every time loop is stopped */
    uint64_t
    cntmax;                     /**<  max value of counter, used to step loop */
    ...
    int      initmapping;
    uint64_t
    LOOPiteration;              /**< Loop iteration - set to zero on creation on aolrun start */
    ...
    float
    maxlimit;                   /**< maximum absolute value for mode values */
    float
    mult;                       /**< multiplication coefficient to be applied at each loop iteration */
...
typedef struct
{
    ...
    float
    hardwlatency;               /**< hardware latency between DM command and WFS response [sec] */
    float
    hardwlatency_frame;         /**< hardware latency between DM command and WFS response [frame] */

    // Computation time for direct WFS->DM mode through single matrix multiplication
    float            complatency;                /**< Computation latency [sec] */
    float
    complatency_frame;          /**< Computation latency (main loop) from WFS image reception to DM command output [frame] */

    // Computation time for full computation including open loop computation
    float
    wfsmextrlatency;            /**< WFS mode extraction latency [sec] */
    float
    wfsmextrlatency_frame;      /**< WFS mode extraction latency [frame] */
...
typedef struct
{
    ...
    uint32_t
    sizeWFS;                    /**< WFS total image (= x size * y size) */
    uint32_t    activeWFScnt;               /**< Number of active WFS pixels */
    uint32_t
    sizeWFS_active[100];        /**< Only takes into account WFS pixels in use/active for each slice */
    uint64_t
    WFScnt;                     /**< WFS stream counter 0 value at WFS image read */
    uint64_t
    WFScntRM;                   /**< WFS stream counter 0 value at WFS image read (RM acqu mode) */

    int      WFSnormalize;               /**< 1 if each WFS frame should be normalized to 1 */
    int      WFSrefzero;                 /**< 1 if WFS reference is zero */
    float
    WFSnormfloor;               /**< normalized by dividing by (total + AOconf[loop].WFSnormfloor)*AOconf[loop].WFSsize */
    float
    WFStotalflux;               /**< Total WFS flux after dark subtraction */
