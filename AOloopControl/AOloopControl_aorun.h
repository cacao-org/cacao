...
typedef struct
{
    ...
    char contrMname[80];
    char name[80];
    int CMMODE; /**< Combined matrix. 0: matrix is WFS pixels -> modes, 1: matrix
                 is WFS pixels -> DM actuators */
    ...
    int initmapping;
    uint64_t LOOPiteration; /**< Loop iteration - set to zero on creation on
                             aolrun start */
    ...
    float mult;     /**< multiplication coefficient to be applied at each loop
                     iteration */
...
typedef struct
{
    ...
    // Computation time for direct WFS->DM mode through single matrix
    // multiplication
    float complatency;       /**< Computation latency [sec] */
    float complatency_frame; /**< Computation latency (main loop) from WFS image
                              reception to DM command output [frame] */
...
typedef struct
{
    ...
    uint32_t sizeWFS_active[100]; /**< Only takes into account WFS pixels in
                                   use/active for each slice */
    uint64_t WFScnt;              /**< WFS stream counter 0 value at WFS image read */
    uint64_t WFScntRM;            /**< WFS stream counter 0 value at WFS image read (RM acqu
                        mode) */

    int WFSnormalize;   /**< 1 if each WFS frame should be normalized to 1 */
    int WFSrefzero;     /**< 1 if WFS reference is zero */
    float WFSnormfloor; /**< normalized by dividing by (total +
                         AOconf[loop].WFSnormfloor)*AOconf[loop].WFSsize */
