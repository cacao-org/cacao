...
errno_t AOloopControl_WFSzpupdate_loop(const char *IDzpdm_name, const char *IDzrespM_name, const char *IDwfszp_name)
{
    ...
    while (data.signal_USR1 == 0)
    {
        ...
        printf("WFS zero point offset update  # %8llu       (%s -> %s)  ", zpcnt, data.image[IDzpdm].name,
            ...
            for (act = 0; act < dmxysize; act++)
                for (elem = 0; elem < wfsxysize; elem++)
                {
                    data.image[IDtmp].array.F[elem] +=
                        data.image[IDzpdm].array.F[act] * data.image[IDzrespM].array.F[act * wfsxysize + elem];
                }
        }
...
//
//
errno_t AOloopControl_WFSzeropoint_sum_update_loop(__attribute__((unused)) long loopnb, const char *ID_WFSzp_name,
                                                   int NBzp, const char *IDwfsref0_name, const char *IDwfsref_name)
{
    uint32_t wfsxsize, wfsysize;
    uint64_t wfsxysize;
    ...
    for (;;)
    {
        ...
        for (ii = 0; ii < wfsxysize; ii++)
        {
            data.image[IDout].array.F[ii] =
                data.image[aoloopcontrol_var.aoconfID_contrM].array.F[ii] / data.image[IDtot].array.F[0] -
        ...
        for (ii = 0; ii < wfsxysize; ii++)
        {
            data.image[IDoutave].array.F[ii] =
                (1.0 - data.image[IDalpha].array.F[0]) * data.image[IDoutave].array.F[ii] +
        ...
        for (ii = 0; ii < wfsxysize; ii++)
        {
            data.image[IDoutrms].array.F[ii] =
                (1.0 - data.image[IDalpha].array.F[0]) * data.image[IDoutrms].array.F[ii] +
                data.image[IDalpha].array.F[0] * (data.image[IDout].array.F[ii] - data.image[IDoutave].array.F[ii]) *
