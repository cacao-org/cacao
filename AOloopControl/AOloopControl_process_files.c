...
errno_t AOloopControl_logprocess_modeval(const char *IDname)
{
    ...
    create_2Dimage_ID("modeval_ol_ave",
                      data.image[ID].md[0].size[0],
                      data.image[ID].md[0].size[1],
                      &IDout_ave);
    create_2Dimage_ID("modeval_ol_rms",
                      data.image[ID].md[0].size[0],
                      data.image[ID].md[0].size[1],
                      &IDout_rms);

    create_2Dimage_ID("modeval1d", data.image[ID].md[0].size[2], 1, &ID1dtmp);
    create_2Dimage_ID("modevalPSD",
                      data.image[ID].md[0].size[2] / 2,
                      1,
                      &ID1dPSD);
    ...
    for (m = 0; m < NBmodes; m++)
    {
        ...
        fprintf(fp,
                "%4ld  %12.8f  %12.8f\n",
                m,
                data.image[IDout_ave].array.F[m],
                data.image[IDout_rms].array.F[m]);
    }
    fclose(fp);
