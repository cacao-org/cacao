            ...
            for (kk = 0; kk < NBframes / 2; kk++)
                {
                    data.image[ID1dPSD].array.F[kk] =
                        data.image[IDft].array.CF[kk].re * data.image[IDft].array.CF[kk].re +
                              ...
                              __attribute__((unused)) char *RMoutname)
{
    ...
    if (dmsize != data.image[IDzrmin].md[0].size[2])
        {
            printf("ERROR: total number of DM actuators (%ld) does not match zsize "
                   "of RM (%ld)\n",
                   dmsize,
    ...
    if ((data.image[IDwfsin].md[0].size[0] != wfsxsize) || (data.image[IDwfsin].md[0].size[1] != wfsysize) ||
        ...
        {
            printf("ERROR: size of WFS mask image \"%s\" (%u %u %u) does not match "
                   "expected size (%u %u %u)\n",
                   WFSmaskname,
                   data.image[IDwfsin].md[0].size[0],
                   data.image[IDwfsin].md[0].size[1],
                   data.image[IDwfsin].md[0].size[2],
                   wfsxsize,
                   wfsysize,
                   NBframes);
            exit(0);
    ...
    if ((data.image[IDdmmask].md[0].size[0] != dmxsize) || (data.image[IDdmmask].md[0].size[1] != dmysize))
        {
            printf("ERROR: size of DM mask image \"%s\" (%u %u) does not match "
                   "expected size (%u %u)\n",
                   DMmaskname,
                   data.image[IDdmmask].md[0].size[0],
                   data.image[IDdmmask].md[0].size[1],
                   dmxsize,
                   dmysize);
            exit(0);
        }
