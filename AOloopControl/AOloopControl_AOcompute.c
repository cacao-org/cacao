...
#ifdef _PRINT_TEST
                            printf("[%s] [%d]  AOcompute: aoloopcontrol_var.normfloorcoeff "
                                   "= %f\n",
                                   __FILE__,
                                   __LINE__,
                                   aoloopcontrol_var.normfloorcoeff);
            ...
            if (AOconf[loop].aorun.initmapping == 0) // compute combined control matrix or matrices
                {
                            ...
                            for (slice = 0; slice < aoloopcontrol_var.PIXSTREAM_NBSLICES; slice++)
                                {
                                        ...
                                        if (data.image[aoloopcontrol_var.aoconfID_wfsmask].array.F[ii] > 0.1)
                                            {
                                                if (slice == 0)
                                                    {
                                                        aoloopcontrol_var
                                                            .WFS_active_map[slice * AOconf[loop].WFSim.sizeWFS + ii1] =
                                                            ii;
                                                        ii1++;
                                                    }
                                                else if (data.image[aoloopcontrol_var.aoconfID_pixstream_wfspixindex]
                                    ...
                                    char imname[200];
                                    if (sprintf(
                                            imname, "aol%ld_imWFS2active_%02d", aoloopcontrol_var.LOOPNUMBER, slice) <
                                        1)
                                        {
                    ...
                    create_image_ID(
                        imname, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &(aoloopcontrol_var.aoconfID_meas_act_active));
                    free(sizearray);
                    ...
                    if (aoloopcontrol_var.aoconfID_meas_act == -1)
                        {
                            ...
                            create_image_ID(
                                imname, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &(aoloopcontrol_var.aoconfID_meas_act));
            ...
            if (AOconf[loop].AOcompute.GPU0 == 0) // no GPU -> run in CPU
                {
                    ...
                    if (AOconf[loop].aorun.CMMODE == 0) // goes explicitly through modes, slower but required for
            ...
            else // run in GPU if possible
                {
                    ...
                    if (AOconf[loop].aorun.CMMODE == 0) // goes explicitly through modes, slower but required for
                        ...
                        {
#ifdef _PRINT_TEST
                            printf("[%s] [%d] - CM mult: GPU=1, CMMODE=0 - using matrix %s "
                                   "   GPU alpha beta = %f %f\n",
                                   __FILE__,
                                   __LINE__,
                                   data.image[aoloopcontrol_var.aoconfID_contrM].md[0].name,
                                    ...
                                    if (initWFSref_GPU[aoloopcontrol_var.PIXSTREAM_SLICE] ==
                                        0) // initialize WFS reference
                                        {
                                            ...
                                            memcpy(data.image[aoloopcontrol_var.aoconfID_contrM].array.F,
                                                   ...
                                                   sizeof(float) * AOconf[loop].WFSim.sizeWFS);
                                            COREMOD_MEMORY_image_set_sempost_byID(aoloopcontrol_var.aoconfID_contrM,
                                                                                  -1);
                                            data.image[aoloopcontrol_var.aoconfID_contrM].md[0].cnt0++;
                                         ...
                                         wfselem_active++)
                                        {
                                            data.image[aoconfID_imWFS2_active[aoloopcontrol_var.PIXSTREAM_SLICE]]
                                                ...
                                                data.image[aoloopcontrol_var.aoconfID_contrM]
                                                    .array.F[aoloopcontrol_var
                                                                 .WFS_active_map[aoloopcontrol_var.PIXSTREAM_SLICE *
                                                                                     AOconf[loop].WFSim.sizeWFS +
                                                                                 wfselem_active]];
                                        }
