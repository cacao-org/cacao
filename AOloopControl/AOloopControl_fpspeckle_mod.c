...
errno_t AOloopControl_OptimizePSF_LO(__attribute__((unused)) const char *psfstream_name, const char *IDmodes_name,
                                     const char *dmstream_name, __attribute__((unused)) long delayframe,
                                     __attribute__((unused)) long NBframes)
{
    ...
    for (uint32_t mode = 0; mode < NBmodes; mode++)
    {
        for (x = -ampl; x < 1.01 * ampl; x += ampl)
        {
            // apply DM pattern
            for (uint64_t ii = 0; ii < dmxsize * dmysize; ii++)
                data.image[IDdm].array.F[ii] = data.image[IDdmbest].array.F[ii] +
                                               ampl * data.image[IDmodes].array.F[dmxsize * dmysize * mode + ii];
                                   ...
                                   long NBprobes)
{
    ...
    for (k = 0; k < NBprobes; k++)
    {
        for (act = 0; act < dmsize; act++)
            data.image[IDdmC].array.F[k * dmsize + act] =
                coeffA[k] * data.image[IDprobeA].array.F[act] + coeffB[k] * data.image[IDprobeB].array.F[act];
    ...
    t = time(NULL);
    uttime = gmtime(&t);
    if (sprintf(flogname, "logfpwfs_%04d-%02d-%02d_%02d:%02d:%02d.txt", 1900 + uttime->tm_year, 1 + uttime->tm_mon,
                uttime->tm_mday, uttime->tm_hour, uttime->tm_min, uttime->tm_sec) < 1)
        PRINT_ERROR("sprintf wrote <1 char");
    ...
    while (loopOK == 1)
    {
        ...
        if (sprintf(timestr, "%02d %02d %02d.%09ld", uttime->tm_hour, uttime->tm_min, uttime->tm_sec,
                    thetime->tv_nsec) < 1)
            PRINT_ERROR("sprintf wrote <1 char");
