...
errno_t AOloopControl_loadconfigure(long loop, int mode, int level)
{
    ...
    AOconf[loop].AOcompute.GPUall =
        AOloopControl_readParam_int("GPUall", 0,
                                    fplog); // Skip CPU image scaling and go straight to GPUs ?
    AOconf[loop].aorun.DMprimaryWriteON = AOloopControl_readParam_int("DMprimaryWriteON", 0,
                                                                      fplog); // Direct DM write ?
    AOconf[loop].aorun.DMfilteredWriteON = AOloopControl_readParam_int("DMfilteredWriteON", 0,
    ...
    aoloopcontrol_var.aoconfID_imWFS0 = AOloopControl_IOtools_2Dloadcreate_shmim(name, " ", AOconf[loop].WFSim.sizexWFS,
                                                                                 AOconf[loop].WFSim.sizeyWFS, 0.0);
    ...
    fprintf(fplog, "Connecting to %s [%s]...", name, " ");
    aoloopcontrol_var.aoconfID_imWFS1 = AOloopControl_IOtools_2Dloadcreate_shmim(name, " ", AOconf[loop].WFSim.sizexWFS,
                                                                                 AOconf[loop].WFSim.sizeyWFS, 0.0);
    ...
    fprintf(fplog, "Connecting to %s [%s]...", name, " ");
    aoloopcontrol_var.aoconfID_imWFS2 = AOloopControl_IOtools_2Dloadcreate_shmim(name, " ", AOconf[loop].WFSim.sizexWFS,
                                                                                 AOconf[loop].WFSim.sizeyWFS, 0.0);
    ...
    if (level >= 10) // Load DM modes (will exit if not successful)
    {
        ...
        if (aoloopcontrol_var.aoconfID_DMmodes == -1) // If not, check file
        {
            ...
            if (vOK == 0) // create shared memory
            {
                ...
                fflush(stdout);
                create_image_ID(AOconf[loop].AOpmodecoeffs.DMmodesname, 3, sizearray, _DATATYPE_FLOAT, 1, 0, 0,
                                &(aoloopcontrol_var.aoconfID_DMmodes));
            }
            ...
            case _DATATYPE_FLOAT:
                memcpy(data.image[aoloopcontrol_var.aoconfID_DMmodes].array.F, data.image[ID1tmp].array.F,
        ...
        fprintf(fplog, "stream %s loaded as ID = %ld, size %u %u %u\n", AOconf[loop].AOpmodecoeffs.DMmodesname,
                aoloopcontrol_var.aoconfID_DMmodes, AOconf[loop].DMctrl.sizexDM, AOconf[loop].DMctrl.sizeyDM,
                AOconf[loop].AOpmodecoeffs.NBDMmodes);
    }
    ...
    if (level >= 10)
    {
        ...
        aoconfID_respM = AOloopControl_IOtools_3Dloadcreate_shmim(
            AOconf[loop].aorun.respMname, fname, AOconf[loop].WFSim.sizexWFS, AOconf[loop].WFSim.sizeyWFS,
            AOconf[loop].AOpmodecoeffs.NBDMmodes, 0.0);
        ...
        aoloopcontrol_var.aoconfID_contrM = AOloopControl_IOtools_3Dloadcreate_shmim(
            AOconf[loop].aorun.contrMname, fname, AOconf[loop].WFSim.sizexWFS, AOconf[loop].WFSim.sizeyWFS,
            AOconf[loop].AOpmodecoeffs.NBDMmodes, 0.0);
        ...
        aoloopcontrol_var.aoconfID_gainb =
            AOloopControl_IOtools_2Dloadcreate_shmim(name, fname, AOconf[loop].AOpmodecoeffs.DMmodesNBblock, 1, 0.0);
        ...
        aoloopcontrol_var.aoconfID_multfb =
            AOloopControl_IOtools_2Dloadcreate_shmim(name, fname, AOconf[loop].AOpmodecoeffs.DMmodesNBblock, 1, 0.0);
        ...
        aoloopcontrol_var.aoconfID_limitb =
            AOloopControl_IOtools_2Dloadcreate_shmim(name, fname, AOconf[loop].AOpmodecoeffs.DMmodesNBblock, 1, 0.0);
        ...
        for (kk = 0; kk < AOconf[loop].AOpmodecoeffs.DMmodesNBblock; kk++)
        {
            ...
            fflush(stdout);
            if ((ID = AOloopControl_IOtools_3Dloadcreate_shmim(name, fname, AOconf[loop].DMctrl.sizexDM,
                                                               AOconf[loop].DMctrl.sizeyDM, 0, 0.0)) != -1)
            {
            ...
            printf("====== LOADING %s to %s\n", fname, name);
            fflush(stdout);
            AOloopControl_IOtools_3Dloadcreate_shmim(name, fname, AOconf[loop].WFSim.sizexWFS,
            ...
            ID = AOloopControl_IOtools_3Dloadcreate_shmim(
            ...
            printf("====== LOADING %s to %s  size %u %u\n", fname, name, AOconf[loop].WFSim.activeWFScnt,
                   AOconf[loop].DMctrl.activeDMcnt);
            ID = AOloopControl_IOtools_2Dloadcreate_shmim(name, fname, AOconf[loop].WFSim.activeWFScnt,
                                                          AOconf[loop].DMctrl.activeDMcnt, 0.0);

            if (kk == 0)
            {
                for (unsigned long ii = 0; ii < AOconf[loop].WFSim.activeWFScnt * AOconf[loop].DMctrl.activeDMcnt; ii++)
                {
