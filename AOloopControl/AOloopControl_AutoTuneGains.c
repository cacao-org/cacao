...
errno_t AOloopControl_AutoTuneGains(long loop, const char *IDout_name, float GainCoeff, long NBsamples)
{
    ...
    create_image_ID(imname, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDmodeWFSnoise);
    ...
    for (;;)
    {
        ...
        for (m = 0; m < NBmodes; m++)
        {
            ...
            modegain[m] = AOconf[loop].aorun.gain * data.image[aoloopcontrol_var.aoconfID_gainb].array.F[block] *
                          data.image[aoloopcontrol_var.aoconfID_DMmode_GAIN].array.F[m];
            modemult[m] = AOconf[loop].aorun.mult * data.image[aoloopcontrol_var.aoconfID_multfb].array.F[block] *
                          data.image[aoloopcontrol_var.aoconfID_MULTF_modes].array.F[m];
            NOISEfactor[m] =
                1.0 + modemult[m] * modemult[m] * modegain[m] * modegain[m] / (1.0 - modemult[m] * modemult[m]);
        }
        ...
        while (cnt < AOconf[loop].AOAutoTune.AUTOTUNEGAINS_NBsamples)
        {
            ...
            if (TESTMODE == 1)
            {
                fprintf(fptest, "%5lld %+12.10f %+12.10f %+12.10f %+12.10f %+12.10f\n", cnt,
                        ...
                        data.image[IDmodeval_dm].array.F[TEST_m], data.image[IDmodeval_dm_now].array.F[TEST_m],
        ...
        for (m = 0; m < NBmodes; m++)
        {
            ...
            data.image[IDout].array.F[m] =
                (1.0 - GainCoeff1) * data.image[IDout].array.F[m] + GainCoeff1 * (gainFactor * gainval_array[kkmin]);
        }
        ...
        for (m = 0; m < NBmodes; m++)
        {
            fprintf(fp, "%5ld   %+12.10f %12.10f %12.10f %12.10f %12.10f   %6.4f  %16.14f %16.14f  %6.2f\n", m,
                    (float)ave0[m], (float)sig0[m], stdev[m], sqrt(array_asq[m]), sqrt(array_sig[m]),
                    data.image[IDout].array.F[m], array_sig1[m], array_sig4[m], NOISEfactor[m]);
        }
        fclose(fp);

        printf("[%8ld]  %8ld   %8.6f -> %8.6f\n", iter, AOconf[loop].AOAutoTune.AUTOTUNEGAINS_NBsamples,
