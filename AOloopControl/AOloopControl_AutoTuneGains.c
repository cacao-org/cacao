...
)
{
    ...
    gain0 = 1.0 / (AOconf[loop].AOtiminginfo.loopfrequ *
                   AOconf[loop].AOAutoTune.AUTOTUNEGAINS_evolTimescale);
    ...
    if(sprintf(imname, "aol%ld_modeval_dm_now",
    ...
    if(sprintf(imname, "aol%ld_modeval_dm_now_filt",
    ...
    for(;;)
    {
        ...
        for(m = 0; m < NBmodes; m++)
        {
            ...
            NOISEfactor[m] = 1.0 + modemult[m] * modemult[m] * modegain[m] * modegain[m] /
        ...
        // prepare gain array
        latency = AOconf[loop].AOtiminginfo.hardwlatency_frame +
        ...
        while(cnt < AOconf[loop].AOAutoTune.AUTOTUNEGAINS_NBsamples)
        {
            ...
            for(m = 0; m < NBmodes; m++)
            {
                ...
                if(cnt > cntstart)
                {
                    ...
                    sig0[m] += data.image[IDmodevalOL].array.F[m] *
                               data.image[IDmodevalOL].array.F[m];
                    sig1[m] += diff1 * diff1;
                    sig2[m] += diff2 * diff2;
                    sig3[m] += diff3 * diff3;
                    sig4[m] += diff4 * diff4;
            ...
            if(TESTMODE == 1)
            {
                fprintf(fptest, "%5lld %+12.10f %+12.10f %+12.10f %+12.10f %+12.10f\n", cnt,
                        data.image[IDmodeval].array.F[TEST_m], data.image[IDmodevalOL].array.F[TEST_m],
                        data.image[IDmodeval_dm].array.F[TEST_m],
                        data.image[IDmodeval_dm_now].array.F[TEST_m],
                        data.image[IDmodeval_dm_now_filt].array.F[TEST_m]);
            }
        ...
        for(m = 0; m < NBmodes; m++)
        {
            ...
            {
                errarray[kk] = array_asq[m] * gainval1_array[kk] + array_sig[m] *
                               gainval2_array[kk];
            }
            ...
            data.image[IDout].array.F[m] = (1.0 - GainCoeff1) * data.image[IDout].array.F[m]
                                           +  GainCoeff1 * (gainFactor * gainval_array[kkmin]);
        ...
        {
            fprintf(fp,
                    "%5ld   %+12.10f %12.10f %12.10f %12.10f %12.10f   %6.4f  %16.14f %16.14f  %6.2f\n",
                    m, (float) ave0[m], (float) sig0[m], stdev[m], sqrt(array_asq[m]),
                    sqrt(array_sig[m]), data.image[IDout].array.F[m], array_sig1[m], array_sig4[m],
                    NOISEfactor[m]);
        }
        fclose(fp);

        printf("[%8ld]  %8ld   %8.6f -> %8.6f\n", iter,
               AOconf[loop].AOAutoTune.AUTOTUNEGAINS_NBsamples,
               AOconf[loop].AOAutoTune.AUTOTUNEGAINS_updateGainCoeff, GainCoeff1);
