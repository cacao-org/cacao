                                        ...
                                        for (jjout = 0; jjout < size_y; jjout++)
                                            {
                                                ...
                                                data.image[IDout].array.UI16[jjout * size_x + iiout] =
                                                    data.image[IDin]
                                                        .array.UI16[jjin * data.image[IDin].md[0].size[0] + iiin];
                                            }
                                    if (IDmask != -1)
                                                ...
                                                for (jjout = 0; jjout < size_y; jjout++)
                                                    {
                                                        ...
                                                        data.image[IDout].array.F[jjout * size_x + iiout] =
                                                            data.image[IDin]
                                                                .array
                                                                .UI16[jjin * data.image[IDin].md[0].size[0] + iiin];
                                                    }
                                        }
                                    else
                                        {
                                                ...
                                                for (jjout = 0; jjout < size_y; jjout++)
                                                    {
                                                        ...
                                                        data.image[IDout].array.F[jjout * size_x + iiout] =
                                                            1.0 *
                                                                data.image[IDin]
                                                                    .array.UI16[jjin * data.image[IDin].md[0].size[0] +
                                                                                iiin] -
                                                            data.image[IDdark]
                                                                .array
                                                                .F[jjin * data.image[IDdark].md[0].size[0] + iiin];
                                                    }
                                        }
                                        ...
                                        for (jjout = 0; jjout < size_y; jjout++)
                                            {
                                                ...
                                                data.image[IDout].array.F[jjout * size_x + iiout] =
                                                    data.image[IDin]
                                                        .array.F[jjin * data.image[IDin].md[0].size[0] + iiin];
                                            }
                                }
                            else
                                {
                                        ...
                                        for (jjout = 0; jjout < size_y; jjout++)
                                            {
                                                ...
                                                data.image[IDout].array.F[jjout * size_x + iiout] =
                                                    data.image[IDin]
                                                        .array.F[jjin * data.image[IDin].md[0].size[0] + iiin] -
                                                    data.image[IDdark]
                                                        .array.F[jjin * data.image[IDdark].md[0].size[0] + iiin];
                                            }
                                }
...
errno_t AOcontrolLoop_IOtools_acquireWFSloop_FPCONF()
{
    ...
    function_parameter_add_entry(
        &fps, ".semindex", "input semaphore index", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &semindex_default, NULL);

    // long fpi_out_fluxtotal =
    function_parameter_add_entry(
        &fps, ".out.fluxtotal", "total flux in WFS image", FPTYPE_FLOAT64, FPFLAG_DEFAULT_OUTPUT, NULL, NULL);

    // long fpi_out_GPUalpha =
    function_parameter_add_entry(
        &fps, ".out.GPUalpha", "GPU alpha coefficient", FPTYPE_FLOAT64, FPFLAG_DEFAULT_OUTPUT, NULL, NULL);

    // long fpi_out_GPUbeta =
    function_parameter_add_entry(
        &fps, ".out.GPUbeta", "GPU beta coefficient", FPTYPE_FLOAT64, FPFLAG_DEFAULT_OUTPUT, NULL, NULL);
  ...
  */
    ...
    switch (WFSatype)
        ...
        case _DATATYPE_UINT16:
            ...
            memcpy(arrayutmp, ptrv,
                   sizeof(unsigned short) * sizeWFS); // AOconf[loop].WFSim.sizeWFS);
            break;

        case _DATATYPE_INT16:
            ptrv = (char *)data.image[ID_wfsim].array.SI16;
            ptrv += sizeof(signed short) * slice * sizeWFS; // AOconf[loop].WFSim.sizeWFS;
            memcpy(arraystmp, ptrv,
            ...
            if (AOLCOMPUTE_DARK_SUBTRACT_THREADinit == 0)
                {
                    ...
                    while (ti < COMPUTE_DARK_SUBTRACT_NBTHREADS)
                        {
                            pthread_create(
                                &thread_dark_subtract[ti], NULL, compute_function_dark_subtract, (void *)&ti);
