...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Acquire spectral data from a wavefront sensor for chromatic "
                        "characterization and dispersed fringe tracking."
};
...
#define FPS_PARAMS(X)                                                                            \
    X(".wfsin", input_shm_name, FPTYPE_STREAMNAME, 1, (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT), \
      "Wavefront sensor input")                                                                  \
    X(".wfsmask", specmask_shm_name, FPTYPE_STREAMNAME, 1,                                       \
      (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT), "wfs spectral extraction mask")                 \
    X(".binning", &binning, FPTYPE_UINT32, 1, (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT),         \
      "spectral trace binning")                                                                  \
    X(".AOloopindex", &AOloopindex, FPTYPE_UINT32, 1, (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT), \
      "loop index")                                                                              \
    X(".semindex", &semindex, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT, "input semaphore index")   \
    X(".comp.darksub", &compWFSsubdark, FPTYPE_ONOFF, 1, FPFLAG_DEFAULT_INPUT,                   \
      "sub aolX_wfsdark -> imWFS0")                                                              \
    X(".comp.WFSnormalize", &compWFSnormalize, FPTYPE_ONOFF, 1, FPFLAG_DEFAULT_INPUT,            \
      "normalize WFS frames -> imWFS1")                                                          \
    X(".comp.WFSrefsub", &compWFSrefsub, FPTYPE_ONOFF, 1, FPFLAG_DEFAULT_INPUT,                  \
...
static errno_t customCONFcheck()
{
    ...
    {
        long fpi_inputshmname = functionparameter_GetParamIndex(milk_data.fpsptr, ".wfsin");
        long fpi_compWFSsubdark =
            functionparameter_GetParamIndex(milk_data.fpsptr, ".comp.darksub");
        long fpi_compWFSnormalize =
            functionparameter_GetParamIndex(milk_data.fpsptr, ".comp.WFSnormalize");
        long fpi_compWFSrefsub =
            functionparameter_GetParamIndex(milk_data.fpsptr, ".comp.WFSrefsub");

        if (fpi_inputshmname > -1)
        {
            milk_data.fpsptr->parray[fpi_inputshmname].fpflag |=
                FPFLAG_STREAM_RUN_REQUIRED | FPFLAG_CHECKSTREAM;
        }

        if (fpi_compWFSsubdark > -1)
        {
            milk_data.fpsptr->parray[fpi_compWFSsubdark].fpflag |= FPFLAG_WRITERUN;
        }
        if (fpi_compWFSnormalize > -1)
        {
            milk_data.fpsptr->parray[fpi_compWFSnormalize].fpflag |= FPFLAG_WRITERUN;
        }
        if (fpi_compWFSrefsub > -1)
        {
            milk_data.fpsptr->parray[fpi_compWFSrefsub].fpflag |= FPFLAG_WRITERUN;
        }
    }
 ...
 */
static errno_t extract_traces(IMGID wfsin, IMGID specmask, IMGID wfsout, uint32_t binning)
{
    DEBUG_TRACE_FSTART();
 ...
 */
static errno_t dark_sub(IMGID wfsin, IMGID wfsdark, IMGID wfsout)
{
 ...
 */
static errno_t spec_norm(IMGID wfsin, IMGID wfsout)
{
    uint32_t j;
    uint32_t sizeWFSx  = wfsin.md->size[0];
...
static errno_t compute_function()
{
    ...
    resolveIMGID(&wfsin, ERRMODE_WARN, dcimg, dcnimg);
    if (wfsin.ID == -1)
    {
    ...
    resolveIMGID(&specmask, ERRMODE_WARN, dcimg, dcnimg);
    if (specmask.ID == -1)
    {
        return RETURN_FAILURE;
    }
    ...
    int wfsim_semwaitindex = ImageStreamIO_getsemwaitindex(wfsin.im, semindex);
    ...
    INSERT_STD_PROCINFO_COMPUTEFUNC_LOOPSTART
    {
        ...
        long fpi_compWFSsubdark =
            functionparameter_GetParamIndex(milk_data.fpsptr, ".comp.darksub");
        if (fpi_compWFSsubdark > -1 &&
            (milk_data.fpsptr->parray[fpi_compWFSsubdark].fpflag & FPFLAG_ONOFF))
        {
            if (imgWFSdark.ID != -1)
            {
                status_darksub = 1;
        ...
        long fpi_compWFSnormalize =
            functionparameter_GetParamIndex(milk_data.fpsptr, ".comp.WFSnormalize");
        if (fpi_compWFSnormalize > -1 &&
            (milk_data.fpsptr->parray[fpi_compWFSnormalize].fpflag & FPFLAG_ONOFF))
        {
            status_normalize = 1;
        ...
        long fpi_compWFSrefsub =
            functionparameter_GetParamIndex(milk_data.fpsptr, ".comp.WFSrefsub");
        if (fpi_compWFSrefsub > -1 &&
            (milk_data.fpsptr->parray[fpi_compWFSrefsub].fpflag & FPFLAG_ONOFF))
        {
            // subtract reference
            status_refsub = 1;
