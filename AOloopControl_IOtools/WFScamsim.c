...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Simulate a wavefront sensor camera by applying photon noise, readout "
                        "noise, and detector response to an ideal WFS image."
};
...
#define FPS_PARAMS(X)                                                                              \
    X(".wfssignal", wfssignal_in, FPTYPE_STREAMNAME, 1, (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT), \
      "Wavefront sensor input signal")                                                             \
    X(".wfscamim", wfsim_out, FPTYPE_STREAMNAME, 1, (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT),     \
      "Wavefront sensor ouput image")                                                              \
    X(".compdarkadd", &compdarkadd, FPTYPE_ONOFF, 1, FPFLAG_DEFAULT_INPUT, "subtract dark")        \
    X(".camdark", wfsdark, FPTYPE_STREAMNAME, 1, (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT),        \
      "camera dark frame")                                                                         \
    X(".fluxtotal", &fluxtotal, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_OUTPUT,                          \
    ...
    X(".camRON", &camRON, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_OUTPUT,                                \
...
static errno_t customCONFsetup()
{
    ...
    {
        long fpi_compdarkadd = functionparameter_GetParamIndex(milk_data.fpsptr, ".compdarkadd");
        ...
        long fpi_compphnoise = functionparameter_GetParamIndex(milk_data.fpsptr, ".compphnoise");
        long fpi_camRON      = functionparameter_GetParamIndex(milk_data.fpsptr, ".camRON");

        if (fpi_compdarkadd > -1)
        {
            milk_data.fpsptr->parray[fpi_compdarkadd].fpflag |= FPFLAG_WRITERUN;
        }
        if (fpi_fluxtotal > -1)
        {
            milk_data.fpsptr->parray[fpi_fluxtotal].fpflag |= FPFLAG_WRITERUN;
        }
        if (fpi_camgain > -1)
        {
            milk_data.fpsptr->parray[fpi_camgain].fpflag |= FPFLAG_WRITERUN;
        }
        if (fpi_compphnoise > -1)
        {
            milk_data.fpsptr->parray[fpi_compphnoise].fpflag |= FPFLAG_WRITERUN;
        }
        if (fpi_camRON > -1)
        {
            milk_data.fpsptr->parray[fpi_camRON].fpflag |= FPFLAG_WRITERUN;
        }
    }
...
static errno_t compute_function()
{
    ...
    resolveIMGID(&wfssignalimg, ERRMODE_WARN, dcimg, dcnimg);
    if (wfssignalimg.ID == -1)
    {
    ...
    resolveIMGID(&wfsdarkimg, ERRMODE_WARN, dcimg, dcnimg);
    ...
    IMGID wfsoutimg;
    {
        wfsoutimg = stream_connect_create_2D(wfsim_out, sizexWFS, sizeyWFS, _DATATYPE_UINT16);
    }
    ...
    INSERT_STD_PROCINFO_COMPUTEFUNC_START
    {
        ...
        if (fluxtotal >= 0.0)
        {
            long fpi_compphnoise =
                functionparameter_GetParamIndex(milk_data.fpsptr, ".compphnoise");
            if (fpi_compphnoise > -1 &&
                (milk_data.fpsptr->parray[fpi_compphnoise].fpflag & FPFLAG_ONOFF))
            {
                for (uint64_t ii = 0; ii < sizeWFS; ii++)
                {
