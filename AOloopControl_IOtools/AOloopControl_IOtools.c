...
errno_t AOloopControl_IOtools_AveStream_cli()
{
    ...
    {
        AOloopControl_IOtools_AveStream(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numf,
...
errno_t AOloopControl_IOtools_imAlignStream_cli()
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 4) + CLI_checkarg(5, 3) +
        ...
        0)
    {
        AOloopControl_IOtools_imAlignStream(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numl,
...
errno_t AOloopControl_IOtools_frameDelay_cli()
{
    ...
    {
        AOloopControl_IOtools_frameDelay(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
errno_t AOloopControl_IOtools_stream3Dto2D_cli()
{
    ...
    {
        AOloopControl_IOtools_stream3Dto2D(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
errno_t AOloopControl_IOtools_RTLOGsave_cli()
{
    ...
    {
        AOloopControl_IOtools_RTLOGsave(data.cmdargtoken[1].val.numl, data.cmdargtoken[2].val.string,
...
static errno_t init_module_CLI()
{
    ...
    RegisterCLIcommand("aolacquireWFSloop", __FILE__, AOloopControl_IOtools_acquireWFSloop_cli, "acquire WFS loop",
                       "<loopindex>", "aolacquireWFSloop 2", "int AOloopControl_IOtools_acquireWFSloop(long loop)");
    ...
    RegisterCLIcommand("cropshim", __FILE__, AOloopControl_IOtools_camimage_extract2D_sharedmem_loop_cli,
                       ...
                       "int AOloopControl_IOtools_camimage_extract2D_sharedmem_loop(char *in_name, const char "
                       "*dark_name, char *out_name, long size_x, long size_y, long xstart, long ystart)");
    ...
    RegisterCLIcommand("aveACshmim", __FILE__, AOloopControl_IOtools_AveStream_cli, "average and AC shared mem image",
                       "<input image> <coeff> <output image ave> <output AC> <output RMS>",
                       "aveACshmim imin 0.01 outave outAC outRMS",
                       "int AOloopControl_IOtools_AveStream(char *IDname, double alpha, char *IDname_out_ave, char "
                       "*IDname_out_AC, char *IDname_out_RMS)");

    RegisterCLIcommand("alignshmim", __FILE__, AOloopControl_IOtools_imAlignStream_cli,
                       ...
                       "errno_t AOloopControl_IOtools_imAlignStream(const char *IDname, int xbox0, int ybox0, const "
                       "char *IDref_name, const char *IDout_name, int insem)");

    RegisterCLIcommand("aolframedelay", __FILE__, AOloopControl_IOtools_frameDelay_cli, "introduce temporal delay",
                       "<in> <temporal kernel> <out> <sem index>", "aolframedelay in kern out 0",
                       "long AOloopControl_IOtools_frameDelay(const char *IDin_name, const char *IDkern_name, const "
                       "char *IDout_name, int insem)");

    RegisterCLIcommand(
        "aolstream3Dto2D", __FILE__, AOloopControl_IOtools_stream3Dto2D_cli, "remaps 3D cube into 2D image",
        "<input 3D stream> <output 2D stream> <# cols> <sem trigger>", "aolstream3Dto2D in3dim out2dim 4 1",
    ...
    RegisterCLIcommand(
        "aolrtlogbuffsave", __FILE__, AOloopControl_IOtools_RTLOGsave_cli, "log realtime buffer stream",
