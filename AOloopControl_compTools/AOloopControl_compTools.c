...
errno_t AOloopControl_compTools_CrossProduct_cli()
{
    ...
    {
        AOloopControl_compTools_CrossProduct(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
errno_t AOloopControl_compTools_mkSimpleZpokeM_cli()
{
    ...
    {
        AOloopControl_compTools_mkSimpleZpokeM(data.cmdargtoken[1].val.numl, data.cmdargtoken[2].val.numl,
                                               data.cmdargtoken[3].val.string);
...
static errno_t init_module_CLI()
{
    ...
    RegisterCLIcommand("aolcrossp", __FILE__, AOloopControl_compTools_CrossProduct_cli,
                       "compute cross product between two cubes. Apply mask if image xpmask exists",
                       "<cube1> <cube2> <output image>", "aolcrossp imc0 imc1 crosspout",
                       "AOloopControl_compTools_CrossProduct(char *ID1_name, char *ID1_name, char *IDout_name)");

    RegisterCLIcommand("aolmksimplezpM", __FILE__, AOloopControl_compTools_mkSimpleZpokeM_cli,
                       "make simple poke sequence", "<dmsizex> <dmsizey> <output image>", "aolmksimplezpM 50 50 pokeM",
            ...
            for (uint32_t z2 = 0; z2 < zsize2; z2++)
            {
                for (uint64_t ii = 0; ii < xysize1; ii++)
                {
                    data.image[IDout].array.F[z2 * zsize1 + z1] +=
                        data.image[ID1].array.F[z1 * xysize1 + ii] * data.image[ID2].array.F[z2 * xysize2 + ii];
                }
            }
    }
    else
