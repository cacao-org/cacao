                                               ...
                                               int         MaskMode)
{
    ...
    if ((msizex != dcimg[IDmask].md[0].size[0]) || (msizey != dcimg[IDmask].md[0].size[1]))
    {
        printf("ERROR: file dmmask size (%u %u) does not match expected size "
               "(%u %u)\n",
               dcimg[IDmask].md[0].size[0], dcimg[IDmask].md[0].size[1], msizex, msizey);
        exit(0);
    }
...

    {
        ...
        resolveIMGID(&imgCPAmask, ERRMODE_WARN, dcimg, dcnimg);
    ...
    create_3Dimage_ID(ID_name, msizex, msizey, dcimg[ID0].md[0].size[2] - 1 + NBZ, &ID);
    ...
    for (uint32_t k = 0; k < (uint32_t) (dcimg[ID0].md[0].size[2] - 1 + NBZ); k++)
    {
        ...
        if (IDeModes != -1)
        {
            ...
            for (uint64_t ii = 0; ii < msizex * msizey; ii++)
            {
                dcimg[IDtm].array.F[ii] = dcimg[ID].array.F[k * msizex * msizey + ii];
            }
            ...
            for (uint64_t ii = 0; ii < msizex * msizey; ii++)
            {
                dcimg[ID].array.F[k * msizex * msizey + ii] =
                    dcimg[IDtm].array.F[ii] - coeff * dcimg[IDem].array.F[ii];
            }
        ...
        for (uint64_t ii = 0; ii < msizex * msizey; ii++)
        {
            ...
            totvm += dcimg[ID].array.F[k * msizex * msizey + ii] * dcimg[IDmask].array.F[ii];
        }
        offset = totvm / totm;
        ...
        for (uint64_t ii = 0; ii < msizex * msizey; ii++)
        {
            ...
            dcimg[ID].array.F[k * msizex * msizey + ii] *= dcimg[IDmask].array.F[ii];
        }
        ...
        for (uint64_t ii = 0; ii < msizex * msizey; ii++)
        {
            dcimg[ID].array.F[k * msizex * msizey + ii] -= offset / msizex / msizey;
    ...
    if (MaskMode == 1)
    {
        ...
        for (citer = 0; citer < NBciter; citer++)
        {
            ...
            gauss_filter(ID_name, "modeg", 4.0 * pow(1.0 * (NBciter - citer) / NBciter, 0.5),
                         kernsize);
    ...
    if ((IDslaved != -1) && (IDmask != -1))
    {
        ...
        for (m = 0; m < dcimg[ID].md[0].size[2]; m++)
        {
            ...
            for (uint64_t ii = 0; ii < msizex * msizey; ii++)
            {
                dcimg[IDtmp].array.F[ii] = dcimg[ID].array.F[m * msizex * msizey + ii];
                dcimg[IDtmp1].array.F[ii] =
                    dcimg[IDmask].array.F[ii] * (1.0 - dcimg[IDslaved].array.F[ii]);
            ...
            while (pixcnt > 0)
            {
                ...
                for (uint32_t ii = 1; ii < (uint32_t) (msizex - 1); ii++)
                {
                    ...
                    {
                        if ((dcimg[IDtmp1].array.F[jj * msizex + ii] < 0.5) &&
                            (dcimg[IDslaved].array.F[jj * msizex + ii] > 0.5))
                        {
                            pixcnt++;
                            vxp = dcimg[IDtmp].array.F[jj * msizex + (ii + 1)];
                            cxp = dcimg[IDtmp1].array.F[jj * msizex + (ii + 1)];

                            vxm = dcimg[IDtmp].array.F[jj * msizex + (ii - 1)];
                            cxm = dcimg[IDtmp1].array.F[jj * msizex + (ii - 1)];

                            vyp = dcimg[IDtmp].array.F[(jj + 1) * msizex + ii];
                            cyp = dcimg[IDtmp1].array.F[(jj + 1) * msizex + ii];

                            vym = dcimg[IDtmp].array.F[(jj - 1) * msizex + ii];
                            cym = dcimg[IDtmp1].array.F[(jj - 1) * msizex + ii];
                            ...
                            if (ctot > 0.5)
                            {
                                dcimg[IDtmp].array.F[jj * msizex + ii] =
                                    (vxp * cxp + vxm * cxm + vyp * cyp + vym * cym) / ctot;
                                dcimg[IDtmp2].array.F[jj * msizex + ii] = 1.0;
                            }
                        }
                    }
                }
                for (uint64_t ii = 0; ii < msizex * msizey; ii++)
                {
                    dcimg[IDtmp1].array.F[ii] = dcimg[IDtmp2].array.F[ii];
                }
            }
            for (uint64_t ii = 0; ii < msizex * msizey; ii++)
            {
                dcimg[ID].array.F[m * msizex * msizey + ii] = dcimg[IDtmp].array.F[ii];
            }
