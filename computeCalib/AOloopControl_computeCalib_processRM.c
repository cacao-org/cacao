 ...
 */
errno_t AOloopControl_computeCalib_Process_zrespM(const char *IDzrespm0_name,
                                                  ...
                                                  const char                         *DMmap_name)
{
        ...
        (image_ID("pixindexim", dcimg, dcnimg) != -1)) // start decoding
    {
        ...
        AOloopControl_computeCalib_Hadamard_decodeRM(IDzrespm0_name, "RMmat", "pixindexim",
                                                     IDzrespm_name);
        IDzrm = image_ID(IDzrespm_name, dcimg, dcnimg);
    ...
    IDdm             = read_sharedmem_image(name, dcimg, dcnimg);
...
// TEST: if "RMpokeC" exists, decode it as well
//
errno_t AOloopControl_computeCalib_ProcessZrespM_medianfilt(const char *zrespm_name,
                                                            ...
                                                            int         normalize)
{
    ...
    {
        if (snprintf(fname, sizeof(fname), "./zresptmp/%s_pos_%03ld.fits", zrespm_name, kmat) < 1)
        {
            PRINT_ERROR("snprintf wrote <1 char");
        }
        ...
        if (snprintf(fname, sizeof(fname), "./zresptmp/%s_neg_%03ld.fits", zrespm_name, kmat) < 1)
        {
            PRINT_ERROR("snprintf wrote <1 char");
        }
...
#    pragma omp parallel for private(fluxpos, fluxneg, ii)
        ...
        for (poke = 0; poke < NBpoke; poke++)
        {
            ...
            for (ii = 0; ii < sizeWFS; ii++)
            {
                if (isnan(dcimg[IDzrespfp].array.F[poke * sizeWFS + ii]) != 0)
                {
            ...
            for (ii = 0; ii < sizeWFS; ii++)
            {
                if (isnan(dcimg[IDzrespfm].array.F[poke * sizeWFS + ii]) != 0)
                {
                    printf("%ld element %ld is NAN -> replacing by 0\n", IDzrespfm,
                           poke * sizeWFS + ii);
            ...
            for (ii = 0; ii < sizeWFS; ii++)
            {
                if (normalize == 1)
                {
                    dcimg[IDzrespfp].array.F[poke * sizeWFS + ii] /= fluxpos;
                    dcimg[IDzrespfm].array.F[poke * sizeWFS + ii] /= fluxneg;
                }
                ...
                if (isnan(dcimg[IDzresp_array[kmat]].array.F[poke * sizeWFS + ii]) != 0)
                {
                    printf("%ld element %ld is NAN -> replacing by 0\n", IDzresp_array[kmat],
                           poke * sizeWFS + ii);
                    dcimg[IDzresp_array[kmat]].array.F[poke * sizeWFS + ii] = 0.0;
                }
                if (isnan(dcimg[IDWFSrefc_array[kmat]].array.F[poke * sizeWFS + ii]) != 0)
                {
                    printf("%ld element %ld is NAN -> replacing by 0\n", IDWFSrefc_array[kmat],
                           poke * sizeWFS + ii);
                    dcimg[IDWFSrefc_array[kmat]].array.F[poke * sizeWFS + ii] = 0.0;
                }
            }
        }
...
#    pragma omp parallel for private(ii, kmat, ave, k, pixvalarray)
    ...
    for (poke = 0; poke < NBpoke; poke++)
    {
        ...
        for (ii = 0; ii < sizeWFS; ii++)
        {
            for (kmat = 0; kmat < NBmat; kmat++)
            {
                pixvalarray[kmat] = dcimg[IDzresp_array[kmat]].array.F[poke * sizeWFS + ii];
            }
...
#    pragma omp parallel for private(poke, kmat, pixvalarray, ave, k)
    ...
    AOloopControl_computeCalib_mkCalib_map_mask(zrespm_name, WFSmap_name, DMmap_name, 0.2, 1.0, 0.7,
                                                0.3, 0.05, 1.0, 0.65, 0.3);
    ...
    for (poke = 0; poke < NBpoke; poke++)
    {
        ...
        for (ii = 0; ii < sizeWFS; ii++)
        {
            tot += dcimg[IDzrm].array.F[poke * sizeWFS + ii] * dcimg[IDWFSmask].array.F[ii];
        }

        for (ii = 0; ii < sizeWFS; ii++)
        {
            dcimg[IDzrm].array.F[poke * sizeWFS + ii] -= tot * dcimg[IDWFSmask].array.F[ii] / totm;
        }

        double tot1 = 0.0;
        for (ii = 0; ii < sizeWFS; ii++)
        {
            tot1 += dcimg[IDzrm].array.F[poke * sizeWFS + ii] * dcimg[IDWFSmask].array.F[ii];
        }
        fprintf(fp, "%6ld %06ld %20f %20f\n", poke, NBpoke, tot, tot1);
    }
    fclose(fp);
...
static FPS_APP_INFO FPS_app_info = {
    .fps_name         = "mkCMsvd",
    .cmdkey           = "mkCMsvd",
    .description      = "compute SVD control matrix from RM",
    .description_long = "Process a response matrix via SVD to compute the control matrix. Applies "
                        "singular value filtering and modal truncation."
};

// Section 2: Local variables for FPS parameters
static int64_t mkCM_AOloopindex;
static double  mkCM_SVDlim;
static char    mkCM_fname_respM[FUNCTION_PARAMETER_STRMAXLEN];
static int64_t mkCM_GPUmode;
static char    mkCM_out_label[FUNCTION_PARAMETER_STRMAXLEN];

// Section 3: FPS_PARAMS X-macro
#define FPS_PARAMS(X)                                                                         \
    X(".AOloopindex", &mkCM_AOloopindex, FPTYPE_INT64, 1, FPFLAG_DEFAULT_INPUT, "loop index") \
    X(".SVDlim", &mkCM_SVDlim, FPTYPE_FLOAT64, 1,                                             \
      (FPFLAG_DEFAULT_INPUT | FPFLAG_MINLIMIT | FPFLAG_MAXLIMIT), "SVD limit value")          \
    X(".fname_respM", mkCM_fname_respM, FPTYPE_FILENAME, 1,                                   \
      (FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED), "response matrix")                   \
    X(".GPUmode", &mkCM_GPUmode, FPTYPE_INT64, 1, FPFLAG_DEFAULT_INPUT, "Using GPU ?")        \
    X(".out.label", mkCM_out_label, FPTYPE_STRING, 1, FPFLAG_DEFAULT_INPUT, "output label")
...
static errno_t compute_function_mkCM()
{
    ...
    strncpy(outdirname, functionparameter_GetParamPtr_STRING(milk_data.fpsptr, ".conf.datadir"),
            FUNCTION_PARAMETER_STRMAXLEN);
    EXECUTE_SYSTEM_COMMAND("mkdir -p %s", outdirname);

    load_fits(mkCM_fname_respM, "respM", 1, NULL);
    ...
    if (mkCM_GPUmode)
    {
        LINALGEBRA_magma_compute_SVDpseudoInverse("respM", cm_name, mkCM_SVDlim, 100000, "VTmat", 0,
                                                  1, 64, 0, NULL);
    }
    else
    {
#endif
        linopt_compute_SVDpseudoInverse("respM", cm_name, mkCM_SVDlim, 10000, "VTmat", NULL);
#ifdef HAVE_MAGMA
    }
#endif

    {
        ...
        imageID  ID_VTmat = image_ID("VTmat", dcimg, dcnimg);
        imageID  ID_DMmodes;
        ...
        for (uint32_t kk = 0; kk < DMxysize; kk++)
        {
            ...
            {
                dcimg[ID_DMmodes].array.F[kk * DMxysize + ii] =
                    dcimg[ID_VTmat].array.F[ii * DMxysize + kk];
            }
        }
        ...
        imageID IDrespM = image_ID("respM", dcimg, dcnimg);
        WFSxsize        = dcimg[IDrespM].md->size[0];
        WFSysize        = dcimg[IDrespM].md->size[1];
        ...
        for (int mi = 0; mi < mimax; mi++)
        {
            ...
            {
                dcimg[ID_WFSmodes].array.F[mi * WFSxysize + ii] = 0.0;
                for (uint32_t jj = 0; jj < DMxysize; jj++)
                {
                    dcimg[ID_WFSmodes].array.F[mi * WFSxysize + ii] +=
                        dcimg[ID_DMmodes].array.F[mi * DMxysize + jj] *
                        dcimg[IDrespM].array.F[jj * WFSxysize + ii];
                }
            }
        }
        printf(" DONE\n");
...
errno_t CLIADDCMD_AOloopControl_computeCalib__mkCMsvd()
{
                                            ...
                                            const char *IDout_name)
{
        ...
        for (jj = 0; jj < ysize; jj++)
        {
            if (dcimg[IDmaskRM].array.F[jj * xsize + ii] < 0.5)
            {
                ...
                for (ii1 = ii1min; ii1 < ii1max + 1; ii1++)
                {
                    ...
                    {
                        if (dcimg[IDmaskRM].array.F[jj1 * xsize + ii1] > 0.5)
                        {
                            dx = 1.0 * (ii - ii1);
                            dy = 1.0 * (jj - jj1);
                            r  = sqrt(dx * dx + dy * dy);
                            if (r < pixrad)
                            {
                                if (r < dcimg[IDout].array.F[jj * xsize + ii])
                                {
                                    dcimg[IDout].array.F[jj * xsize + ii] = r;
                                }
                            }
                        }
                    }
    ...
    for (ii = 0; ii < xsize; ii++)
    {
        for (jj = 0; jj < ysize; jj++)
        {
            if (dcimg[IDout].array.F[jj * xsize + ii] > (xsize + ysize) / 2)
            {
