                ...
                if(ii1 == jj1)
                {
                    ...
                    else
                    {
                        gsl_matrix_set(
                            ...
                            ii1,
                            jj1,
                            1.0 / gsl_vector_get(matrix_DtraD_eval, ii1));
                    }
                }
                else
                {
    ...
    const char *IDcmatc_active_name)
{
    ...
    memcpy(matrix_Mc,
           data.image[IDcmatc].array.F,
           sizeof(float) * sizeWFS * sizeDM);
    ...
    //    }
    //# endif
    memcpy(data.image[IDcmatc].array.F,
           matrix_Mc,
           sizeof(float) * sizeWFS * sizeDM);
    free(matrix_cmp);
    ...
    */
            ...
            slice++)
    {
        ...
        printf(
            "PIXEL SLICE %d     Keeping only active pixels / actuators : "
            "%ld x %ld   ->   %ld x %ld\n",
            slice,
            sizeWFS,
            sizeDM,
