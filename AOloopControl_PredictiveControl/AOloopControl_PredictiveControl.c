...
errno_t AOloopControl_PredictiveControl_mapPredictiveFilter_cli()
{
    ...
    {
        AOloopControl_PredictiveControl_mapPredictiveFilter(data.cmdargtoken[1].val.string,
...
errno_t AOloopControl_PredictiveControl_testPredictiveFilter_cli()
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 2) + CLI_checkarg(3, 1) + CLI_checkarg(4, 2) + CLI_checkarg(5, 3) == 0)
    {
        AOloopControl_PredictiveControl_testPredictiveFilter(
            ...
            data.cmdargtoken[4].val.numl, data.cmdargtoken[5].val.string, 1e-10);
...
static errno_t init_module_CLI()
{
    ...
    RegisterCLIcommand("aolPFwatchin", __FILE__, AOloopControl_PredictiveControl_builPFloop_WatchInput_cli,
                       "watch telemetry for predictive filter input", "<loop #> <PFblock #> <start> <end>",
                       "aolPFwatchin 0 2",
                       "long AOloopControl_builPFloop_WatchInput(long loop, long PFblock, long PFblockStart, long "
                       "PFblockEnd, long NBbuff)");

    RegisterCLIcommand("aolmappfilt", __FILE__, AOloopControl_PredictiveControl_mapPredictiveFilter_cli,
                       "map/search predictive filter", "<input coeffs> <mode number> <delay [frames]>",
                       "aolmkapfilt coeffim 23 2.4",
                       "long AOloopControl_mapPredictiveFilter(char *IDmodecoeff_name, long modeout, double delayfr)");

    RegisterCLIcommand("aolmkpfilt", __FILE__, AOloopControl_PredictiveControl_testPredictiveFilter_cli,
                       "test predictive filter",
                       "<trace im> <mode number> <delay [frames]> <filter size> <out filter name>",
                       "aolmkpfilt traceim 23 2.4 20 filt23",
                       "long AOloopControl_testPredictiveFilter(char *IDtrace_name, long mode, double delayfr, long "
                       "filtsize, char *IDfilt_name, double SVDeps)");

    RegisterCLIcommand("aolpfsetave", __FILE__, AOloopControl_PredictiveControl_setPFsimpleAve_cli,
                       "set predictive filter to integrator", "<PredFilter> <DecayCoeff>", "aolpfsetave outPFb0 0.5",
