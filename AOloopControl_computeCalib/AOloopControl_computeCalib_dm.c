...
)
{
    ...
    for(kk = 0; kk < zsize; kk++)
    {
        for(ii = 0; ii < xsize; ii++)
            ...
            {
                index = jj * xsize + ii;
                coeff =  data.image[IDpixdist].array.F[index] / ((1.0 * xsize /
                         (data.image[IDcpa].array.F[kk] + 0.1)) * 0.8);
                ...
                data.image[IDout].array.F[kk * xysize + index] = coeff *
    ...
    for(ii = 0; ii < xsize; ii++)
        for(jj = 0; jj < ysize; jj++)
        {
            ...
            if(data.image[IDmask].array.F[index] > 0.5)
            {
                for(kk = 0; kk < zsize; kk++)
                {
                    data.image[IDout].array.F[kk * xysize + index] = data.image[IDin].array.F[kk *
                            xysize + index];
                }
            }
            else if(data.image[IDsl].array.F[index] > 0.5)
            {
                for(kk = 0; kk < zsize; kk++)
                {
                    ...
                    if(val1cnt > 0.0001)
                    {
                        data.image[IDout].array.F[kk * xysize + index] = (val1 / val1cnt) * exp(-
                                (valr / r0) * (valr / r0));
                    }
                }
            }
