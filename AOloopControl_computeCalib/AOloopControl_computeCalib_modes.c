                                           ...
                                           char *outdir)
{
    ...
    // imageID  IDz;
    long zindex[10];
    double zcpa[10]; /// CPA for each Zernike (somewhat arbitrary... used to sort
                     /// modes in CPA)
    ...
    if (MODAL == 0)
        {
            ...
            if ((msizex != data.image[IDmaskRM].md[0].size[0]) || (msizey != data.image[IDmaskRM].md[0].size[1]))
                {
                    printf("ERROR: file dmmaskRM size (%u %u) does not match expected "
                           "size (%u %u)\n",
    ...
    if (COMPUTE_DM_MODES == 1) // DM modes fmodes2b
        {
            ...
            if (MODAL == 0)
              ...
              DELETE_IMAGE_ERRMODE_WARNING);
                            ...
                            for (citer = 0; citer < NBciter; citer++)
                                {
                                    ...
                                    gauss_filter(
                                        ID_name, "modeg", 4.0 * pow(1.0 * (NBciter - citer) / NBciter, 0.5), kernsize);
                    ...
                    fflush(stdout);
                    IDmaskRMedge =
                        AOloopControl_computeCalib_DMedgeDetect(data.image[IDmaskRM].md[0].name, "dmmaskRMedge");
                    ...
                    fflush(stdout);
                    IDtmp = AOloopControl_computeCalib_DMextrapolateModes(
                        ID_name, "dmmaskRMin", "modesfreqcpa", "fmodes0test");
...
#pragma omp parallel for private(m, m1, act, act1, act2, wfselem)
                    ...
                    for (uint32_t m = 0; m < data.image[IDmodes0all].md[0].size[2]; m++)
                        {
                            ...
                            linopt_imtools_image_fitModes(
                                "imfitim", "RMMmodes", "dmmaskRM", 1.0e-2, "linfitcoeff", linfitreuse, NULL);
                            linfitreuse = 1;
            ...
            printf("[%5d] STEP 3: REMOVE NULL SPACE WITHIN EACH BLOCK - USE "
            ...
            for (uint32_t mblock = 0; mblock < NBmblock; mblock++)
                {
                    ...
                    printf("[%5d] STEP3  -  BLOCK %u/%u: keeping %u / %u modes  ( %f "
                           "%f ) [%u  %u %u]\n",
                           __LINE__,
                           mblock,
                           NBmblock,
            ...
            printf("[%5d] STEP 4: REMOVE MODES THAT ARE CONTAINED IN PREVIOUS "
                            ...
                            for (uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                                {
                                    ...
                                    linopt_imtools_image_fitModes(
                                        "SVDmodein", imname, "SVDmask", 1.0e-2, "modecoeff", reuse, NULL);
            ...
            for (uint32_t mblock = 0; mblock < NBmblock; mblock++)
                {
                    ...
                    for (uint32_t m = 0; m < cnt; m++)
                        {
                            ...
                            for (uint64_t ii = 0; ii < msizexy; ii++)
                                {
                                    value1 +=
                                        data.image[IDm].array.F[m * msizexy + ii] * data.image[IDmaskRM].array.F[ii];
                    ...
                    IDtmp = AOloopControl_computeCalib_DMslaveExt(
                        data.image[IDm].md[0].name, data.image[IDmaskRM].md[0].name, "dmslaved", "fmodesext", 100.0);
            ...
            if (BlockNB < 0)
                {
                    ...
                    if (data.image[IDzrespM].md[0].size[2] != msizexy)
                        {
                            printf("[%d] ERROR: zrespM has wrong z size : %ld, should be "
                                   "%ld\n",
                                   __LINE__,
                                    ...
                                    if ((IDRMMmodes != -1) && (IDRMMresp != -1))
                                        {
                                            ...
                                            WRITE_FULLFILENAME(
                                                fnameLOcoeff, "./%s/mkmodestmp/LOcoeff_%02u.txt", outdir, mblock);
                                            ...
                                            create_2Dimage_ID(
                                                "imfitmat", linfitsize, data.image[ID].md[0].size[2], &IDcoeffmat);
                            ...
                            for (uint32_t mblock0 = 0; mblock0 < mblock; mblock0++)
                                {
                                    ...
                                    for (uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                                        {
                                            int slen =
                                                snprintf(imname, STRINGMAXLEN_IMGNAME, "fmodesWFS0_%02u", mblock);
                                            if (slen < 1)
                                                {
                                                    PRINT_ERROR("snprintf wrote <1 char");
                                                    abort(); // can't handle this error any other way
                                            ...
                                            if (mok[m] == 1)
                                                {
                                                    ...
                                                    printf("[%5d] max index IDmwfs1 %u  = %ld / %ld    "
                                                           "[ %ld %ld %ld ]\n",
                                                           __LINE__,
                                                           m1,
                                                    ...
                                                    printf("[%5d] max index IDmwfs  %u  = %ld / %ld    "
                                                           "[ %ld %ld %ld ]\n",
                                                           __LINE__,
                                                           m,
                                                    ...
                                                    printf("[%5d] max index IDmdm1  %u  = %ld / %ld    "
                                                           "[ %ld %ld %ld ]\n",
                                                           __LINE__,
                                                           m1,
                                                    ...
                                                    printf("[%5d] max index IDmdm   %u  = %ld / %ld    "
                                                           "[ %ld %ld %ld ]\n",
                                                           __LINE__,
                                                           m,
                    ...
                    if (BlockNB > -1) // LOAD & VERIFY SIZE
                        {
                            ...
                            if ((data.image[ID].md[0].size[0] != msizex) && (msizey != data.image[ID].md[0].size[0]))
                                {
                                    printf("ERROR: image size (%u %u) does not match expected "
                                           "size (%u %u)\n",
                            ...
                            for (uint32_t kk = 0; kk < data.image[IDSVDcoeff].md[0].size[0]; kk++)
                                {
                                    fprintf(fpcoeff,
                                            ...
                                            data.image[IDSVDcoeff].array.F[0],
                                            cnt,
                                            data.image[IDSVDcoeff].array.F[kk] / data.image[IDSVDcoeff].array.F[0],
                                            SVDlim);
                                    printf("==== %u %12g %12g  %3u\n",
                                           kk,
                                           data.image[IDSVDcoeff].array.F[kk],
                                            ...
                                            for (uint64_t ii = 0; ii < msizexy; ii++)
                                                {
                                                    data.image[IDmdm1].array.F[kk * msizexy + ii] +=
                                                        data.image[ID_VTmatrix]
                                                            .array.F[kk1 * data.image[IDSVDcoeff].md[0].size[0] + kk] *
            ...
            for (uint32_t mblock = 0; mblock < NBmblock; mblock++)
                {
                    ...
                    if ((BlockNB < 0) || (BlockNB == (int)mblock))
                            ...
                            if (MBLOCK_NBmode[mblock] > 0)
                                {
                                    ...
                                    CUDACOMP_magma_compute_SVDpseudoInverse(
                                        imname, imnameCM, SVDlim1, 10000, "VTmat", 0, 0, 1.e-4, 1.e-7, 0, 64, NULL);
#else
                                    ...
                                    printf("[%5d] -- Block %u/%u COMPUTE ZONAL CONTROL MATRIX "
                                    ...
                                    AOloopControl_computeCalib_compute_CombinedControlMatrix(
                                        imnameCM, imname, "wfsmask", "dmmask", imnameCMc, imnameCMcact);
                                    printf("[%5d] Block %u/%u : "
                                           "AOloopControl_computeCalib_compute_"
                        ...
                        {
                            printf("[%5d] LOADING WFS MODES, MODAL CONTROL MATRICES: block "
                                   "%u/%u\n",
                                   __LINE__,
                                   mblock,
                                   NBmblock);
            ...
            for (uint32_t mblock = 0; mblock < NBmblock; mblock++)
                {
                    EXECUTE_SYSTEM_COMMAND(
                        "echo \"%u\" > ./%s/block%02u_NBmodes.txt", MBLOCK_NBmode[mblock], outdir, mblock);
                    WRITE_IMAGENAME(imname, "fmodesWFS_%02u", mblock);
                    imageID IDmwfs = image_ID(imname);
...
imageID AOloopControl_computeCalib_mkModes_Simple(const char *IDin_name, long NBmblock, long Cmblock, float SVDlim)
{
    ...
    if (NBmblock == 1)
    ...
    else
        {
            for (mblock = 0; mblock < NBmblock; mblock++)
                {
                    int slen =
                        snprintf(fname, STRINGMAXLEN_FULLFILENAME, "./conf_staged/param_block%02ldend.txt", mblock);
                    if (slen < 1)
                        {
                            PRINT_ERROR("snprintf wrote <1 char");
                            abort(); // can't handle this error any other way
...
#ifdef HAVE_MAGMA
                    CUDACOMP_magma_compute_SVDpseudoInverse(
                        imname, imnameCM, SVDlim, 10000, "VTmat", 0, 0, 1.e-4, 1.e-7, 0, 64, NULL);
#else
    ...
    for (mblock = 0; mblock < NBmblock; mblock++)
        {
            if (sprintf(command, "echo \"%ld\" > ./conf_staged/block%02ld_NBmodes.txt", MBLOCK_NBmode[mblock], mblock) <
                1)
                {
                    PRINT_ERROR("sprintf wrote <1 char");
                }
