...
imageID AOloopControl_computeCalib_mkModes(const char *ID_name, uint32_t msizex, uint32_t msizey, float CPAmax,
                                           float deltaCPA, double xc, double yc, double r0, double r1, int MaskMode,
                                           int BlockNB, float SVDlim, char *outdir)
{
    DEBUG_TRACE_FSTART();
    ...
    if (COMPUTE_DM_MODES == 1) // DM modes fmodes2b
    {
        ...
        if (MODAL == 0)
        {
            ...
            for (uint64_t ii = 0; ii < msizex * msizey; ii++)
            {
                data.image[IDmask].array.F[ii] =
                    1.0 - (1.0 - data.image[IDmaskRM].array.F[ii]) * (1.0 - data.image[IDslaved].array.F[ii]);
            ...
            for (uint64_t ii = 0; ii < msizex * msizey; ii++)
            {
                data.image[IDmaskRMin].array.F[ii] =
                    data.image[IDmaskRM].array.F[ii] * (1.0 - data.image[IDmaskRMedge].array.F[ii]);
            }
            ...
            for (uint32_t m = 0; m < data.image[ID].md[0].size[2]; m++)
            {
                for (uint64_t ii = 0; ii < msizex * msizey; ii++)
                {
                    data.image[ID].array.F[m * msizex * msizey + ii] =
                        data.image[IDtmp].array.F[m * msizex * msizey + ii] * data.image[IDmask].array.F[ii];
                }
            }
        }
        else
        ...
        for (uint32_t k = 0; k < (uint32_t)(data.image[ID0].md[0].size[2] - 1 + NBZ); k++)
        {
            ...
            if (IDeModes != -1)
            {
                ...
                linopt_imtools_image_fitModes("tmpmode", "emodes", "dmmask", 1.0e-3, "lcoeff", 0, NULL);
        ...
        create_3Dimage_ID("fmodesWFS00all", wfsxsize, wfsysize, data.image[ID].md[0].size[2], &IDm);
...
#pragma omp parallel for private(m, m1, act, act1, act2, wfselem)
                ...
                for (act = 0; act < msizexy; act++)
                {
                    ...
                    for (wfselem = 0; wfselem < wfssize; wfselem++)
                    {
                        data.image[IDm].array.F[m1 + wfselem] +=
                            data.image[ID].array.F[act1] * data.image[IDzrespM].array.F[act2 + wfselem];
                    }
                }
            }
        }
        ...
        if ((IDRMMmodes != -1) && (IDRMMresp != -1))
        {
            ...
            create_2Dimage_ID("imfitmat", linfitsize, data.image[ID].md[0].size[2], &IDcoeffmat);
                ...
                if (LOcoeff > 0.01)
                {
                    ...
                    for (uint64_t wfselem = 0; wfselem < wfssize; wfselem++)
                    {
                        data.image[IDm].array.F[m * wfssize + wfselem] =
                            ...
                            (1.0 - LOcoeff) * data.image[IDm].array.F[m * wfssize + wfselem];
                    }
                }
            }
        ...
        for (uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            ...
            create_3Dimage_ID(imname, msizex, msizey, MBLOCK_NBmode[mblock], &(MBLOCK_ID[mblock1]));
        ...
        if (IDextrablock != -1)
        {
            ...
            create_3Dimage_ID(imname, msizex, msizey, MBLOCK_NBmode[mblock], &(MBLOCK_ID[mblock]));
        ...
        printf("[%5d] STEP 3: REMOVE NULL SPACE WITHIN EACH BLOCK - USE SVDlim00 FOR CUTOFF -> fmodes1all.fits  (DM "
               "space)\n",
               __LINE__);
        fflush(stdout);
        ...
        for (uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            ...
            printf("[%5d] STEP3  -  BLOCK %u/%u: keeping %u / %u modes  ( %f %f ) [%u  %u %u]\n", __LINE__, mblock,
                   NBmblock, cnt, data.image[IDSVDcoeff].md[0].size[0], SVDlim00, svdcoeff0,
                   data.image[IDSVDcoeff].md[0].size[0], msizex, msizey);
            fflush(stdout);
        ...
        printf("[%5d] STEP 4: REMOVE MODES THAT ARE CONTAINED IN PREVIOUS BLOCKS, AND ENFORCE DM-SPACE ORTHOGONALITY "
        ...
        for (uint32_t mblock = 0; mblock < NBmblock; mblock++) // outer block loop
        {
            ...
            for (uint32_t mblock0 = 0; mblock0 < mblock; mblock0++) // inner block loop
            {
                ...
                for (uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                {
                    ...
                    linopt_imtools_image_fitModes("SVDmodein", imname, "SVDmask", 1.0e-2, "modecoeff", reuse, NULL);
        ...
        for (uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            ...
            for (uint32_t m = 0; m < data.image[IDSVDcoeff].md[0].size[0]; m++)
            {
                fprintf(fpcoeff, "%5u   %12g   %12g  %5u     %10.8f  %10.8f\n", m, data.image[IDSVDcoeff].array.F[m],
                        ...
                        data.image[IDSVDcoeff].array.F[m] / data.image[IDSVDcoeff].array.F[0], SVDlim01);
            ...
            IDtmp = AOloopControl_computeCalib_DMslaveExt(data.image[IDm].md[0].name, data.image[IDmaskRM].md[0].name,
                                                          "dmslaved", "fmodesext", 100.0);
    ...
    if (IDzrespM != -1) // compute WFS response to DM modes
    {
        ...
        if (BlockNB < 0)
        {
            ...
            for (uint32_t mblock = 0; mblock < NBmblock; mblock++)
            {
                ...
                {
                    imageID IDwfsMresp;
                    create_3Dimage_ID(imname, wfsxsize, wfsysize, MBLOCK_NBmode[mblock], &IDwfsMresp);
...
#pragma omp parallel for private(m, act, wfselem)
                    ...
                    if ((IDRMMmodes != -1) && (IDRMMresp != -1))
                    {
                        ...
                        for (uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                        {
                            ...
                            linopt_imtools_image_fitModes("imfitim", "RMMmodes", "dmmaskRM", 1.0e-2, "linfitcoeff",
                                                          linfitreuse, NULL);
                            linfitreuse = 1;
                            ...
                            if (LOcoeff > 0.01)
                            {
                                ...
                                for (wfselem = 0; wfselem < wfssize; wfselem++)
                                {
                                    data.image[IDwfsMresp].array.F[m * wfssize + wfselem] =
                                        LOcoeff * data.image[IDwfstmp].array.F[wfselem] +
                                        (1.0 - LOcoeff) * data.image[IDwfsMresp].array.F[m * wfssize + wfselem];
                                }
                            }
                        }
            ...
            for (uint32_t mblock = 0; mblock < NBmblock; mblock++)
            {
                ...
                for (uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                {
                    ...
                    {
                        value1 +=
                            data.image[IDmwfs].array.F[m * wfssize + ii] * data.image[IDmwfs].array.F[m * wfssize + ii];
                    }
                    ...
                    for (uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                    {
                        if (mok[m] == 1)
                        {
                            ...
                            printf("[%5d] max index IDmwfs1 %u  = %ld / %ld    [ %ld %ld %ld ]\n", __LINE__, m1,
                                   ...
                                   (long)(data.image[IDmwfs1].md[0].size[0] * data.image[IDmwfs1].md[0].size[1] *
                                          data.image[IDmwfs1].md[0].size[2]),
                                   (long)data.image[IDmwfs1].md[0].size[0], (long)data.image[IDmwfs1].md[0].size[1],
                                   (long)data.image[IDmwfs1].md[0].size[2]);
                            printf("[%5d] max index IDmwfs  %u  = %ld / %ld    [ %ld %ld %ld ]\n", __LINE__, m,
                                   (long)(m * wfssize + wfssize - 1),
                                   (long)(data.image[IDmwfs].md[0].size[0] * data.image[IDmwfs].md[0].size[1] *
                                          data.image[IDmwfs].md[0].size[2]),
                                   (long)data.image[IDmwfs].md[0].size[0], (long)data.image[IDmwfs].md[0].size[1],
                                   (long)data.image[IDmwfs].md[0].size[2]);

                            printf("[%5d] max index IDmdm1  %u  = %ld / %ld    [ %ld %ld %ld ]\n", __LINE__, m1,
                                   (long)(m1 * msizexy + msizexy - 1),
                                   (long)(data.image[IDmdm1].md[0].size[0] * data.image[IDmdm1].md[0].size[1] *
                                          data.image[IDmdm1].md[0].size[2]),
                                   (long)data.image[IDmdm1].md[0].size[0], (long)data.image[IDmdm1].md[0].size[1],
                                   (long)data.image[IDmdm1].md[0].size[2]);
                            printf("[%5d] max index IDmdm   %u  = %ld / %ld    [ %ld %ld %ld ]\n", __LINE__, m,
                                   (long)(m * msizexy + msizexy - 1),
                                   (long)(data.image[IDmdm].md[0].size[0] * data.image[IDmdm].md[0].size[1] *
                ...
                for (uint32_t kk = 0; kk < cnt; kk++) /// eigen mode index
                {
                    ...
                    for (uint64_t ii = 0; ii < msizexy; ii++)
                    {
                        value1 += data.image[IDmdm1].array.F[kk * msizexy + ii] *
                                  data.image[IDmdm1].array.F[kk * msizexy + ii] * data.image[IDmaskRM].array.F[ii];
                    }
                    rms = sqrt(value1 / value1cnt);
            ...
            if ((BlockNB < 0) || (BlockNB == (int)mblock))
            {
                if (MBLOCK_NBmode[mblock] > 0)
                {
...
#ifdef HAVE_MAGMA
                    CUDACOMP_magma_compute_SVDpseudoInverse(imname, imnameCM, SVDlim1, 10000, "VTmat", 0, 0, 1.e-4,
                                                            1.e-7, 0, 64, NULL);
#else
                    ...
                    printf("[%5d] Block %u/%u : run AOloopControl_computeCalib_compute_CombinedControlMatrix\n",
                           __LINE__, mblock, NBmblock);
                    fflush(stdout);
                    AOloopControl_computeCalib_compute_CombinedControlMatrix(imnameCM, imname, "wfsmask", "dmmask",
                                                                             imnameCMc, imnameCMcact);
...
imageID AOloopControl_computeCalib_mkModes_Simple(const char *IDin_name, long NBmblock, long Cmblock, float SVDlim)
{
    ...
    for (mblock = 0; mblock < NBmblock; mblock++)
    {
        ...
        if ((Cmblock == mblock) || (Cmblock == -1))
        {
            ...
            int slen = snprintf(command, STRINGMAXLEN_COMMAND, "echo \"%f\" > ./conf_staged/block%02ld_SVDlim.txt",
                                SVDlim, mblock);
            if (slen < 1)
            {
                PRINT_ERROR("snprintf wrote <1 char");
            ...
            for (kk = 0; kk < MBLOCK_NBmode[mblock]; kk++)
            {
                for (ii = 0; ii < wfssize; ii++)
                {
                    data.image[ID].array.F[kk * wfssize + ii] =
                        data.image[IDin].array.F[(kk + MBLOCK_blockstart[mblock]) * wfssize + ii];
                }
            }
...
#ifdef HAVE_MAGMA
            CUDACOMP_magma_compute_SVDpseudoInverse(imname, imnameCM, SVDlim, 10000, "VTmat", 0, 0, 1.e-4, 1.e-7, 0, 64,
                                                    NULL);
#else
            ...
            AOloopControl_computeCalib_compute_CombinedControlMatrix(imnameCM, imname, "wfsmask", "dmmask", imnameCMc,
                                                                     imnameCMcact);
...
errno_t AOloopControl_computeCalib_mkCalib_map_mask(long loop, const char *zrespm_name, const char *WFSmap_name,
                                                    const char *DMmap_name, float dmmask_perclow, float dmmask_coefflow,
                                                    float dmmask_perchigh, float dmmask_coeffhigh,
                                                    float wfsmask_perclow, float wfsmask_coefflow,
                                                    float wfsmask_perchigh, float wfsmask_coeffhigh)
{
