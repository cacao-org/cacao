 ...
 */
errno_t AOloopControl_computeCalib_Process_zrespM(long loop, const char *IDzrespm0_name,
...
#pragma omp parallel for private(poke, kmat, pixvalarray, ave, k)
    ...
    AOloopControl_computeCalib_mkCalib_map_mask(loop, zrespm_name, WFSmap_name, DMmap_name, 0.2, 1.0, 0.7, 0.3, 0.05,
                                                1.0, 0.65, 0.3);
...
errno_t AOloopControl_computeCalib_mkCM_FPCONF()
{
    ...
    function_parameter_add_entry(&fps, ".SVDlim", "SVD limit value", FPTYPE_FLOAT64, FPFLAG, &SVDlimdefault, NULL);

    // Input file name
    FPFLAG = FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED;
    // __attribute__((unused)) long fpi_filename_respm        =
    function_parameter_add_entry(&fps, ".fname_respM", "response matrix", FPTYPE_FILENAME, FPFLAG, pNull, NULL);

    long GPUmode_default[4] = {0, 0, 1, 0};
    //  long fpi_GPUmode = 0;
    function_parameter_add_entry(&fps, ".GPUmode", "Using GPU ?", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &GPUmode_default,
                                 NULL);
    //    (void) fpi_GPUmode;
...
errno_t AOloopControl_computeCalib_mkCM_RUN()
{
    ...
    */
    ...
    __attribute__((unused)) long loop = functionparameter_GetParamValue_INT64(&fps, ".loop");
    ...
    strncpy(outdirname, functionparameter_GetParamPtr_STRING(&fps, ".conf.datadir"), FUNCTION_PARAMETER_STRMAXLEN);
    ...
    if (usingGPU == 1)
    {
        CUDACOMP_magma_compute_SVDpseudoInverse("respM", cm_name, SVDlim, 100000, "VTmat", 0, 0, 1.e-4, 1.e-7, 0, 64,
                                                NULL);
    }
    else
    {
