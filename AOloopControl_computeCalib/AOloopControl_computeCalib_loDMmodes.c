...
int clock_gettime(int clk_id, struct mach_timespec *t) {
    ...
    double nseconds = ((double)time *
(double)timebase.numer)/((double)timebase.denom); double seconds = ((double)time
* (double)timebase.numer)/((double)timebase.denom * 1e9); t->tv_sec = seconds;
    t->tv_nsec = nseconds;
    return 0;
}
#else
                    ...
                    for (uint64_t ii = 0; ii < msizex * msizey; ii++)
                        {
                            ...
                            data.image[IDtmp1].array.F[ii] =
                                data.image[IDmask].array.F[ii] * (1.0 - data.image[IDslaved].array.F[ii]);
                    ...
                    while (pixcnt > 0)
                        {
                            ...
                            for (uint32_t ii = 1; ii < (uint32_t)(msizex - 1); ii++)
                                for (uint32_t jj = 1; jj < (uint32_t)(msizey - 1); jj++)
                                    {
                                        if ((data.image[IDtmp1].array.F[jj * msizex + ii] < 0.5) &&
                                            (data.image[IDslaved].array.F[jj * msizex + ii] > 0.5))
                                            {
                                                pixcnt++;
