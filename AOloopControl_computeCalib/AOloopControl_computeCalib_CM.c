...
imageID AOloopControl_computeCalib_compute_ControlMatrix(long loop, __attribute__((unused)) long NB_MODE_REMOVED,
                                                         const char *ID_Rmatrix_name, const char *ID_Cmatrix_name,
                                                         const char *ID_VTmatrix_name, double Beta,
                                                         long NB_MODE_REMOVED_STEP, float eigenvlim)
{
    FILE *fp;
    ...
    uint64_t n = data.image[ID_Rmatrix].md[0].size[0] *
                 data.image[ID_Rmatrix].md[0].size[1]; //AOconf[loop].AOpmodecoeffs.NBDMmodes;
    ...
    create_3Dimage_ID("eigenmodesrespM", data.image[ID_Rmatrix].md[0].size[0], data.image[ID_Rmatrix].md[0].size[1],
                      data.image[ID_Rmatrix].md[0].size[2], &IDeigenmodesResp);
    ...
    if (IDmodes != -1)
    {
        ...
        else
        {
            ...
            create_3Dimage_ID("eigenmodesM", xsize_modes, ysize_modes, m, &IDeigenmodes);
                ...
                for (uint32_t kk1 = 0; kk1 < m; kk1++)
                {
                    for (uint64_t ii = 0; ii < xsize_modes * ysize_modes; ii++)
                        data.image[IDeigenmodes].array.F[kk * xsize_modes * ysize_modes + ii] +=
                            ...
                            data.image[IDmodes].array.F[kk1 * xsize_modes * ysize_modes + ii];
                }
            }
            printf("\n");
        }
    ...
    arraysizetmp[2] = m;
    create_image_ID(ID_Cmatrix_name, 3, arraysizetmp, _DATATYPE_FLOAT, 0, 0, 0, &ID_Cmatrix);
                                                                 ...
                                                                 const char *IDcmatc_active_name)
{
        ...
        for (uint64_t act = 0; act < sizeDM; act++)
        {
            for (uint64_t wfselem = 0; wfselem < sizeWFS; wfselem++)
            {
                matrix_Mc[act * sizeWFS + wfselem] +=
                    matrix_cmp[mode * sizeWFS + wfselem] * matrix_DMmodes[mode * sizeDM + act];
            }
        }
    }
    //# ifdef _OPENMP
    ...
    */
    ...
    for (uint32_t slice = 0; slice < (uint32_t)aoloopcontrol_var.PIXSTREAM_NBSLICES; slice++)
    {
        ...
        create_2Dimage_ID(imname, sizeWFS_active[slice], sizeDM_active, &(IDcmatc_active[slice]));
        ...
        printf("PIXEL SLICE %d     Keeping only active pixels / actuators : %ld x %ld   ->   %ld x %ld\n", slice,
               sizeWFS, sizeDM, sizeWFS_active[slice], sizeDM_active);
    }
...
imageID AOloopControl_computeCalib_loadCM(long loop, const char *CMfname)
{
    ...
    if (ID != -1)
    {
        ...
        if (vOK == 1)
        {
            ...
            for (uint64_t ii = 0;
                 ii < AOconf[loop].WFSim.sizexWFS * AOconf[loop].WFSim.sizeyWFS * AOconf[loop].AOpmodecoeffs.NBDMmodes;
                 ii++)
            {
