                                                         ...
                                                         float eigenvlim)
{
   ...
   */
    ...
    create_3Dimage_ID("eigenmodesrespM",
    ...
    if (IDmodes != -1)
        {
            ...
            if (zsize_modes != m)
                {
                    printf("ERROR: zsize (%ld) of modesM does not match expected size "
                           "(%ld)\n",
                           (long)zsize_modes,
                           (long)m);
                }
    ...
    /// second, build the "inverse" of the diagonal matrix of eigenvalues
    /// (matrix1)
                                                                 ...
                                                                 const char *IDcmatc_active_name)
{
    ...
    //# ifdef _OPENMP
    //   #pragma omp parallel shared(matrix_Mc, matrix_cmp, matrix_DMmodes ,chunk)
    //   private( mode, act, wfselem)
    //  {
                    ...
                    for (uint64_t wfselem_active = 0; wfselem_active < sizeWFS_active[slice]; wfselem_active++)
                        {
                            ...
                            data.image[IDcmatc_active[slice]]
                                .array.F[act_active * sizeWFS_active[slice] + wfselem_active] =
                                matrix_Mc[act * sizeWFS + wfselem];
                        }
                }
            printf("PIXEL SLICE %d     Keeping only active pixels / actuators : "
                   "%ld x %ld   ->   %ld x %ld\n",
                   slice,
                   sizeWFS,
                   sizeDM,
            ...
            if (vOK == 1)
                {
                    ...
                    for (uint64_t ii = 0; ii < AOconf[loop].WFSim.sizexWFS * AOconf[loop].WFSim.sizeyWFS *
