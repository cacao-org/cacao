...
errno_t AOloopControl_computeCalib_mkModes_Simple_cli()
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 1) == 0)
        {
            AOloopControl_computeCalib_mkModes_Simple(data.cmdargtoken[1].val.string,
                                                      data.cmdargtoken[2].val.numl,
                                                      data.cmdargtoken[3].val.numl,
                                                      data.cmdargtoken[4].val.numf);
...
errno_t AOloopControl_computeCalib_computeCM_cli()
{
    if (CLI_checkarg(1, 2) + CLI_checkarg(2, 4) + CLI_checkarg(3, 3) + CLI_checkarg(4, 1) + CLI_checkarg(5, 2) +
        ...
        0)
        {
            AOloopControl_computeCalib_compute_ControlMatrix(LOOPNUMBER,
...
errno_t AOloopControl_computeCalib_Hadamard_decodeRM_cli()
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 4) + CLI_checkarg(4, 3) == 0)
        {
            AOloopControl_computeCalib_Hadamard_decodeRM(data.cmdargtoken[1].val.string,
                                                         data.cmdargtoken[2].val.string,
                                                         data.cmdargtoken[3].val.string,
...
errno_t AOloopControl_computeCalib_mkCalib_map_mask_cli()
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 3) + CLI_checkarg(3, 3) + CLI_checkarg(4, 1) + CLI_checkarg(5, 1) +
        ...
        0)
        {
            AOloopControl_computeCalib_mkCalib_map_mask(LOOPNUMBER,
...
errno_t AOloopControl_computeCalib_Process_zrespM_cli()
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 3) + CLI_checkarg(4, 3) + CLI_checkarg(5, 3) == 0)
        {
            AOloopControl_computeCalib_Process_zrespM(LOOPNUMBER,
                                                      data.cmdargtoken[1].val.string,
                                                      data.cmdargtoken[2].val.string,
                                                      data.cmdargtoken[3].val.string,
                                                      data.cmdargtoken[4].val.string,
...
errno_t AOloopControl_computeCalib_ProcessZrespM_cli()
{
    if (CLI_checkarg(1, 3) + CLI_checkarg(2, 3) + CLI_checkarg(3, 3) + CLI_checkarg(4, 3) + CLI_checkarg(5, 1) +
        ...
        0)
        {
            AOloopControl_computeCalib_ProcessZrespM_medianfilt(LOOPNUMBER,
                                                                data.cmdargtoken[1].val.string,
                                                                data.cmdargtoken[2].val.string,
                                                                data.cmdargtoken[3].val.string,
                                                                data.cmdargtoken[4].val.string,
...
errno_t AOloopControl_computeCalib_compute_CombinedControlMatrix_cli()
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 4) + CLI_checkarg(4, 4) + CLI_checkarg(5, 3) +
        ...
        0)
        {
            AOloopControl_computeCalib_compute_CombinedControlMatrix(data.cmdargtoken[1].val.string,
                                                                     data.cmdargtoken[2].val.string,
                                                                     data.cmdargtoken[3].val.string,
                                                                     data.cmdargtoken[4].val.string,
                                                                     data.cmdargtoken[5].val.string,
...
static errno_t init_module_CLI()
{
   ...
   */
    ...
    RegisterCLIcommand("aolmkmodesM",
                       __FILE__,
    ...
    RegisterCLIcommand("aolRMmkmasks",
                       __FILE__,
    ...
    RegisterCLIcommand("aolcleanzrm",
                       __FILE__,
    ...
    RegisterCLIcommand("aolcompcmatc",
                       __FILE__,
...
errno_t AOcontrolLoop_computeCalib_ComputeCM_FPCONF()
{
    ...
    function_parameter_add_entry(
        &fps, ".AOloopindex", "loop index", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &loop_default, NULL);

    double SVDlim_default[4] = {0.01, 0.000001, 1.0, 0.01};
    //__attribute__((unused)) long fpi_SVDlim =
    function_parameter_add_entry(
        &fps, ".SVDlim", "RM poke amplitude", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, &SVDlim_default, NULL);

    double CPAmax_default[4] = {10.0, 0.1, 100.0, 10.0};
    //__attribute__((unused)) long fpi_CPAmax =
    function_parameter_add_entry(
        &fps, ".CPAmax", "maximum controlled CPA", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, &CPAmax_default, NULL);

    double deltaCPA_default[4] = {0.8, 0.1, 2.0, 0.8};
    //__attribute__((unused)) long fpi_deltaCPA =
    function_parameter_add_entry(
        &fps, ".deltaCPA", "delta CPA", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, &deltaCPA_default, NULL);
    ...
    function_parameter_add_entry(
        &fps, ".align.ID", "mask I.D. (if no DMmaskRM)", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, &alignID_default, NULL);

    double alignOD_default[4] = {0.0, 0.0, 2000.0, 0.0};
    //__attribute__((unused)) long fpi_alignOD =
    function_parameter_add_entry(
        &fps, ".align.OD", "mask O.D. (if no DMmaskRM)", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, &alignOD_default, NULL);

    long DMxsize_default[4] = {10, 1, 2000, 10};
    //__attribute__((unused)) long fpi_DMxsize =
    function_parameter_add_entry(
        &fps, ".DMxsize", "DM x size", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &DMxsize_default, NULL);

    long DMysize_default[4] = {10, 1, 2000, 10};
    //__attribute__((unused)) long fpi_DMysize =
    function_parameter_add_entry(
        &fps, ".DMysize", "DM y size", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &DMysize_default, NULL);
    ...
    function_parameter_add_entry(
        &fps, ".option.extrablockM", "extra modes block", FPTYPE_FITSFILENAME, FPFLAG_DEFAULT_INPUT, pNull, NULL);

    //__attribute__((unused)) long fpi_fname_exclmodes =
    function_parameter_add_entry(
        &fps, ".option.exclmodes", "excluded modes", FPTYPE_FITSFILENAME, FPFLAG_DEFAULT_INPUT, pNull, NULL);

    // Update Actions
    long fpi_update_RMfiles = 0;
    function_parameter_add_entry(
        &fps, ".upRMfiles", "update RM files", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, pNull, &fpi_update_RMfiles);
    ...
    function_parameter_add_entry(
        &fps, ".out.dirname", "output directory", FPTYPE_DIRNAME, FPFLAG_DEFAULT_INPUT, pNull, NULL);
    ...
    function_parameter_add_entry(
        &fps, ".out.timestring", "output timestring", FPTYPE_STRING, FPFLAG_DEFAULT_INPUT, pNull, NULL);
