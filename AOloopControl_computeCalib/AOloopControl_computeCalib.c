...
errno_t AOloopControl_computeCalib_mkSlavedAct_cli()
{
    ...
    {
        AOloopControl_computeCalib_mkSlavedAct(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numf,
...
{
    if (CLI_checkarg(1, 3) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 1) + CLI_checkarg(5, 1) +
            CLI_checkarg(6, 1) + CLI_checkarg(7, 1) + CLI_checkarg(8, 1) + CLI_checkarg(9, 1) + CLI_checkarg(10, 2) ==
        0)
    {
        AOloopControl_computeCalib_mkloDMmodes(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numl,
                                               ...
                                               data.cmdargtoken[9].val.numf, data.cmdargtoken[10].val.numl);
        return CLICMD_SUCCESS;
    }
    else
    {
...
errno_t AOloopControl_computeCalib_mkModes_cli()
{
    if (CLI_checkarg(1, 3) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 1) + CLI_checkarg(5, 1) +
            CLI_checkarg(6, 1) + CLI_checkarg(7, 1) + CLI_checkarg(8, 1) + CLI_checkarg(9, 1) + CLI_checkarg(10, 2) +
            CLI_checkarg(11, 2) + CLI_checkarg(12, 1) ==
        0)
    {
        AOloopControl_computeCalib_mkModes(
            ...
            data.cmdargtoken[7].val.numf, data.cmdargtoken[8].val.numf, data.cmdargtoken[9].val.numf,
            data.cmdargtoken[10].val.numl, data.cmdargtoken[11].val.numl, data.cmdargtoken[12].val.numf, "NULL");
...
errno_t AOloopControl_computeCalib_mkModes_Simple_cli()
{
    ...
    {
        AOloopControl_computeCalib_mkModes_Simple(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numl,
                                                  data.cmdargtoken[3].val.numl, data.cmdargtoken[4].val.numf);
...
errno_t AOloopControl_computeCalib_computeCM_cli()
{
    if (CLI_checkarg(1, 2) + CLI_checkarg(2, 4) + CLI_checkarg(3, 3) + CLI_checkarg(4, 1) + CLI_checkarg(5, 2) +
            CLI_checkarg(6, 1) ==
        0)
    {
        AOloopControl_computeCalib_compute_ControlMatrix(
            LOOPNUMBER, data.cmdargtoken[1].val.numl, data.cmdargtoken[2].val.string, data.cmdargtoken[3].val.string,
            "evecM", data.cmdargtoken[4].val.numf, data.cmdargtoken[5].val.numl, data.cmdargtoken[6].val.numf);
...
errno_t AOloopControl_computeCalib_Hadamard_decodeRM_cli()
{
    ...
    {
        AOloopControl_computeCalib_Hadamard_decodeRM(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 3) + CLI_checkarg(3, 3) + CLI_checkarg(4, 1) + CLI_checkarg(5, 1) +
            CLI_checkarg(6, 1) + CLI_checkarg(7, 1) + CLI_checkarg(8, 1) + CLI_checkarg(9, 1) + CLI_checkarg(10, 1) +
            CLI_checkarg(11, 1) ==
        0)
    {
        AOloopControl_computeCalib_mkCalib_map_mask(
            LOOPNUMBER, data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string, data.cmdargtoken[3].val.string,
            data.cmdargtoken[4].val.numf, data.cmdargtoken[5].val.numf, data.cmdargtoken[6].val.numf,
            data.cmdargtoken[7].val.numf, data.cmdargtoken[8].val.numf, data.cmdargtoken[9].val.numf,
            data.cmdargtoken[10].val.numf, data.cmdargtoken[11].val.numf);
...
errno_t AOloopControl_computeCalib_Process_zrespM_cli()
{
    ...
    {
        AOloopControl_computeCalib_Process_zrespM(LOOPNUMBER, data.cmdargtoken[1].val.string,
...
{
    if (CLI_checkarg(1, 3) + CLI_checkarg(2, 3) + CLI_checkarg(3, 3) + CLI_checkarg(4, 3) + CLI_checkarg(5, 1) +
            CLI_checkarg(6, 2) ==
        0)
    {
        AOloopControl_computeCalib_ProcessZrespM_medianfilt(
            LOOPNUMBER, data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string, data.cmdargtoken[3].val.string,
            data.cmdargtoken[4].val.string, data.cmdargtoken[5].val.numf, data.cmdargtoken[6].val.numl);
...
{
    if (CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 4) + CLI_checkarg(4, 4) + CLI_checkarg(5, 3) +
            CLI_checkarg(6, 3) ==
        0)
    {
...
static errno_t init_module_CLI()
{
    ...
    RegisterCLIcommand("aolmkH", __FILE__, AOloopControl_computeCalib_mkHadamardModes_cli,
                       "make Hadamard poke sequence", "<DM pixel mask> <output fname [string]>",
                       "aolmkH dm50mask h50pokec",
                       "long AOloopControl_computeCalib_mkHadamardModes(char *DMmask_name, char outname)");

    RegisterCLIcommand("aolHaddec", __FILE__, AOloopControl_computeCalib_Hadamard_decodeRM_cli,
                       "decode Hadamard matrix", "<input RM> <Hadamard matrix> <DMpix index frame> <output RM>",
                       "aolHaddec imRMh Hmat pixiind imRM",
                       "long AOloopControl_computeCalib_Hadamard_decodeRM(char *inname, char *Hmatname, char "
                       "*indexname, char *outname)");

    RegisterCLIcommand(
        "aolmkslact", __FILE__, AOloopControl_computeCalib_mkSlavedAct_cli,
        "create slaved actuators map based on proximity", "<maskRM> <distance> <outslact>",
    ...
    RegisterCLIcommand("aolmklodmmodes", __FILE__, AOloopControl_computeCalib_mkloDMmodes_cli,
                       ...
                       "long AOloopControl_computeCalib_mkloDMmodes(char *ID_name, long msizex, long msizey, float "
                       "CPAmax, float deltaCPA, double xc, double yx, double r0, double r1, int MaskMode)");

    RegisterCLIcommand("aolcomputeCM", __FILE__, AOloopControl_computeCalib_ComputeCM_cli,
                       "compute control matrix, high level function FPS only", "<CODE>", "aolcomputeCM _CONFRUN_",
                       "FPS only");

    RegisterCLIcommand("aolRM2CM", __FILE__, AOloopControl_computeCalib_mkCM_cli,
                       "make control matrix from response matrix", "<RMimage> <SVDlim>", "aolRM2CM respM contrM 0.1",
                       "long AOloopControl_computeCalib_mkCM(char *respm_name, char *cm_name, float SVDlim)");

    RegisterCLIcommand(
        "aolmkmodes", __FILE__, AOloopControl_computeCalib_mkModes_cli, "make control modes",
        "<output modes> <sizex> <sizey> <max CPA> <delta CPA> <cx> <cy> <r0> <r1> <masking mode> <block> <SVDlim>",
        "aolmkmodes modes 50 50 5.0 0.8 1 2 0.01",
        "long AOloopControl_computeCalib_mkModes(char *ID_name, long msizex, long msizey, float CPAmax, float "
        "deltaCPA, double xc, double yx, double r0, double r1, int MaskMode, int BlockNB, float SVDlim, char "
        "*stagedir)");

    RegisterCLIcommand(
        "aolmkmodesM", __FILE__, AOloopControl_computeCalib_mkModes_Simple_cli, "make control modes in modal DM mode",
    ...
    RegisterCLIcommand(
        "aolRMmkmasks", __FILE__, AOloopControl_computeCalib_mkCalib_map_mask_cli,
        "make sensitivity maps and masks from response matrix",
        "<zrespm fname [string]> <output WFS response map fname [string]>  <output DM response map fname [string]> "
        "<percentile low> <coefficient low> <percentile high> <coefficient high>",
        "aolRMmkmasks .. 0.2 1.0 0.5 0.3 0.05 1.0 0.65 0.3",
        "int AOloopControl_computeCalib_mkCalib_map_mask(long loop, char *zrespm_name, char *WFSmap_name, char "
        "*DMmap_name, float dmmask_perclow, float dmmask_coefflow, float dmmask_perchigh, float dmmask_coeffhigh, "
    ...
    RegisterCLIcommand("aolproczrm", __FILE__, AOloopControl_computeCalib_Process_zrespM_cli,
                       ...
                       "<input zrespm fname [string]> <input WFS ref fname [string]> <output zrespm [string]> <output "
                       "WFS response map fname [string]>  <output DM response map fname [string]>",
                       "aolproczrm zrespmat0 wfsref0 zrespm wfsmap dmmap",
                       "int AOloopControl_computeCalib_Process_zrespM(long loop, char *IDzrespm0_name, char "
                       "*IDwfsref_name, char *IDzrespm_name, char *WFSmap_name, char *DMmap_name)");

    RegisterCLIcommand("aolcleanzrm", __FILE__, AOloopControl_computeCalib_ProcessZrespM_cli,
                       "clean zonal resp mat, WFS ref, DM and WFS response maps",
                       "<zrespm fname [string]> <output WFS ref fname [string]>  <output WFS response map fname "
                       "[string]>  <output DM response map fname [string]> <RM ampl [um]>",
                       "aolcleanzrm zrm wfsref wfsmap dmmap 0.05",
                       "int AOloopControl_computeCalib_ProcessZrespM_medianfilt(long loop, char *zrespm_name, char "
                       "*WFSref0_name, char *WFSmap_name, char *DMmap_name, double ampl)");

    RegisterCLIcommand(
        "aolcompcmatc", __FILE__, AOloopControl_computeCalib_compute_CombinedControlMatrix_cli,
        ...
        "long AOloopControl_computeCalib_compute_CombinedControlMatrix(char *IDcmat_name, char *IDmodes_name, char* "
        "IDwfsmask_name, char *IDdmmask_name, char *IDcmatc_name, char *IDcmatc_active_name)");

    RegisterCLIcommand(
        "aolcmmake", __FILE__, AOloopControl_computeCalib_computeCM_cli, "make control matrix",
        "<NBmodes removed> <RespMatrix> <ContrMatrix> <beta> <nbremovedstep> <eigenvlim>", "aolcmmake 8 respm cmat",
        "int AOloopControl_computeCalib_compute_ControlMatrix(long loop, long NB_MODE_REMOVED, char *ID_Rmatrix_name, "
        "char *ID_Cmatrix_name, char *ID_VTmatrix_name, double Beta, long NB_MODE_REMOVED_STEP, float eigenvlim)");

    RegisterCLIcommand("aolloadcm", __FILE__, AOloopControl_computeCalib_loadCM_cli,
                       "load new control matrix from file", "<fname>", "aolloadcm cm32.fits",
...
errno_t AOcontrolLoop_computeCalib_ComputeCM_FPCONF()
{
    ...
    function_parameter_add_entry(&fps, ".AOloopindex", "loop index", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &loop_default,
                                 NULL);

    double SVDlim_default[4] = {0.01, 0.000001, 1.0, 0.01};
    //__attribute__((unused)) long fpi_SVDlim =
    function_parameter_add_entry(&fps, ".SVDlim", "RM poke amplitude", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT,
                                 &SVDlim_default, NULL);

    double CPAmax_default[4] = {10.0, 0.1, 100.0, 10.0};
    //__attribute__((unused)) long fpi_CPAmax =
    function_parameter_add_entry(&fps, ".CPAmax", "maximum controlled CPA", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT,
                                 &CPAmax_default, NULL);

    double deltaCPA_default[4] = {0.8, 0.1, 2.0, 0.8};
    //__attribute__((unused)) long fpi_deltaCPA =
    function_parameter_add_entry(&fps, ".deltaCPA", "delta CPA", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT,
                                 &deltaCPA_default, NULL);

    double alignCX_default[4] = {0.0, 0.0, 2000.0, 0.0};
    //__attribute__((unused)) long fpi_alignCX =
    function_parameter_add_entry(&fps, ".align.CX", "DM mask center X (if no DMmaskRM)", FPTYPE_FLOAT64,
                                 FPFLAG_DEFAULT_INPUT, &alignCX_default, NULL);

    double alignCY_default[4] = {0.0, 0.0, 2000.0, 0.0};
    //__attribute__((unused)) long fpi_alignCY =
    function_parameter_add_entry(&fps, ".align.CY", "DM mask center Y (if no DMmaskRM)", FPTYPE_FLOAT64,
                                 FPFLAG_DEFAULT_INPUT, &alignCY_default, NULL);

    double alignID_default[4] = {0.0, 0.0, 2000.0, 0.0};
    //__attribute__((unused)) long fpi_alignID =
    function_parameter_add_entry(&fps, ".align.ID", "mask I.D. (if no DMmaskRM)", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT,
                                 &alignID_default, NULL);

    double alignOD_default[4] = {0.0, 0.0, 2000.0, 0.0};
    //__attribute__((unused)) long fpi_alignOD =
    function_parameter_add_entry(&fps, ".align.OD", "mask O.D. (if no DMmaskRM)", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT,
                                 &alignOD_default, NULL);

    long DMxsize_default[4] = {10, 1, 2000, 10};
    //__attribute__((unused)) long fpi_DMxsize =
    function_parameter_add_entry(&fps, ".DMxsize", "DM x size", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &DMxsize_default,
                                 NULL);

    long DMysize_default[4] = {10, 1, 2000, 10};
    //__attribute__((unused)) long fpi_DMysize =
    function_parameter_add_entry(&fps, ".DMysize", "DM y size", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &DMysize_default,
                                 NULL);

    // FPS

    long fpi_FPS_zRMacqu = 0;
    function_parameter_add_entry(&fps, ".FPS_zRMacqu", "FPS zonal RM acquisition", FPTYPE_FPSNAME,
                                 FPFLAG_DEFAULT_INPUT | FPFLAG_FPS_RUN_REQUIRED, pNull, &fpi_FPS_zRMacqu);
    FUNCTION_PARAMETER_STRUCT FPS_zRMacqu;
    fps.parray[fpi_FPS_zRMacqu].info.fps.FPSNBparamMAX = 0;

    long fpi_FPS_loRMacqu = 0;
    function_parameter_add_entry(&fps, ".FPS_loRMacqu", "FPS low order modal RM acquisition", FPTYPE_FPSNAME,
                                 FPFLAG_DEFAULT_INPUT | FPFLAG_FPS_RUN_REQUIRED, pNull, &fpi_FPS_loRMacqu);
    ...
    function_parameter_add_entry(&fps, ".DMslaved", "DM slaved actuators", FPTYPE_FITSFILENAME,
                                 FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, pNull, NULL);

    //__attribute__((unused)) long fpi_fname_zrespM =
    function_parameter_add_entry(&fps, ".zrespM", "Zonal response matrix", FPTYPE_FITSFILENAME,
                                 FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, pNull, NULL);

    //__attribute__((unused)) long fpi_fname_dmmaskRM =
    function_parameter_add_entry(&fps, ".DMmaskRM", "actuators directly controlled", FPTYPE_FITSFILENAME,
                                 FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, pNull, NULL);

    //__attribute__((unused)) long fpi_fname_WFSmask =
    function_parameter_add_entry(&fps, ".WFSmask", "WFS mask", FPTYPE_FITSFILENAME,
                                 FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, pNull, NULL);

    //__attribute__((unused)) long fpi_fname_loRM =
    function_parameter_add_entry(&fps, ".loRM", "low order modal response matrix", FPTYPE_FITSFILENAME,
                                 FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, pNull, NULL);

    //__attribute__((unused)) long fpi_fname_loRMmodes =
    function_parameter_add_entry(&fps, ".loRMmodes", "low order RM modes", FPTYPE_FITSFILENAME,
                                 FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, pNull, NULL);

    //__attribute__((unused)) long fpi_fname_extrablockM =
    function_parameter_add_entry(&fps, ".option.extrablockM", "extra modes block", FPTYPE_FITSFILENAME,
                                 FPFLAG_DEFAULT_INPUT, pNull, NULL);

    //__attribute__((unused)) long fpi_fname_exclmodes =
    function_parameter_add_entry(&fps, ".option.exclmodes", "excluded modes", FPTYPE_FITSFILENAME, FPFLAG_DEFAULT_INPUT,
                                 pNull, NULL);

    // Update Actions
    long fpi_update_RMfiles = 0;
    function_parameter_add_entry(&fps, ".upRMfiles", "update RM files", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, pNull,
                                 &fpi_update_RMfiles);

    long fpi_update_align = 0;
    function_parameter_add_entry(&fps, ".upAlign", "update default align (if no DMmaskRM)", FPTYPE_ONOFF,
                                 FPFLAG_DEFAULT_INPUT, pNull, &fpi_update_align);
    ...
    function_parameter_add_entry(&fps, ".out.label", "output label", FPTYPE_STRING, FPFLAG_DEFAULT_INPUT, pNull, NULL);
        ...
        if (function_parameter_FPCONFloopstep(&fps) == 1) // Apply logic if update is needed
        {
            ...
            if (fps.parray[fpi_update_RMfiles].fpflag & FPFLAG_ONOFF)
            {
                ...
                if (fps.parray[fpi_FPS_loRMacqu].info.fps.FPSNBparamMAX > 0)
                {
                    ...
                    strncpy(datadir, functionparameter_GetParamPtr_STRING(&FPS_loRMacqu, ".conf.datadir"),
                            FUNCTION_PARAMETER_STRMAXLEN);
...
errno_t AOcontrolLoop_computeCalib_ComputeCM_RUN()
{
    ...
    // ===============================
    __attribute__((unused)) long loop = functionparameter_GetParamValue_INT64(&fps, ".AOloopindex");
    ...
    sprintf(datestring, "%04d%02d%02dT%02d%02d%02d", 1900 + tmnow->tm_year, 1 + tmnow->tm_mon, tmnow->tm_mday,
            tmnow->tm_hour, tmnow->tm_min, tmnow->tm_sec);
    ...
    AOloopControl_computeCalib_mkModes("fmodes", DMxsize, DMysize, CPAmax, deltaCPA, align_CX, align_CY, align_ID,
                                       align_OD, MaskMode, BlockNB, SVDlim, fps.md->datadir);
    ...
    WRITE_FULLFILENAME(fnamesrc, "./%s/mkmodestmp/fmodesWFSall.fits", fps.md->datadir);
    ...
    EXECUTE_SYSTEM_COMMAND("cp ./%s/mkmodestmp/NBmodes.txt ./%s/param_NBmodes.txt", fps.md->datadir, fps.md->datadir);

    int OKloop = 1;
    for (int i = 0; i < 20; i++)
    {
        WRITE_FULLFILENAME(fnamesrc, "./%s/mkmodestmp/fmodes_%02d.fits", fps.md->datadir, i);
        ...
        if (OKloop == 1)
        {
            if (is_fits_file(fnamesrc) == 1)
            {
                ...
                WRITE_FULLFILENAME(fnametxt, "./%s/shmim.DMmodes%02d.fname.txt", fps.md->datadir, i);

                EXECUTE_SYSTEM_COMMAND("cp %s %s", fnamesrc, fnamedest);
                WRITE_STRING_TO_FILE(fnametxt, "%s", fnamedest);

                WRITE_FULLFILENAME(fnamesrc, "./%s/mkmodestmp/fmodesWFS_%02d.fits", fps.md->datadir, i);
                WRITE_FULLFILENAME(fnamedest, "respM/respM%02d_%s.fits", i, datestring);
                WRITE_FULLFILENAME(fnametxt, "./%s/shmim.respM%02d.fname.txt", fps.md->datadir, i);

                EXECUTE_SYSTEM_COMMAND("cp %s %s", fnamesrc, fnamedest);
                WRITE_STRING_TO_FILE(fnametxt, "%s", fnamedest);

                WRITE_FULLFILENAME(fnamesrc, "./%s/mkmodestmp/cmat_%02d.fits", fps.md->datadir, i);
                WRITE_FULLFILENAME(fnamedest, "contrM/contrM%02d_%s.fits", i, datestring);
                WRITE_FULLFILENAME(fnametxt, "./%s/shmim.contrM%02d.fname.txt", fps.md->datadir, i);

                EXECUTE_SYSTEM_COMMAND("cp %s %s", fnamesrc, fnamedest);
                WRITE_STRING_TO_FILE(fnametxt, "%s", fnamedest);

                WRITE_FULLFILENAME(fnamesrc, "./%s/mkmodestmp/cmatc_%02d.fits", fps.md->datadir, i);
                WRITE_FULLFILENAME(fnamedest, "contrMc/contrMc%02d_%s.fits", i, datestring);
                WRITE_FULLFILENAME(fnametxt, "./%s/shmim.contrMc%02d.fname.txt", fps.md->datadir, i);

                EXECUTE_SYSTEM_COMMAND("cp %s %s", fnamesrc, fnamedest);
                WRITE_STRING_TO_FILE(fnametxt, "%s", fnamedest);

                WRITE_FULLFILENAME(fnamesrc, "./%s/mkmodestmp/cmatcact_%02d.fits", fps.md->datadir, i);
                WRITE_FULLFILENAME(fnamedest, "contrMcact/contrMcact%02d_%s.fits", i, datestring);
                WRITE_FULLFILENAME(fnametxt, "./%s/shmim.contrMcact%02d.fname.txt", fps.md->datadir, i);
