...
{
    if (CLI_checkarg(1, 1) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 2) + CLI_checkarg(5, 2) +
            CLI_checkarg(6, 2) + CLI_checkarg(7, 2) ==
        0)
    {
        FPAOloopControl_MeasureResp_level1(data.cmdargtoken[1].val.numf, data.cmdargtoken[2].val.numl,
...
{
    if (CLI_checkarg(1, 5) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 2) + CLI_checkarg(5, 2) +
            CLI_checkarg(6, 3) ==
        0)
    {
        FPAOloopControl_MakeLinComb_seq(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numl,
                                        data.cmdargtoken[3].val.numl, data.cmdargtoken[4].val.numl,
...
static errno_t init_module_CLI()
{
    ...
    */
    ...
    RegisterCLIcommand("FPaolloadconf", __FILE__, FPAOloopControl_loadconfigure_cli, "load FPAO loop configuration",
                       "<loop #>", "FPaolloadconf 1", "int FPAOloopControl_loadconfigure(long loopnb, 1, 10)");

    RegisterCLIcommand("FPaoconfshow", __FILE__, FPAOloopControl_showparams_cli, "show FPAOconf parameters",
                       "no argument", "FPaoconfshow", "int FPAOloopControl_showparams(long loop)");

    RegisterCLIcommand("FPaolsethlat", __FILE__, FPAOloopControl_set_hardwlatency_frame_cli,
                       "set FPAO hardware latency", "<hardware latency [frame]>", "FPaolsethlat 0.7",
    ...
    RegisterCLIcommand("FPaoMeasRespl1", __FILE__, FPAOloopControl_MeasureResp_level1_cli,
                       ...
                       "<ampl [um]> <delay frame [long]> <delayus [long]> <NBave> <NB frame excl> <initMode> <NBiter>",
                       "FPaoMeasRespl1 0.05 1 231 5 1 0 10",
                       "long FPAOloopControl_MeasureResp_level1(float ampl, long delayfr, long delayRM1us, long NBave, "
                       "long NBexcl, int FPAOinitMode, long NBiter)");

    RegisterCLIcommand("FPaomklincombs", __FILE__, FPAOloopControl_MakeLinComb_seq_cli,
                       "make linear comb sequence of DM pokes from set of masters",
                       "<master cube (optional)> <xsize> <ysize> <NBmaster> <N (1+2N steps)> <outCube>",
                       "FPaomklincombs masterC 50 50 3 2 outC",
                       "long FPAOloopControl_MakeLinComb_seq(char *IDpC_name, long xsize0, long ysize0, long "
...
errno_t FPAOloopControl_InitializeMemory(__attribute__((unused)) int mode)
{
    ...
    FPAOconf = (FPAOLOOPCONTROL_CONF *)mmap(0, sizeof(FPAOLOOPCONTROL_CONF) * NB_FPAOloopcontrol,
...
errno_t FPAOloopControl_loadconfigure(long loop, __attribute__((unused)) int mode, __attribute__((unused)) int level)
{
    ...
    FPaoconfID_wfsdark =
        AOloopControl_IOtools_2Dloadcreate_shmim(name, fname, FPAOconf[loop].sizexWFS, FPAOconf[loop].sizeyWFS, 0.0);
    ...
    FPaoconfID_imWFS0 =
        AOloopControl_IOtools_2Dloadcreate_shmim(name, " ", FPAOconf[loop].sizexWFS, FPAOconf[loop].sizeyWFS, 0.0);
...
#pragma omp for
        ...
        for (ii = 0; ii < nelem; ii++)
        {
            data.image[FPaoconfID_imWFS0].array.F[ii] =
                ((float)arrayutmp[ii]) - data.image[FPaoconfID_wfsdark].array.F[ii];
        }
#ifdef _OPENMP
    }
#endif
                           ...
                           char *IDoutC_name, int FPAOinitMode, long NBcycle)
{
    ...
    while ((iter < NBiter) && (data.signal_USR1 == 0))
    {
        ...
        memcpy(data.image[FPaoconfID_dmRM].array.F, ptr0 + PokeIndex1 * framesize,
        ...
        while ((PokeIndex < NBpoke) && (data.signal_USR1 == 0))
        {
            ...
            for (kk = 0; kk < NBave + NBexcl; kk++)
            {
                ...
                if (kk < NBave)
                    for (ii = 0; ii < FPAOconf[loop].sizeWFS; ii++)
                    {
                        data.image[IDoutC].array.F[PokeIndex * FPAOconf[loop].sizeWFS + ii] +=
                                        ...
                                        int FPAOinitMode, long NBiter)
{
    ...
    // CREATE POKE CUBE
    create_3Dimage_ID("pokeC", FPAOconf[loop].sizexDM, FPAOconf[loop].sizeyDM, NBpokes, &IDpokeC);
    ...
    for (ii = 0; ii < FPAOconf[loop].sizexDM; ii++)
        for (jj = 0; jj < FPAOconf[loop].sizexDM; jj++)
        {
            data.image[IDpokeC].array.F[poke * FPAOconf[loop].sizeDM + jj * FPAOconf[loop].sizexDM + ii] =
                ampl * (ii % 2);
        }
        ...
        for (k = 0; k < NBmaster; k++)
        {
            ...
            for (ii = 0; ii < xysize; ii++)
            {
                data.image[IDout].array.F[xysize * kk + ii] +=
                    N1array[narray[k]] * data.image[IDpC].array.F[xysize * k + ii];
            }
        }
        printf("\n");
