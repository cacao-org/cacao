...
long AOloopControl_mkDM_TT_circle(char *IDoutname, long DMindex, long NBpts,
                                  float ampl)
{
    ...
    for(kk = 0; kk < zsize; kk++)
    {
        ...
        for(ii = 0; ii < xsize; ii++)
        {
            ...
            {
                y = 2.0 * jj / ysize - 1.0;
                data.image[IDout].array.F[kk * xysize + jj * xsize + ii] = x * xslope + y *
                        yslope;
            }
        }
    }
...
long AOloopControl_DM_mkAstroGrid_seq(char *IDoutname, long DMindex, int XYmode,
                                      int bin, long NBcycle)
{
    long xsize, ysize, zsize, xysize;
    long IDout;
    long ii, jj, kk, kk1;
    ...
    for(ii = 0; ii < xsize; ii++)
        for(jj = 0; jj < ysize; jj++)
        {
            data.image[IDxy].array.F[jj * xsize + ii] = data.image[IDx].array.F[jj * xsize +
                    ii] * data.image[IDy].array.F[jj * xsize + ii];
        }
        ...
        case 5: // XYdiag -> OFF ->
            ...
            for(kk = 0; kk < 2 * NBcycle; kk++)
            {
                for(ii = 0; ii < xsize; ii++)
                    for(jj = 0; jj < ysize; jj++)
                    {
                        data.image[IDout].array.F[kk * xysize + jj * ysize + ii] = sign *
                                data.image[IDxy].array.F[jj * xsize + ii];
                    }
                sign *= -1;
            }
