...
int AOloopControl_DM_CombineChannels_FPCONF()
{
    ...
    function_parameter_add_entry(
        &fps, ".DMindex", "Deformable mirror index", FPTYPE_INT64, FPFLAG, &DMindex_default, &fp_DMindex);
    ...
    long fp_DMxsize = 0;
    function_parameter_add_entry(
        &fps, ".DMxsize", "Deformable mirror X size", FPTYPE_INT64, FPFLAG, &DMsize_default, &fp_DMxsize);
    long fp_DMysize = 0;
    function_parameter_add_entry(
        &fps, ".DMysize", "Deformable mirror Y size", FPTYPE_INT64, FPFLAG, &DMsize_default, &fp_DMysize);

    long DMMODE_default[4] = {0, 0, 1, 0};
    long fp_DMMODE = 0;
    function_parameter_add_entry(
        &fps, ".DMMODE", "0:SquareGrid, 1:Generic", FPTYPE_INT64, FPFLAG, &DMMODE_default, &fp_DMMODE);
    ...
    long fp_NBchannel = 0;
    function_parameter_add_entry(
        &fps, ".NBchannel", "Number of channels", FPTYPE_INT64, FPFLAG, &NBchannel_default, &fp_NBchannel);
    ...
    long fp_AveMode = 0;
    function_parameter_add_entry(
        &fps, ".AveMode", "Averaging mode", FPTYPE_INT64, FPFLAG, &AveMode_default, &fp_AveMode);
    ...
    function_parameter_add_entry(
        &fps, ".option.wfsrefmode", "WFS ref mode", FPTYPE_ONOFF, FPFLAG, pNull, &fp_wfsrefmode);
    ...
    function_parameter_add_entry(
        &fps, ".option.wfsref_out", "Output WFS", FPTYPE_STREAMNAME, FPFLAG_DEFAULT_INPUT, pNull, &fp_wfsref_out);

    long fp_voltmode = 0;
    function_parameter_add_entry(
        &fps, ".option.voltmode", "Volt mode", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, pNull, &fp_voltmode);

    long fp_volttype = 0;
    function_parameter_add_entry(
        &fps, ".option.volttype", "Volt type", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, pNull, &fp_volttype);
    ...
    long fp_voltname = 0;
    function_parameter_add_entry(
        &fps, ".option.voltname", "Stream name for volt output", FPTYPE_STREAMNAME, FPFLAG, pNull, &fp_voltname);

    double DClevel_default[4] = {0.0, 0.0, 100.0, 0.0};
    long fp_DClevel = 0;
    function_parameter_add_entry(
        &fps, ".option.DClevel", "DC level [um]", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, &DClevel_default, &fp_DClevel);

    long fp_maxvolt = 0;
    function_parameter_add_entry(
        &fps, ".option.maxvolt", "Maximum voltage", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, pNull, &fp_maxvolt);

    // status (RO)
    long fp_loopcnt = 0;
    function_parameter_add_entry(
        &fps, ".status.loopcnt", "Loop counter", FPTYPE_INT64, FPFLAG_DEFAULT_STATUS, pNull, &fp_loopcnt);
...
int AOloopControl_DM_CombineChannels_RUN()
{
  ...
  */
    ...
    if (wfsrefmode == 1)
        {
            strncpy(wfsref_WFSRespMat,
                    functionparameter_GetParamPtr_STRING(&fps, ".option.wfsref_WFSRespMat"),
                    FUNCTION_PARAMETER_STRMAXLEN);
            strncpy(wfsref_out,
    ...
    if (dm2dm_mode == 1)
        {
            ...
            if (data.image[dmdispcombconf[DMindex].ID_dm2dm_outdisp].md[0].size[0] != dmdispcombconf[DMindex].xsizeout)
                {
                    PRINT_ERROR(
                        "image \"%s\" should have x axis = %ld", dm2dm_outdisp, dmdispcombconf[DMindex].xsizeout);
                    exit(0);
                }
            if (data.image[dmdispcombconf[DMindex].ID_dm2dm_outdisp].md[0].size[1] != dmdispcombconf[DMindex].ysizeout)
                {
                    PRINT_ERROR(
                        "image \"%s\" should have y axis = %ld", dm2dm_outdisp, dmdispcombconf[DMindex].ysizeout);
                    exit(0);
                }

            create_2Dimage_ID(
                "_tmpoutdm", dmdispcombconf[DMindex].xsizeout, dmdispcombconf[DMindex].ysizeout, &IDtmpoutdm);
    ...
    if (wfsrefmode == 1)
        {
            ...
            if (data.image[dmdispcombconf[DMindex].ID_wfsref_out].md[0].size[0] != dmdispcombconf[DMindex].xsizewfsref)
                {
                    PRINT_ERROR(
                        "image \"%s\" should have x axis = %ld", wfsref_out, dmdispcombconf[DMindex].xsizewfsref);
                    exit(0);
                }
            if (data.image[dmdispcombconf[DMindex].ID_wfsref_out].md[0].size[1] != dmdispcombconf[DMindex].ysizewfsref)
                {
                    PRINT_ERROR(
                        "image \"%s\" should have y axis = %ld", wfsref_out, dmdispcombconf[DMindex].ysizewfsref);
                    exit(0);
                }
            ...
            fflush(stdout);
            create_2Dimage_ID(
                "_tmpoutref", dmdispcombconf[DMindex].xsizewfsref, dmdispcombconf[DMindex].ysizewfsref, &IDtmpoutref);
                                    ...
                                    for (uint64_t ii = 0; ii < sizexywfsref; ii++)
                                        {
                                            data.image[IDtmpoutref].array.F[ii] +=
                                                data.image[dmdispcombconf[DMindex].IDdisp].array.F[kk] *
                                                data.image[dmdispcombconf[DMindex].ID_wfsref_RespMat]
