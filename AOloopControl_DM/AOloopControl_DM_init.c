...
static errno_t AOloopControl_DM_createconf()
{
    ...
    if (dmdispcomb_loaded == 0)
    {
        ...
        {
            int slen = snprintf(
                errstr, STRINGMAXLEN_ERRORMSG,
                "Error opening (O_RDWR | O_CREAT | O_TRUNC) file \"%s\", function AOloopControl_DM_createconf", fname);
            if (slen < 1)
            {
        ...
        dmdispcombconf = (AOLOOPCONTROL_DM_DISPCOMB_CONF *)mmap(0, sizeof(AOLOOPCONTROL_DM_DISPCOMB_CONF) * NB_DMindex,
