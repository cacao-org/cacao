...
static errno_t AOloopControl_DM_createconf()
{
    ...
    if(dmdispcomb_loaded == 0)
    {
        ...
        if(SMfd == -1)
        {
            int slen = snprintf(errstr, STRINGMAXLEN_ERRORMSG,
                                "Error opening (O_RDWR | O_CREAT | O_TRUNC) file \"%s\", function AOloopControl_DM_createconf",
                                fname);
            if(slen < 1)
            {
        ...
        result = lseek(SMfd, sizeof(AOLOOPCONTROL_DM_DISPCOMB_CONF) * NB_DMindex - 1,
                       SEEK_SET);
        if(result == -1)
        {
            close(SMfd);
        ...
        dmdispcombconf = (AOLOOPCONTROL_DM_DISPCOMB_CONF *)mmap(0,
                         sizeof(AOLOOPCONTROL_DM_DISPCOMB_CONF) * NB_DMindex, PROT_READ | PROT_WRITE,
                         MAP_SHARED, SMfd, 0);
...
errno_t AOloopControl_DM_unloadconf()
{
    ...
    {
        if(munmap(dmdispcombconf,
                  sizeof(AOLOOPCONTROL_DM_DISPCOMB_CONF)*NB_DMindex) == -1)
        {
