...
errno_t AOloopControl_printDMconf()
{
    ...
    printf("DM | on |  x |  y | Nbch | busy | ave | DClevel | monint  | stat | IDdisp | voltmode | volttype | "
    ...
    for (DMindex = 0; DMindex < NB_DMindex; DMindex++)
    {
        ...
        printf("%02ld |  %1d |%3u |%3u |  %02ld  |   %1d  |  %1d  | %6.2f  |%8ld |   %02d |   %3ld  |    %4d  |    %4d "
               " | %9.2f |   %3s  |  %6s | %11s |\n",
               DMindex, dmdispcombconf[DMindex].ON, dmdispcombconf[DMindex].xsize, dmdispcombconf[DMindex].ysize,
               dmdispcombconf[DMindex].NBchannel, dmdispcombconf[DMindex].busy, dmdispcombconf[DMindex].AveMode,
               dmdispcombconf[DMindex].DClevel, dmdispcombconf[DMindex].moninterval, dmdispcombconf[DMindex].status,
               dmdispcombconf[DMindex].IDdisp, dmdispcombconf[DMindex].voltmode, dmdispcombconf[DMindex].volttype,
               dmdispcombconf[DMindex].stroke100, IDvolt_str, maxvolt_str, voltname_str);
    }
...
int AOloopControl_DM_dmdispcombstatus(long DMindex)
{
    ...
    while (!kbdhit())
    {
        ...
        printw("size              = %ld x %ld = %ld\n", dmdispcombconf[DMindex].xsize, dmdispcombconf[DMindex].ysize,
        ...
        for (ch = 0; ch < dmdispcombconf[DMindex].NBchannel; ch++)
        {
            printw(" CHANNEL %2d  gain = %10.3f   dm%02lddisp%02ld   %10ld\n", ch,
                   dmdispcombconf[DMindex].dmdispgain[ch], DMindex, ch, dmdispcombconf[DMindex].dmdispcnt[ch]);
        }
        printw("\n");
