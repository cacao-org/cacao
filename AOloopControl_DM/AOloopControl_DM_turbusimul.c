...
errno_t AOloopControl_printDMturbconf()
{
    ...
    {
        printf("%ld  %d  %10f  %10ld  %10f %5f  %5f\n", DMindex, dmturbconf[DMindex].on, dmturbconf[DMindex].ampl,
               dmturbconf[DMindex].tint, dmturbconf[DMindex].simtime, dmturbconf[DMindex].wspeed,
...
errno_t AOloopControl_DMturb_createconf()
{
    ...
    if (dmturb_loaded == 0)
    {
        ...
        if (SMturbfd == -1)
        {
            snprintf(errstr, STRINGMAXLEN_DEFAULT, "Error opening (O_RDWR | O_CREAT | O_TRUNC) file \"%s\"",
                     DMTURBCONF_FILENAME);
            perror(errstr);
            exit(EXIT_FAILURE);
        }
        ...
        dmturbconf = (AOLOOPCONTROL_DMTURBCONF *)mmap(0, sizeof(AOLOOPCONTROL_DMTURBCONF) * NB_DMindex,
                                                      PROT_READ | PROT_WRITE, MAP_SHARED, SMturbfd, 0);
...
errno_t AOloopControl_DMturb_loadconf()
{
    ...
    if (dmturb_loaded == 0)
    {
        ...
        if (SMturbfd == -1)
        {
            sprintf(errstr, "Error opening (O_RDWR) file \"%s\" in function AOloopControl_DMturb_loadconf",
                    DMTURBCONF_FILENAME);
            perror(errstr);
        }
        else
            ...
            for (jj = 0; jj < DM_Ysize; jj++)
            {
                ...
                data.image[IDturb].array.F[ii1] =
                    ...
                    (data.image[IDs1].array.F[ypix1 * size_sx + xpix1] -
                     (1.0 - dmturbconf[DMindex].LOcoeff) * data.image[IDs2].array.F[ypix1 * size_sx + xpix1]);
                data.image[IDturb].array.F[ii1] +=
                    (xpixf) * (1.0 - ypixf) *
                    (data.image[IDs1].array.F[ypix1 * size_sx + xpix2] -
                     (1.0 - dmturbconf[DMindex].LOcoeff) * data.image[IDs2].array.F[ypix1 * size_sx + xpix2]);
                data.image[IDturb].array.F[ii1] +=
                    (1.0 - xpixf) * (ypixf) *
                    (data.image[IDs1].array.F[ypix2 * size_sx + xpix1] -
                     (1.0 - dmturbconf[DMindex].LOcoeff) * data.image[IDs2].array.F[ypix2 * size_sx + xpix1]);
                data.image[IDturb].array.F[ii1] +=
                    xpixf * ypixf *
                    (data.image[IDs1].array.F[ypix2 * size_sx + xpix2] -
                     (1.0 - dmturbconf[DMindex].LOcoeff) * data.image[IDs2].array.F[ypix2 * size_sx + xpix2]);
            }
