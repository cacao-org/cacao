...
//    AOloopControl_DMturb_loadconf(0);
    ...
    {
        printf("%ld  %d  %10f  %10ld  %10f %5f  %5f\n", DMindex, dmturbconf[DMindex].on,
               dmturbconf[DMindex].ampl, dmturbconf[DMindex].tint, dmturbconf[DMindex].simtime,
               dmturbconf[DMindex].wspeed, dmturbconf[DMindex].LOcoeff);
    }
...
errno_t AOloopControl_DMturb_createconf()
{
    ...
    if(dmturb_loaded == 0)
    {
        ...
        result = lseek(SMturbfd, sizeof(AOLOOPCONTROL_DMTURBCONF) * NB_DMindex - 1,
                       SEEK_SET);
        if(result == -1)
        {
            close(SMturbfd);
        ...
        dmturbconf = (AOLOOPCONTROL_DMTURBCONF *)mmap(0,
                     sizeof(AOLOOPCONTROL_DMTURBCONF) * NB_DMindex, PROT_READ | PROT_WRITE,
                     MAP_SHARED, SMturbfd, 0);
...
errno_t AOloopControl_DMturb_loadconf()
{
    ...
    if(dmturb_loaded == 0)
    {
        ...
        {
            sprintf(errstr,
                    "Error opening (O_RDWR) file \"%s\" in function AOloopControl_DMturb_loadconf",
                    DMTURBCONF_FILENAME);
            perror(errstr);
        }
        else
...
int AOloopControl_DM_dmturb(long DMindex, int mode, const char *IDout_name,
                            long NBsamples)
{
    ...
    if(mode == 1) // force periodic sequence if wind speed is sufficiently large
    {
        ...
        dX = dmturbconf[DMindex].wspeed * 1.0e-6 * dmturbconf[DMindex].tint * NBsamples
             * cos(angle);
        dY = dmturbconf[DMindex].wspeed * 1.0e-6 * dmturbconf[DMindex].tint * NBsamples
             * sin(angle);
    ...
    while(turbON == 1) // computation loop
    {
        ...
        for(ii = 0; ii < DM_Xsize; ii++)
            for(jj = 0; jj < DM_Ysize; jj++)
            {
                ...
                data.image[IDturb].array.F[ii1] = (1.0 - xpixf) * (1.0 - ypixf) *
                                                  (data.image[IDs1].array.F[ypix1 * size_sx + xpix1] - (1.0 -
                                                          dmturbconf[DMindex].LOcoeff) * data.image[IDs2].array.F[ypix1 * size_sx +
                                                                  xpix1]);
                data.image[IDturb].array.F[ii1]  += (xpixf) * (1.0 - ypixf) *
                                                    (data.image[IDs1].array.F[ypix1 * size_sx + xpix2] - (1.0 -
                                                            dmturbconf[DMindex].LOcoeff) * data.image[IDs2].array.F[ypix1 * size_sx +
                                                                    xpix2]);
                data.image[IDturb].array.F[ii1]  += (1.0 - xpixf) * (ypixf) *
                                                    (data.image[IDs1].array.F[ypix2 * size_sx + xpix1] - (1.0 -
                                                            dmturbconf[DMindex].LOcoeff) * data.image[IDs2].array.F[ypix2 * size_sx +
                                                                    xpix1]);
                data.image[IDturb].array.F[ii1]  += xpixf * ypixf *
                                                    (data.image[IDs1].array.F[ypix2 * size_sx + xpix2] - (1.0 -
                                                            dmturbconf[DMindex].LOcoeff) * data.image[IDs2].array.F[ypix2 * size_sx +
                                                                    xpix2]);
            }
