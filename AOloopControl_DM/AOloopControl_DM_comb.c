...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long =
        "Combine multiple DM command channels (correction, offsets, perturbations) into a single "
        "output. Applies per-channel gain, clipping limits, and response matrix mapping."
};
...
static errno_t DMdisp_add_disp_from_circular_buffer(DMCOMB_STATE *state)
{
    if (state->DMdisp_add_disp_from_circular_buffer_init == 0)
    {
        ...
        read_sharedmem_image(astrogridsname, dcimg, dcnimg);
        state->ag_imgdispbuffer = imgid_make_from_name(astrogridsname);
        resolveIMGID(&state->ag_imgdispbuffer, ERRMODE_WARN, dcimg, dcnimg);
        if (state->ag_imgdispbuffer.ID == -1)
        {
    ...
    if ((astrogrid) &FPFLAG_ONOFF)
    {
        ...
        if (state->ag_framecnt >= (astrogridNBframe))
        {
            ...
            if (chan < NBchannel)
            {
                ...
                const float *MILK_RESTRICT inptr =
                    MILK_ASSUME_ALIGNED(state->ag_imgdispbuffer.im->array.F);
 ...
 * from the hot loop.
 */
static errno_t DM_displ2V(IMGID imgdisp, IMGID imgvolt, DMCOMB_STATE *state)
{
    uint64_t xysize   = (uint64_t) (DMxsize) * (DMysize);
    double  *valarray = state->valarray;

    if ((volttype) == 1)
    {
        float inrange = (maxvolt) * (stroke100) / 100.0f;
    ...
    else if ((volttype) == 2)
    {
        ...
        outv_outrange_max = (maxvolt) / 300.0f * 16384.0f;
    }

    {
        ...
        if (fabsf(exp_val - 1.0f) < 1.0e-6f)
        {
            ...
            {
                float x = (imgdisp.im->array.F[ii] - inmin) * inv_range;
                if (x < 0.0f)
                {
                    x = 0.0f;
                }
                ...
                valarray[ii] = outmin + x * outscale;
            }
        }
        else if (fabsf(exp_val - 0.5f) < 1.0e-6f)
        {
            /* exp == 0.5: use sqrtf */
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
                ...
                valarray[ii] = outmin + sqrtf(x) * outscale;
            }
        }
        else
        {
            ...
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
                ...
                valarray[ii] = outmin + powf(x, exp_val) * outscale;
            }
        }
    }
    ...
    if ((volttype) == 1)
    {
        float scale = 100.0f / (stroke100);
        float maxv  = maxvolt;
        for (uint64_t ii = 0; ii < xysize; ii++)
        {
            float v = scale * imgdisp.im->array.F[ii];
            if (v > maxv)
            {
                v = maxv;
            }
    ...
    else if ((volttype) == 2)
    {
        ...
        for (uint64_t ii = 0; ii < xysize; ii++)
        {
            ...
            float volt = 100.0f * sqrtf(val * inv_s);
            if (volt > maxv)
            {
                volt = maxv;
            }
            imgvolt.im->array.UI16[ii] = (unsigned short int) (volt * vscale);
        }
    }
    else if ((volttype) == 3)
    {
        ...
        for (uint64_t ii = 0; ii < xysize; ii++)
        {
            float volt = imgdisp.im->array.F[ii] * inv_s + 0.5f;
            if (volt > maxv)
            {
                volt = remainderf(volt, 1.0f);
            }
            if (volt < 0.0f)
            {
                volt = remainderf(volt, 1.0f);
    ...
    else if ((volttype) == 0)
    {
        ...
        else if (strcmp(outv_ftype, "uint16") == 0)
        {
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
        ...
        else if (strcmp(outv_ftype, "uint32") == 0)
        {
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
        ...
        else if (strcmp(outv_ftype, "uint64") == 0)
        {
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
        ...
        else if (strcmp(outv_ftype, "int16") == 0)
        {
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
        ...
        else if (strcmp(outv_ftype, "int32") == 0)
        {
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
        ...
        else if (strcmp(outv_ftype, "int64") == 0)
        {
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
...
static errno_t update_dmdispzpo(IMGID  imgdisp,
                                IMGID *imgch,
                                float *dmdisptmp,
...
static DMCOMB_STATE *dmcomb_init()
{
    ...
    for (uint32_t ch = 0; ch < NBchannel; ch++)
    {
        ...
        imageID IDch = read_sharedmem_image(name, dcimg, dcnimg);
        if (UNLIKELY(milk_data.Debug > 0))
        {
            printf("DEBUG: ID = %ld\n", IDch);
            fflush(stdout);
        ...
        state->imgch[ch] = stream_connect_create_2Df32(name, DMxsize, DMysize);
    }
    ...
    if (DMcombout[0] != '\0')
    {
        state->imgdisp = stream_connect_create_2Df32(DMcombout, DMxsize, DMysize);
    }

    if (DMcomboutzpo[0] != '\0')
    {
        state->imgdispzpo = stream_connect_create_2Df32(DMcomboutzpo, DMxsize, DMysize);
    }
    ...
    state->dmdisptmp = malloc(sizeof(float) * (DMxsize) * (DMysize));
    state->valarray  = malloc(sizeof(double) * (DMxsize) * (DMysize));
    ...
    {
        if (image_ID(voltname, dcimg, dcnimg) == -1)
        {
            read_sharedmem_image(voltname, dcimg, dcnimg);
        }
        state->imgdmvolt = imgid_make_from_name(voltname);
        resolveIMGID(&state->imgdmvolt, ERRMODE_WARN, dcimg, dcnimg);
        if (state->imgdmvolt.ID == -1)
        {
            return NULL;
        }
    }
...
static void dmcomb_step(PROCESSINFO *processinfo, FPS *fps, DMCOMB_STATE *state)
{
    if (UNLIKELY(milk_data.Debug > 0))
    {
        printf("DEBUG  %s [%d] %s\n", __FILE__, __LINE__, __FUNCTION__);
    ...
    {
        if (state->zpoffset_channel[ch])
        {
            state->zpochecksum += (1 << ch);
        }
    }
    if (state->zpochecksum != state->zpochecksum0)
    {
        zpooffsetchange = 1;
    }
    ...
    if (((zpoffsetenable) &FPFLAG_ONOFF) && zpooffsetchange)
    {
        DMupdatezpo = 1;
    }
    ...
    if (LIKELY(DMupdate))
    {
        if (!((astrogrid) &FPFLAG_ONOFF))
        {
            state->DMdisp_add_disp_from_circular_buffer_init = 0;
        }

        if (((astrogrid) &FPFLAG_ONOFF) && (astrogridtdelay == 0))
        {
            DMdisp_add_disp_from_circular_buffer(state);
            processinfo_update_output_stream(processinfo, state->imgch[astrogridchan].im, NULL);
        }
    ...
    if (UNLIKELY(DMupdatezpo))
    {
        ...
        {
            update_dmdispzpo(state->imgdispzpo, state->imgch, state->dmdisptmp,
...
#define FPS_PARAMS(X)                                                                             \
    X(".DMindex", &DMindex, FPTYPE_UINT32, 1,                                                     \
      FPFLAG_DEFAULT_INPUT | FPFLAG_MINLIMIT | FPFLAG_MAXLIMIT, "DM index")                       \
    X(".DMcombout", DMcombout, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT,                        \
      "output combined cmd stream")                                                               \
    X(".DMxsize", &DMxsize, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT, "x size")                     \
    X(".DMysize", &DMysize, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT, "y size")                     \
    X(".NBchannel", &NBchannel, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT, "number of DM channels")  \
    X(".DMmode", &DMmode, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT, "0:SquareGrid 1:Generic")       \
    X(".AveMode", &AveMode, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT, "Piston (mean) subtract")     \
    X(".option.dm2dm_mode", &dm2dm_mode, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                   \
      "DM to DM offset mode")                                                                     \
    X(".option.dm2dm_DMmodes", dm2dm_DMmodes, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,         \
      "output stream DM to DM")                                                                   \
    X(".option.dm2dm_outdisp", dm2dm_outdisp, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,         \
      "DM output data stream")                                                                    \
    X(".option.wfsrefmode", &wfsrefmode, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT, "WFS ref mode")   \
    X(".option.wfsref_WFSRespMat", wfsref_WFSRespMat, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT, \
      "Output WFS resp matrix")                                                                   \
    X(".option.wfsref_out", wfsref_out, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT, "Output WFS") \
    X(".option.voltmode", &voltmode, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT, "Volt mode")          \
    X(".option.volttype", &volttype, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT, "volt type")         \
    X(".option.stroke100", &stroke100, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT,                   \
      "Stroke for 100V [um]")                                                                     \
    X(".option.voltname", voltname, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,                   \
      "Stream name volt output")                                                                  \
    X(".option.outv_ftype", outv_ftype, FPTYPE_STRING, 0, FPFLAG_DEFAULT_INPUT,                   \
      "output volt type")                                                                         \
    X(".option.outv_exp", &outv_exp, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT,                     \
      "output volt power exp")                                                                    \
    X(".option.outv_inrange_min", &outv_inrange_min, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT,     \
      "inrange min")                                                                              \
    X(".option.outv_inrange_max", &outv_inrange_max, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT,     \
      "inrange max")                                                                              \
    X(".option.outv_outrange_min", &outv_outrange_min, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT,   \
      "outrange min")                                                                             \
    X(".option.outv_outrange_max", &outv_outrange_max, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT,   \
      "outrange max")                                                                             \
    X(".option.DClevel", &DClevel, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT, "DC level [um]")      \
    X(".option.maxvolt", &maxvolt, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT, "Maximum voltage")    \
    X(".status.loopcnt", &loopcnt, FPTYPE_UINT64, 0, FPFLAG_DEFAULT_INPUT, "Loop counter")        \
    X(".astrogrid.mode", &astrogrid, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                       \
      "circular buffer on/off")                                                                   \
    X(".astrogrid.chan", &astrogridchan, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT,                  \
      "astrogrid DM channel")                                                                     \
    X(".astrogrid.sname", astrogridsname, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,             \
      "astrogrid cube name")                                                                      \
    X(".astrogrid.mult", &astrogridmult, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT,                 \
      "astrogrid mult coeff")                                                                     \
    X(".astrogrid.delay", &astrogridtdelay, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT,               \
      "astrogrid delay [us]")                                                                     \
    X(".astrogrid.nbframe", &astrogridNBframe, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT,            \
      "nb frame per slice")                                                                       \
    X(".zpoffset.enable", &zpoffsetenable, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                 \
      "zero point offset enable")                                                                 \
    X(".zpoffset.DMcomboutzpo", DMcomboutzpo, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,         \
      "output combined ZPO")                                                                      \
    X(".zpoffset.ch00", &zpoffsetch00, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 00 zpoffset")                                                                      \
    X(".zpoffset.ch01", &zpoffsetch01, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 01 zpoffset")                                                                      \
    X(".zpoffset.ch02", &zpoffsetch02, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 02 zpoffset")                                                                      \
    X(".zpoffset.ch03", &zpoffsetch03, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 03 zpoffset")                                                                      \
    X(".zpoffset.ch04", &zpoffsetch04, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 04 zpoffset")                                                                      \
    X(".zpoffset.ch05", &zpoffsetch05, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 05 zpoffset")                                                                      \
    X(".zpoffset.ch06", &zpoffsetch06, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 06 zpoffset")                                                                      \
    X(".zpoffset.ch07", &zpoffsetch07, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 07 zpoffset")                                                                      \
    X(".zpoffset.ch08", &zpoffsetch08, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 08 zpoffset")                                                                      \
    X(".zpoffset.ch09", &zpoffsetch09, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 09 zpoffset")                                                                      \
    X(".zpoffset.ch10", &zpoffsetch10, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                     \
      "channel 10 zpoffset")                                                                      \
    X(".zpoffset.ch11", &zpoffsetch11, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT, "channel 11 zpoffset")
...
static errno_t compute_function()
{
    ...
    dmcomb_step(processinfo, milk_data.fpsptr, state);
