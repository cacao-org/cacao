...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long =
        "Measure the end-to-end latency between DM commands and WFS responses. Applies a known DM "
        "pattern and cross-correlates with WFS to find the delay."
};
...
#define FPS_PARAMS(X)                                                                             \
    X(".dmstream", &dmstream, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "DM stream")            \
    X(".wfsstream", &wfsstream, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "WFS stream")         \
    X(".OPDamp", &OPDamp, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT, "poke amplitude [um]")         \
    X(".pokemap", &pokemap, FPTYPE_STREAMNAME, 1, FPFLAG_DEFAULT_INPUT, "optional DM poke map")   \
    X(".CPA", &CPA, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT, "Cycles/aperture [float]")           \
    X(".frameratewait", &frameratewait, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT,                  \
      "time for frame rate measurement")                                                          \
    X(".NBiter", &NBiter, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT, "Number of iteration")          \
    X(".wfsNBframemax", &wfsNBframemax, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT,                   \
      "Number frames in meas sequence")                                                           \
    X(".status.wfsdt", &wfsdt, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_OUTPUT, "WFS frame interval")    \
    X(".status.twaitus", &twaitus, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_OUTPUT, "initial wait [us]") \
    X(".status.refdtoffset", &refdtoffset, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_OUTPUT,              \
      "baseline time offset to poke")                                                             \
    X(".status.dtoffset", &dtoffset, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_OUTPUT,                    \
      "actual time offset to poke")                                                               \
    X(".out.framerateHz", &framerateHz, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_OUTPUT,                 \
      "WFS frame rate [Hz]")                                                                      \
    X(".out.latencyfr", &latencyfr, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_OUTPUT,                     \
      "hardware latency [frame]")                                                                 \
    X(".option.saveraw", &saveraw, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT, "Save raw image cubes") \
    X(".option.saveseq", &saveseq, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT,                         \
      "Save sequence image cube")                                                                 \
    X(".option.seqNBframe", &seqNBframe, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT,                  \
      "Number of frames in seq cube")                                                             \
    X(".option.seqdtfr", &seqdtframe, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_OUTPUT,                   \
...
static errno_t compute_function()
{
    ...
    resolveIMGID(&imgdm, ERRMODE_WARN, dcimg, dcnimg);
    if (imgdm.ID == -1)
    {
    ...
    resolveIMGID(&imgwfs, ERRMODE_WARN, dcimg, dcnimg);
    if (imgwfs.ID == -1)
    {
        return RETURN_FAILURE;
    }
    ...
    resolveIMGID(&imgpokemap, ERRMODE_WARN, dcimg, dcnimg);
    if (imgpokemap.ID != -1)
    {
    ...
    imageID IDwfsc;
    {
        ...
        {
            IMGID imgwfsc         = imgid_make_from_name("_testwfsc");
            imgwfsc.mdt->naxis    = 3;
            imgwfsc.mdt->size[0]  = naxes[0];
            imgwfsc.mdt->size[1]  = naxes[1];
            imgwfsc.mdt->size[2]  = naxes[2];
            imgwfsc.mdt->datatype = imgwfs.md->datatype;
            imgwfsc.im            = (IMAGE *) calloc(1, sizeof(IMAGE));
    ...
    float *diffseqkkcnt =
        (float *) malloc(sizeof(float) * diffseqsize); // count how many frames go in each slice
    ...
    IMGID imgdiffseq;
    imgdiffseq =
        imgid_make_from_name_3D("CMmodesWFS", imgwfs.md->size[0], imgwfs.md->size[1], diffseqsize);
    ...
    imageID IDdm1 = -1;
    {
        ...
        for (uint32_t ii = 0; ii < dmxsize; ii++)
        {
            for (uint32_t jj = 0; jj < dmysize; jj++)
            {
                dcimg[IDdm1].array.F[jj * dmxsize + ii] *= (*OPDamp) / RMStot;
            }
        }
    ...
    INSERT_STD_PROCINFO_COMPUTEFUNC_START
    {
        ...
        if (framerateOK == 1)
        {
            ...
            *refdtoffset = 0.5 * *wfsdt;
            ...
            FILE *fphwlat = fps_write_RUNoutput_file(milk_data.fpsptr, "hardwlatency", "dat");
            ...
            while (loopOK == 1)
            {
                ...
                processinfo_WriteMessage_fmt(processinfo, "iteration %5u / %5u", iter, *NBiter);
                ...
                while ((dt < dtmax) && (wfsframe < *wfsNBframemax))
                {
                    ...
                    double tdouble =
                        1.0 * tarray[wfsframe].tv_sec + 1.0e-9 * tarray[wfsframe].tv_nsec;
                ...
                if (functionparameter_GetParamValue_ONOFF(milk_data.fpsptr, ".option.saveraw") == 1)
                {
                    // Save each datacube
                    //
                    char ffnameC[STRINGMAXLEN_FULLFILENAME];
                    WRITE_FULLFILENAME(ffnameC, "mlat-testC-%04d", iter);
...
#define IMAGE_SUMMING_CASE(IMG_PTR_ID)                                                  \
    {                                                                                   \
        ...
        {                                                                               \
            double tmp = 1.0 * dcimg[IDwfsc].array.IMG_PTR_ID[kk * wfssize + ii] -      \
            ...
            diffseqvalarray[ii] = tmp;                                                  \
                ...
                for (long kk = 1; kk < NBwfsframe; kk++)
                {
                    ...
                    {
                        float timeoffset =
                            (0.5 * (dtarray[kk] + dtarray[kk - 1]) - *dtoffset) * (*framerateHz);
                        int diffseqkk = timeoffset / diffseqdtframe;
                        if ((diffseqkk >= 0) && (diffseqkk < diffseqsize))
                        {
                            for (uint64_t ii = 0; ii < wfssize; ii++)
                            {
                                imgdiffseq.im->array.F[diffseqkk * wfssize + ii] +=
                                    diffseqvalarray[ii];
                            }
            ...
            // PROCESS RESULTS
            //
            processinfo_WriteMessage_fmt(processinfo, "Processing Data (%u iterations)", (*NBiter));
            ...
            // Save imgdiffseq
            //
            if (functionparameter_GetParamValue_ONOFF(milk_data.fpsptr, ".option.saveseq") == 1)
            {
                char ffnameC[STRINGMAXLEN_FULLFILENAME];
                WRITE_FULLFILENAME(ffnameC, "mlat-diffseq");
                fps_write_RUNoutput_image(milk_data.fpsptr, imgdiffseq.name, ffnameC);
            }
            ...
            *framerateHz = 1.0 * (wfscntend - wfscntstart) / dt;
...

            {
                ...
                {
                    while ((dtarrayall[iimax] < dtarrayall[ii] + dtrange) &&
                           (iimax < NBrawmeas - 1))
                    {
                        iimax++;
                    }
                    while ((dtarrayall[iimin] < dtarrayall[ii] - dtrange) &&
                           (iimax < NBrawmeas - 1))
                    {
                        iimin++;
                    }
                    ...
                    fprintf(fpout, "%d %g %g %d %d %g %g  %g %g %ld\n", ii, dtarrayall[ii],
                            valarrayall[ii], iimin, iimax, dtarrayall[iimin], dtarrayall[iimax],
            ...
            *latencyfr = latencymeaspeakdt * (*framerateHz);
            ...
            {
                FILE *fpout;
                fpout = fps_write_RUNoutput_file(milk_data.fpsptr, "param_hardwlatency", "txt");
