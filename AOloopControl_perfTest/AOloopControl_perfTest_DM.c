    ...
    for(uint32_t kk = 0; kk < NBpts; kk++)
    {
        ...
        for(uint32_t ii = 0; ii < dmxsize; ii++)
            for(uint32_t jj = 0; jj < dmysize; jj++)
            {
                ...
                data.image[ID1].array.F[kk * dmsize + jj * dmxsize + ii] =
                    ampl * x1;
            }
    }
        ...
        const char *IDout_name)
{
    ...
    {
        printf(
            "ERROR: x size of \"%s\"  (%ld) does not match x size of \"%s\" "
            "(%ld)\n",
            DMstream_in_name,
            (long) data.image[IDdmin].md[0].size[0],
            DMmodes_name,
    ...
    {
        printf(
            "ERROR: y size of \"%s\"  (%ld) does not match y size of \"%s\" "
            "(%ld)\n",
            DMstream_in_name,
            (long) data.image[IDdmin].md[0].size[1],
            DMmodes_name,
    ...
    if(data.image[IDdmout].md[0].size[0] != data.image[IDmodes].md[0].size[0])
    {
        printf(
            ...
            "(%ld)\n",
            DMstream_out_name,
            (long) data.image[IDdmout].md[0].size[0],
            DMmodes_name,
            (long) data.image[IDmodes].md[0].size[0]);
        exit(0);
    }
    ...
    if(data.image[IDdmout].md[0].size[1] != data.image[IDmodes].md[0].size[1])
    {
        printf(
            ...
            "(%ld)\n",
            DMstream_out_name,
            (long) data.image[IDdmout].md[0].size[1],
            DMmodes_name,
            (long) data.image[IDmodes].md[0].size[1]);
        exit(0);
    }
    ...
    if(data.image[IDdmmask].md[0].size[0] != data.image[IDmodes].md[0].size[0])
    {
        printf(
            ...
            "(%ld)\n",
            DMmask_name,
            (long) data.image[IDdmmask].md[0].size[0],
            DMmodes_name,
            (long) data.image[IDmodes].md[0].size[0]);
        exit(0);
    }
    ...
    if(data.image[IDdmmask].md[0].size[1] != data.image[IDmodes].md[0].size[1])
    {
        printf(
            ...
            "(%ld)\n",
            DMmask_name,
            (long) data.image[IDdmmask].md[0].size[1],
            DMmodes_name,
            (long) data.image[IDmodes].md[0].size[1]);
        exit(0);
    }
    ...
    for(ii = 0; ii < dmsize; ii++)
    {
        data.image[ID].array.F[ii] =
            data.image[IDmodes].array.F[kk * dmsize + ii];
    }
    ...
    for(f = fmin; f < fmax; f *= fmultstep)
    {
        ...
        while((runtime < avetime) && (k < kmax))
        {
            ...
            printf(
                "mode %4ld  f = %f ( %f -> %f)   runtime = %10.3f sec    "
                "ampl = %f   pha = %f   coeff = %f\n",
                kk,
                f,
                fmin,
                fmax,
            ...
            for(ii = 0; ii < dmsize; ii++)
            {
                data.image[IDdmin].array.F[ii] =
                    coeff * data.image[IDmodes].array.F[kk * dmsize + ii];
            }
            ...
            memcpy(ptr,
                   data.image[IDdmout].array.F,
                   sizeof(float) * dmsize); // out->in
        ...
        for(k = 0; k < k1; k++)
        {
            ...
            // decompose in modes
            linopt_imtools_image_fitModes("_tmpdm",
                                          DMmodes_name,
                                          DMmask_name,
                                          SVDeps,
                                          "dmcoeffs",
        ...
        for(k = k1 / 4; k < k1; k++)
        {
            ...
            coscoeff +=
                cos(pha) * data.image[IDcoeffarray].array.F[kk * kmax + k];
            sincoeff +=
                sin(pha) * data.image[IDcoeffarray].array.F[kk * kmax + k];
        }
...
imageID
AOloopControl_perfTest_TestDMmodes_Recovery(const char *DMmodes_name,
        float       ampl,
        const char *DMmask_name,
        const char *DMstream_in_name,
        const char *DMstream_out_name,
        ...
        const char *IDoutmeasrms_name)
{
    ...
    {
        printf(
            "ERROR: y size of \"%s\"  (%ld) does not match x size of \"%s\" "
            "(%ld)\n",
            DMstream_out_name,
            (long) data.image[IDdmout].md[0].size[0],
            DMmodes_name,
    ...
    if(data.image[IDmeasout].md[0].size[0] !=
            data.image[IDmodes].md[0].size[0])
    {
        printf(
            "ERROR: y size of \"%s\"  (%ld) does not match x size of \"%s\" "
            "(%ld)\n",
            DMstream_meas_name,
            (long) data.image[IDmeasout].md[0].size[0],
            DMmodes_name,
            (long) data.image[IDmodes].md[0].size[0]);
        exit(0);
    }

    if(data.image[IDmeasout].md[0].size[1] !=
            data.image[IDmodes].md[0].size[1])
    {
        printf(
            ...
            "(%ld)\n",
            DMstream_meas_name,
            (long) data.image[IDmeasout].md[0].size[1],
            DMmodes_name,
            (long) data.image[IDmodes].md[0].size[1]);
        exit(0);
    }
    ...
    for(uint32_t kk = 0; kk < NBmodes; kk++)
    {
        ...
        {
            data.image[IDdmin].array.F[ii] =
                ampl * data.image[IDmodes].array.F[kk * dmsize + ii];
        }
        ...
        while(i < NBave)
        {
            ...
            memcpy(data.image[IDdmtmp].array.F,
                   data.image[IDdmout].array.F,
                   sizeof(float) * dmsize);
            memcpy(data.image[IDmeastmp].array.F,
                   data.image[IDmeasout].array.F,
                   sizeof(float) * dmsize);
            ...
            for(uint32_t kk1 = 0; kk1 < NBmodes; kk1++)
            {
                data.image[IDcoeffarray].array.F[kk1 * NBave + i] =
                    0.5 * data.image[IDcoeff].array.F[kk1];
            }
            delete_image_ID("dmcoeffs", DELETE_IMAGE_ERRMODE_WARNING);

            linopt_imtools_image_fitModes("_tmpmeas",
                                          DMmodes_name,
                                          DMmask_name,
                                          SVDeps,
                                          "dmcoeffs",
