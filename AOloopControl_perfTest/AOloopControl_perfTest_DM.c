                                              ...
                                              const char *IDout_name)
{
    ...
    if (data.image[IDdmin].md[0].size[0] != data.image[IDmodes].md[0].size[0])
        {
            printf("ERROR: x size of \"%s\"  (%ld) does not match x size of \"%s\" "
                   "(%ld)\n",
                   DMstream_in_name,
    ...
    if (data.image[IDdmin].md[0].size[1] != data.image[IDmodes].md[0].size[1])
        {
            printf("ERROR: y size of \"%s\"  (%ld) does not match y size of \"%s\" "
                   "(%ld)\n",
                   DMstream_in_name,
            ...
            while ((runtime < avetime) && (k < kmax))
                {
                    ...
                    printf("mode %4ld  f = %f ( %f -> %f)   runtime = %10.3f sec    "
                    ...
                    memcpy(ptr, data.image[IDdmout].array.F,
            ...
            for (k = 0; k < k1; k++)
                {
                    ...
                    linopt_imtools_image_fitModes(
                        "_tmpdm", DMmodes_name, DMmask_name, SVDeps, "dmcoeffs", SVDreuse, NULL);
                    SVDreuse = 1;
                                                    ...
                                                    const char *IDoutmeasrms_name)
{
    ...
    if (data.image[IDdmout].md[0].size[0] != data.image[IDmodes].md[0].size[0])
        {
            printf("ERROR: y size of \"%s\"  (%ld) does not match x size of \"%s\" "
                   "(%ld)\n",
                   DMstream_out_name,
                   (long)data.image[IDdmout].md[0].size[0],
                   DMmodes_name,
            ...
            while (i < NBave)
                {
                    ...
                    linopt_imtools_image_fitModes(
                        "_tmpmeas", DMmodes_name, DMmask_name, SVDeps, "dmcoeffs", SVDreuse, NULL);
                    IDcoeff = image_ID("dmcoeffs");
                    for (uint32_t kk1 = 0; kk1 < NBmodes; kk1++)
                        {
