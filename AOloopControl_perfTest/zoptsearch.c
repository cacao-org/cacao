...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Search for optimal zonal control parameters by systematically varying "
                        "gains and analyzing closed-loop residuals."
};

#define FPS_PARAMS(X)                                                                           \
    X(".ctrlsname", &ctrlsname, FPTYPE_STREAMNAME, 1,                                           \
      FPFLAG_DEFAULT_INPUT | FPFLAG_STREAM_RUN_REQUIRED | FPFLAG_CHECKSTREAM, "control stream") \
    ...
    X(".ctrlampmap", &ctrlampmap, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,                   \
      "control stream amplitude map")                                                           \
    X(".senssname", &senssname, FPTYPE_STREAMNAME, 1,                                           \
      FPFLAG_DEFAULT_INPUT | FPFLAG_STREAM_RUN_REQUIRED | FPFLAG_CHECKSTREAM, "sensing stream") \
    X(".sensprocout", &sensproc, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,                    \
      "processed image output")                                                                 \
    X(".sproc.sensref0", &sensref0, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,                 \
      "sensing reference 0")                                                                    \
    X(".sproc.sensmask0", &sensmask0, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,               \
      "sensing mask 0 (float)")                                                                 \
    X(".sproc.sensnorm0", &sensnorm0, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT | FPFLAG_WRITERUN,  \
      "normalization 0 on/off")                                                                 \
    X(".sproc.sensref1", &sensref1, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,                 \
      "sensing reference 1")                                                                    \
    X(".sproc.sensmask1", &sensmask1, FPTYPE_STREAMNAME, 0, FPFLAG_DEFAULT_INPUT,               \
      "sensing mask 1 (float)")                                                                 \
    X(".sproc.sensnorm1", &sensnorm1, FPTYPE_ONOFF, 0, FPFLAG_DEFAULT_INPUT | FPFLAG_WRITERUN,  \
      "normalization 1 on/off")                                                                 \
    X(".optm.opttype", &opttype, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT,                        \
      "1:min, 2:max, 3:absmin, 4:absmax")                                                       \
    X(".optm.optcomp", &optcomp, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT,                        \
      "1:tot, 2:norma, 3:tota, 4:percr")                                                        \
    X(".optm.optparam0", &optparam0, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT | FPFLAG_WRITERUN, \
      "optimization parameter 0")                                                               \
    X(".optm.optparam1", &optparam1, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT | FPFLAG_WRITERUN, \
      "optimization parameter 1")                                                               \
    X(".twaitsec", &twaitframe, FPTYPE_FLOAT32, 0, FPFLAG_DEFAULT_INPUT | FPFLAG_WRITERUN,      \
      "time to wait after poke before measurement")                                             \
    X(".tintframe", &tintframe, FPTYPE_UINT32, 0, FPFLAG_DEFAULT_INPUT | FPFLAG_WRITERUN,       \
      "number of frames to integrate per measurement")
// Returns optimization metric value
//
static double image_optvalue(IMGID                   imgin,
...
static CLICMDDATA CLIcmddata = { "zoptsearch", "stream zonal control optimize search",
...
static errno_t compute_function()
{
    ...
    resolveIMGID(&imgctrl, ERRMODE_WARN, dcimg, dcnimg);
    if (imgctrl.ID == -1)
    {
    ...
    if ((ctrlampmap != NULL) && (strcmp(ctrlampmap, "null") != 0))
    {
        ...
        resolveIMGID(&imgctrlamp, ERRMODE_WARN, dcimg, dcnimg);
        if (imgctrlamp.ID == -1)
        {
            return RETURN_FAILURE;
        }
    ...
    resolveIMGID(&imgsens, ERRMODE_WARN, dcimg, dcnimg);
    if (imgsens.ID == -1)
    {
        return RETURN_FAILURE;
    }
    ...
    if (strcmp(sensref0, "null"))
    {
        ...
        resolveIMGID(&imgsensref0, ERRMODE_WARN, dcimg, dcnimg);
        if (imgsensref0.ID == -1)
        {
            return RETURN_FAILURE;
        }
    ...
    if (strcmp(sensmask0, "null"))
    {
        ...
        resolveIMGID(&imgsensmask0, ERRMODE_WARN, dcimg, dcnimg);
        if (imgsensmask0.ID == -1)
        {
            return RETURN_FAILURE;
        }
    }
    ...
    if (strcmp(sensref1, "null"))
    {
        ...
        resolveIMGID(&imgsensref1, ERRMODE_WARN, dcimg, dcnimg);
        if (imgsensref1.ID == -1)
        {
            return RETURN_FAILURE;
        }
    ...
    if (strcmp(sensmask1, "null"))
    {
        ...
        resolveIMGID(&imgsensmask1, ERRMODE_WARN, dcimg, dcnimg);
        if (imgsensmask1.ID == -1)
        {
            return RETURN_FAILURE;
        }
    }
    ...
    INSERT_STD_PROCINFO_COMPUTEFUNC_START
    {
        ...
        if (framestep == 0)
        {
            ...
            //
            {
                ...
                else
                {
                    for (uint32_t ii = 0; ii < ctrlxysize; ii++)
                    {
                        imgctrl.im->array.F[ii] =
                            (1.0 - 2.0 * ran1()) * imgctrlamp.im->array.F[ii] * (*ctrlamp);
                    }
                }
                ...
                if (pokeindex < *nbpoke)
                {
                    ...
                    memcpy(ptrdest, // + sizeof(float)*ctrlxysize*pokeindex,
                           imgctrl.im->array.F, sizeof(float) * ctrlxysize);
                }
            }
            framecollected = 0;
        ...
        if (framecollected == *tintframe)
        {
            ...
            if (pokeindex < *nbpoke)
            {
                ...
                memcpy(ptrdest, // + sizeof(float)*ctrlxysize*pokeindex,
                       imgacc.im->array.F, sizeof(float) * sensxysize);
            }
            ...
            double optval = image_optvalue(imgacc, optm, imgsensref0, imgsensmask0, imgsensref1,
                                           imgsensmask1, imgsensproc);
