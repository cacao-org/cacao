...
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 1) == 0)
    {
        AOcontrolLoop_perfTest_TestDMSpeed(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numl,
...
errno_t AOcontrolLoop_perfTest_TestSystemLatency_cli()
{
    ...
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 1) + CLI_checkarg(4, 2) == 0)
    {
        AOcontrolLoop_perfTest_TestSystemLatency(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 2) + CLI_checkarg(3, 1) + CLI_checkarg(4, 1) + CLI_checkarg(5, 1) +
            CLI_checkarg(6, 1) + CLI_checkarg(7, 1) + CLI_checkarg(8, 2) + CLI_checkarg(9, 4) + CLI_checkarg(10, 4) +
            CLI_checkarg(11, 4) + CLI_checkarg(12, 3) ==
        0)
    {
        AOloopControl_perfTest_TestDMmodeResp(
            ...
            data.cmdargtoken[7].val.numf, data.cmdargtoken[8].val.numl, data.cmdargtoken[9].val.string,
            data.cmdargtoken[10].val.string, data.cmdargtoken[11].val.string, data.cmdargtoken[12].val.string);
...
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 1) + CLI_checkarg(3, 4) + CLI_checkarg(4, 4) + CLI_checkarg(5, 4) +
            CLI_checkarg(6, 4) + CLI_checkarg(7, 1) + CLI_checkarg(8, 2) + CLI_checkarg(9, 3) + CLI_checkarg(10, 3) +
            CLI_checkarg(11, 3) + CLI_checkarg(12, 3) ==
        0)
    {
        AOloopControl_perfTest_TestDMmodes_Recovery(
            data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numf, data.cmdargtoken[3].val.string,
            data.cmdargtoken[4].val.string, data.cmdargtoken[5].val.string, data.cmdargtoken[6].val.string,
            data.cmdargtoken[7].val.numf, data.cmdargtoken[8].val.numl, data.cmdargtoken[9].val.string,
            data.cmdargtoken[10].val.string, data.cmdargtoken[11].val.string, data.cmdargtoken[12].val.string);
...
{
    if (0 + CLI_checkarg(1, 3) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 2) == 0)
    {
        AOloopControl_perfTest_mkTestDynamicModeSeq(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numl,
...
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 4) + CLI_checkarg(4, 4) + CLI_checkarg(4, 4) +
            CLI_checkarg(5, 4) + CLI_checkarg(6, 1) + CLI_checkarg(7, 1) + CLI_checkarg(8, 3) ==
        0)
    {
        AOloopControl_perfTest_AnalyzeRM_sensitivity(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
errno_t AOloopControl_LoopTimer_Analysis_cli()
{
    ...
    {
        AOloopControl_LoopTimer_Analysis(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
{
    if (0 + CLI_checkarg(1, 5) + CLI_checkarg(2, 5) + CLI_checkarg(3, 5) + CLI_checkarg(4, 1) + CLI_checkarg(5, 1) +
            CLI_checkarg(6, 1) + CLI_checkarg(7, 1) ==
        0)
    {
        AOloopControl_perfTest_mkSyncStreamFiles2(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
                                                  data.cmdargtoken[3].val.string, data.cmdargtoken[4].val.numf,
...
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 4) + CLI_checkarg(4, 4) + CLI_checkarg(5, 2) +
            CLI_checkarg(6, 2) ==
        0)
    {
        AOloopControl_perfTest_StatAnalysis_2streams(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
                                                     data.cmdargtoken[3].val.string, data.cmdargtoken[4].val.string,
...
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 1) + CLI_checkarg(4, 2) + CLI_checkarg(5, 2) +
            CLI_checkarg(6, 2) + CLI_checkarg(7, 2) + CLI_checkarg(8, 2) + CLI_checkarg(9, 1) ==
        0)
    {
        AOloopControl_perfTest_SelectWFSframes_from_PSFframes(
            data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string, data.cmdargtoken[3].val.numf,
            data.cmdargtoken[4].val.numl, data.cmdargtoken[5].val.numl, data.cmdargtoken[6].val.numl,
            data.cmdargtoken[7].val.numl, data.cmdargtoken[8].val.numl, data.cmdargtoken[9].val.numf);
...
static errno_t init_module_CLI()
{
    ...
    RegisterCLIcommand("aoldmtestsp", __FILE__, AOcontrolLoop_perfTest_TestDMSpeed_cli,
                       "test DM speed by sending circular tip-tilt", "<dmname> <delay us [long]> <NB pts> <ampl>",
                       "aoldmtestsp dmdisp2 100 20 0.1",
                       "long AOcontrolLoop_perfTest_TestDMSpeed(char *dmname, long delayus, long NBpts, float ampl)");

    RegisterCLIcommand(
        "aoltestlat", __FILE__, AOcontrolLoop_perfTest_TestSystemLatency_cli, "test system latency",
        "<dm stream> <wfs stream> <ampl [um]> <NBiter>", "aoltestlat dmC wfsim 0.1 5000",
    ...
    RegisterCLIcommand("aoltestmresp", __FILE__, AOloopControl_perfTest_TestDMmodeResp_cli,
                       ...
                       "<DM modes [3D im]> <mode #> <ampl [um]> <fmin [Hz]> <fmax [Hz]> <fstep> <meas. time [sec]> "
                       "<time step [us]> <DM mask> <DM in [2D stream]> <DM out [2D stream]>  <output [2D im]>",
                       "aoltestmresp DMmodesC 5 0.05 10.0 100.0 1.2 1.0 1000 dmmask dmdisp3 dmC out",
                       "long AOloopControl_perfTest_TestDMmodeResp(char *DMmodes_name, long index, float ampl, float "
                       "fmin, float fmax, float fmultstep, float avetime, long dtus, char *DMmask_name, char "
                       "*DMstream_in_name, char *DMstream_out_name, char *IDout_name)");

    RegisterCLIcommand(
        "aoltestdmrec", __FILE__, AOloopControl_perfTest_TestDMmodes_Recovery_cli, "Test system DM modes recovery",
        "<DM modes [3D im]> <ampl [um]> <DM mask [2D im]> <DM in [2D stream]> <DM out [2D stream]> <meas out [2D "
        "stream]> <lag time [us]>  <NB averages [long]>  <out ave [2D im]> <out rms [2D im]> <out meas ave [2D im]> "
        "<out meas rms [2D im]>",
        "aoltestdmrec DMmodesC 0.05 DMmask dmsisp2 dmoutr 2000  20 outave outrms outmave outmrms",
        "long AOloopControl_perfTest_TestDMmodes_Recovery(char *DMmodes_name, float ampl, char *DMmask_name, char "
        "*DMstream_in_name, char *DMstream_out_name, char *DMstream_meas_name, long tlagus, long NBave, char "
        "*IDout_name, char *IDoutrms_name, char *IDoutmeas_name, char *IDoutmeasrms_name)");

    RegisterCLIcommand("aolresetrms", __FILE__, AOloopControl_perfTest_resetRMSperf, "reset RMS performance monitor",
                       "no arg", "aolresetrms", "int AOloopControl_perfTest_resetRMSperf()");

    RegisterCLIcommand("aolinjectmode", __FILE__, AOloopControl_perfTest_InjectMode_cli,
                       "inject single mode error into RM channel", "<index> <ampl>", "aolinjectmode 20 0.1",
                       "int AOloopControl_perfTest_InjectMode()");

    RegisterCLIcommand("aolstatusstats", __FILE__, AOloopControl_perfTest_statusStats_cli,
                       "measures distribution of status values", "<update flag [int]> <NBsample [long]>",
                       "aolstatusstats 0 100000",
                       "int AOloopControl_perfTest_statusStats(int updateconf, long NBsample)");

    RegisterCLIcommand("aolmon", __FILE__, AOloopControl_perfTest_loopMonitor_cli, "monitor loop", "<frequ> <Nbcols>",
                       "aolmon 10.0 3", "int AOloopControl_perfTest_loopMonitor(long loop, double frequ)");

    RegisterCLIcommand("aolblockstats", __FILE__, AOloopControl_perfTest_blockstats_cli,
                       "measures mode stats per block", "<loopnb> <outim>", "aolblockstats 2 outstats",
    ...
    RegisterCLIcommand("aolmktestmseq", __FILE__, AOloopControl_perfTest_mkTestDynamicModeSeq_cli,
                       ...
                       "<outname> <number of slices> <number of modes> <firstmode>", "aolmktestmseq outmc 100 50 0",
                       "long AOloopControl_perfTest_mkTestDynamicModeSeq(const char *IDname_out, long NBpt, long "
                       "NBmodes, long StartMode)");

    RegisterCLIcommand("aolzrmsens", __FILE__, AOloopControl_perfTest_AnalyzeRM_sensitivity_cli,
                       ...
                       "long AOloopControl_perfTest_AnalyzeRM_sensitivity(const char *IDdmmodes_name, const char "
                       "*IDdmmask_name, const char *IDwfsref_name, const char *IDwfsresp_name, const char "
                       "*IDwfsmask_name, float amplimitnm, float lambdanm, const char *foutname)");

    RegisterCLIcommand("aoltimingstat", __FILE__, AOloopControl_LoopTimer_Analysis_cli, "Analysis of loop timing data",
    ...
    RegisterCLIcommand(
        "aolptmksyncs2", __FILE__, AOloopControl_perfTest_mkSyncStreamFiles2_cli,
        ...
        "int AOloopControl_perfTest_mkSyncStreamFiles2(char *datadir, char *stream0, char *stream1, double tstart, "
        "double tend, double dt, double dtlag)");

    RegisterCLIcommand("aolperfcompsimM", __FILE__, AOloopControl_perfTest_ComputeSimilarityMatrix_cli,
                       "compute similarity matrix from image sequence", "<input cube> <output matrix>",
                       "aolperfcompsimM imc outM",
                       "int AOloopControl_perfTest_ComputeSimilarityMatrix(char *IDname, char *IDname_out)");

    RegisterCLIcommand(
        "aolperfsimpairs", __FILE__, AOloopControl_perfTest_StatAnalysis_2streams_cli,
        ...
        "int AOloopControl_perfTest_StatAnalysis_2streams(char *IDname_stream0, char *IDname_stream1, char "
        "*IDname_simM0, char *IDname_simM1, long dtmin, long NBselected)");

    RegisterCLIcommand("aolperfselwfsfpsf", __FILE__, AOloopControl_perfTest_SelectWFSframes_from_PSFframes_cli,
                       ...
                       "int AOloopControl_perfTest_SelectWFSframes_from_PSFframes(char *IDnameWFS, char *IDnamePSF, "
                       "float frac, long x0, long x1, long y0, long y1, int EvalMode, float alpha)");
...
errno_t AOcontrolLoop_perfTest_TestSystemLatency_FPCONF()
{
    ...
    function_parameter_add_entry(&fps, ".frameratewait", "time period for frame rate measurement", FPTYPE_FLOAT64,
                                 FPFLAG_DEFAULT_INPUT, &frameratewait_default, NULL);

    double OPDamp_default[4] = {0.1, 0.1, 1.0, 0.1};
    // long fpi_OPDamp =
    function_parameter_add_entry(&fps, ".OPDamp", "poke amplitude [um]", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT,
                                 &OPDamp_default, NULL);

    long NBiter_default[4] = {100, 10, 100000, 100};
    // long fpi_NBiter =
    function_parameter_add_entry(&fps, ".NBiter", "Number of iteration", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT,
                                 &NBiter_default, NULL);
    ...
    function_parameter_add_entry(&fps, ".sn_dm", "DM stream name", FPTYPE_STREAMNAME, FPFLAG, NULL, NULL);

    // long fp_streamname_wfs       =
    function_parameter_add_entry(&fps, ".sn_wfs", "WFS stream name", FPTYPE_STREAMNAME, FPFLAG, NULL, NULL);

    long wfsNBframesmax_default[4] = {50, 10, 100000, 50};

    long fpi_status_wfsNBframesmax = 0;
    function_parameter_add_entry(&fps, ".status.wfsNBframemax", "Number frames in measurement sequence", FPTYPE_INT64,
                                 FPFLAG_DEFAULT_INPUT, &wfsNBframesmax_default, &fpi_status_wfsNBframesmax);

    // status
    long fpi_wfsdt = 0;
    function_parameter_add_entry(&fps, ".status.wfsdt", "WFS frame interval", FPTYPE_FLOAT64, FPFLAG_DEFAULT_OUTPUT,
                                 NULL, &fpi_wfsdt);

    // long fpi_twaitus      =
    function_parameter_add_entry(&fps, ".status.twaitus", "initial wait [us]", FPTYPE_INT64, FPFLAG_DEFAULT_OUTPUT,
                                 NULL, NULL);

    // long fpi_refdtoffset  =
    function_parameter_add_entry(&fps, ".status.refdtoffset", "baseline time offset to poke", FPTYPE_FLOAT64,
                                 FPFLAG_DEFAULT_OUTPUT, NULL, NULL);

    // long fpi_dtoffset     =
    function_parameter_add_entry(&fps, ".status.dtoffset", "actual time offset to poke", FPTYPE_FLOAT64,
                                 FPFLAG_DEFAULT_OUTPUT, NULL, NULL);

    // output
    // long fpi_outframerateHz =
    function_parameter_add_entry(&fps, ".out.framerateHz", "WFS frame rate [Hz]", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT,
                                 NULL, NULL);

    // long fpi_outHardLatencyfr =
    function_parameter_add_entry(&fps, ".out.latencyfr", "hardware latency [frame]", FPTYPE_FLOAT64,
...
errno_t AOcontrolLoop_perfTest_TestSystemLatency_RUN()
{
    ...
    strncpy(outdirname, functionparameter_GetParamPtr_STRING(&fps, ".conf.outdir"), FUNCTION_PARAMETER_STRMAXLEN);
    ...
    while (loopOK == 1)
    {
        ...
        for (wfsframe = 1; wfsframe < NBwfsframe; wfsframe++)
        {
            fprintf(fphwlat, "%ld   %10.2f     %g\n", wfsframe - kkoffset,
                    1.0e6 * (0.5 * (dtarray[wfsframe] + dtarray[wfsframe - 1]) - *dtoffset), valarray[wfsframe]);
        }
    ...
    *latencyfr = latencystepave;
    ...
    snprintf(msgstring, stringmaxlen, "Measured %8.3f ms @ %.3f Hz", latencyave * 1000.0,
...
imageID AOloopControl_perfTest_blockstats(long loop, const char *IDout_name)
{
    ...
    create_image_ID(fname, 2, sizeout, _DATATYPE_FLOAT, 1, 0, 0, &IDblockRMS_ave);
    COREMOD_MEMORY_image_set_createsem(fname, 10);
    ...
    for (;;)
    {
        ...
        for (blk = 0; blk < NBblock; blk++)
        {
            data.image[IDblockRMS_ave].array.F[blk] =
                (1.0 - alpha) * data.image[IDblockRMS_ave].array.F[blk] + alpha * rmsarray[blk];
        }
...
errno_t AOloopControl_perfTest_InjectMode(long index, float ampl)
{
    ...
    else
    {
        ...
        for (unsigned int i = 0; i < AOconf[LOOPNUMBER].DMctrl.sizeDM; i++)
        {
            arrayf[i] =
                ampl *
                data.image[aoloopcontrol_var.aoconfID_DMmodes].array.F[index * AOconf[LOOPNUMBER].DMctrl.sizeDM + i];
        }
...
//
errno_t AOloopControl_perfTest_AnalyzeRM_sensitivity(const char *IDdmmodes_name, const char *IDdmmask_name,
                                                     const char *IDwfsref_name, const char *IDwfsresp_name,
                                                     const char *IDwfsmask_name, float amplimitnm, float lambdanm,
                                                     const char *foutname)
{
    FILE *fp;
    long IDdmmodes;
        ...
        for (ii = 0; ii < wfsxysize; ii++)
        {
            ...
            if (data.image[IDwfsmask].array.F[ii] > 0.1)
                if (data.image[IDwfsref].array.F[ii] >
                    fabs(data.image[IDwfsresp].array.F[mode * wfsxysize + ii] * amplimitnm * 0.001))
                {
                    SNR1 =
                        data.image[IDwfsresp].array.F[mode * wfsxysize + ii] / sqrt(data.image[IDwfsref].array.F[ii]);
                    SNR1 /= wfsreftot;
                    SNR += SNR1 * SNR1;
    ...
    for (mode = 0; mode < NBmodes; mode++)
        for (mode1 = 0; mode1 < mode + 1; mode1++)
        {
            ...
            for (ii = 0; ii < dmxysize; ii++)
            {
                XPval += data.image[IDdmmask].array.F[ii] * data.image[IDdmmodes].array.F[mode * dmxysize + ii] *
        ...
        for (m = 0; m < NBmodes; m++)
        {
            ...
            for (ii = 0; ii < xysize; ii++)
            {
                data.image[IDout].array.F[kk * xysize + ii] +=
                    ampl * data.image[aoloopcontrol_var.aoconfID_DMmodes].array.F[m1 * xysize + ii];
            }
        }
    }
...
errno_t AOloopControl_LoopTimer_Analysis(char *IDname, char *fnametxt, char *outfname)
{
    ...
    for (timer = 0; timer < NBtimer; timer++)
    {
        printf("|  %3d  | %7.2f - %7.2f - %7.2f | %7.2f |\n", timer, timer_min[timer] * 1e6, timer_ave[timer] * 1e6,
               timer_max[timer] * 1e6, timer_dev[timer] * 1e6);
    }
    printf("-------------------------------------------------\n");
...
errno_t AOloopControl_perfTest_mkTimingFile(char *inTimingfname, char *outTimingfname, char *tmpstring)
{
    ...
    {
        fprintf(fpout, "%s   %20.9f %20.9f   %10ld  %10.3f\n", tmpstring, datfile.tstart, datfile.tend, datfile.cnt,
 ...
 *
 */
errno_t AOloopControl_perfTest_mkSyncStreamFiles2(char *datadir, char *stream0, char *stream1, double tstart,
                                                  double tend, double dt, double dtlag)
{
    int stringmaxlen = 500;
    ...
    for (stream = 0; stream < NBstream; stream++)
    {
                ...
                else
                {
                    ...
                    if (datfileOK != 0)
                    {
                        ...
                        while (scanOK == 1)
                        {
                            ...
                            if (line[0] != '#')
                            {
                                ret = sscanf(line, "%s   %lf %lf   %ld  %f\n", tmpstring, &datfile[NBdatFiles].tstart,
                                             &datfile[NBdatFiles].tend, &datfile[NBdatFiles].cnt, &tmpv);
        ...
        for (i = 0; i < NBdatFiles; i++)
        {
            printf("FILE [%ld]: %20s       %20.9f -> %20.9f   [%10ld]  %10.3f Hz\n", i, datfile[i].name,
                   datfile[i].tstart, datfile[i].tend, datfile[i].cnt,
        ...
        for (i = 0; i < NBdatFiles; i++)
        {
            printf("FILE: %20s       %20.9f -> %20.9f   [%10ld]  %10.3f Hz\n", datfile[i].name, datfile[i].tstart,
                   datfile[i].tend, datfile[i].cnt, datfile[i].cnt / (datfile[i].tend - datfile[i].tstart));
            ...
            for (tstep = 0; tstep < zsize; tstep++)
            {
                ...
                while ((intarray_start[j] < (tendarray[tstep] + dtoffset)) && (j < datfile[i].cnt))
                {
                    ...
                    case _DATATYPE_UINT8:
                        for (ii = 0; ii < xysize; ii++)
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.UI8[xysize * j + ii];
                        }
                        break;
                    ...
                    case _DATATYPE_INT8:
                        ...
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.SI8[xysize * j + ii];
                        }
                        break;
                    ...
                    case _DATATYPE_UINT16:
                        ...
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.UI16[xysize * j + ii];
                        }
                        break;
                    ...
                    case _DATATYPE_INT16:
                        ...
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.SI16[xysize * j + ii];
                        }
                        break;
                    ...
                    case _DATATYPE_UINT32:
                        ...
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.UI32[xysize * j + ii];
                        }
                        break;
                    ...
                    case _DATATYPE_INT32:
                        ...
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.SI32[xysize * j + ii];
                        }
                        break;
                    ...
                    case _DATATYPE_UINT64:
                        ...
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.UI64[xysize * j + ii];
                        }
                        break;
                    ...
                    case _DATATYPE_INT64:
                        ...
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.SI64[xysize * j + ii];
                        }
                        break;
                    ...
                    case _DATATYPE_FLOAT:
                        ...
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.F[xysize * j + ii];
                        }
                        break;
                    ...
                    case _DATATYPE_DOUBLE:
                        ...
                        {
                            data.image[IDout].array.F[xysize * tstep + ii] +=
                                expfrac * data.image[IDc].array.D[xysize * j + ii];
                        }
                        break;
    ...
    for (tstep = 0; tstep < zsize; tstep++)
    {
        fprintf(fp, "%5ld %5ld %d %10.6f %10.6f %10.6f\n", tstep, NBframeOK, frameOKarray[tstep], tstartarray[tstep],
                exparray0[tstep], exparray1[tstep]);
...
errno_t AOloopControl_perfTest_ComputeSimilarityMatrix(char *IDname, char *IDname_out)
{
    ...
    for (k1 = 0; k1 < zsize; k1++)
    {
        ...
        for (k2 = 0; k2 < k1; k2++)
        {
            ...
            {
                printf("\r [%5.2f %%]   %5ld / %5ld    %5ld / %5ld     ", 100.0 * cnt / (zsize * (zsize - 1) / 2), k1,
                       (long)zsize, k2, (long)zsize);
                fflush(stdout);
...
errno_t AOloopControl_perfTest_StatAnalysis_2streams(char *IDname_stream0, char *IDname_stream1, char *IDname_simM0,
                                                     char *IDname_simM1, long dtmin, unsigned long NBselected)
{
    imageID IDstream0;
    imageID IDstream1;
    imageID IDsimM0;
    ...
    printf("NBpairMax = %llu x %llu =  %llu\n", (unsigned long long)NBframe0, (unsigned long long)(NBframe0 - 1) / 2,
           NBpairMax);
    ...
    if (NBframe0 != data.image[IDsimM0].md[0].size[0])
    {
        printf("[%s] [%s] [%d]  ERROR: NBframe0 (%ld) != simM0 xsize (%ld)\n", __FILE__, __FUNCTION__, __LINE__,
               NBframe0, (long)data.image[IDsimM0].md[0].size[0]);
        exit(0);
    }
    ...
    if (NBframe0 != data.image[IDsimM0].md[0].size[1])
    {
        printf("[%s] [%s] [%d]  ERROR: NBframe0 (%ld) != simM0 ysize (%ld)\n", __FILE__, __FUNCTION__, __LINE__,
               NBframe0, (long)data.image[IDsimM0].md[0].size[1]);
        exit(0);
    }
    ...
    if (NBframe1 != data.image[IDsimM1].md[0].size[0])
    {
        printf("[%s] [%s] [%d]  ERROR: NBframe1 (%ld) != simM1 xsize (%ld)\n", __FILE__, __FUNCTION__, __LINE__,
               NBframe1, (long)data.image[IDsimM1].md[0].size[0]);
        exit(0);
    }
    ...
    if (NBframe1 != data.image[IDsimM1].md[0].size[1])
    {
        printf("[%s] [%s] [%d]  ERROR: NBframe1 (%ld) != simM1 ysize (%ld)\n", __FILE__, __FUNCTION__, __LINE__,
               NBframe1, (long)data.image[IDsimM1].md[0].size[1]);
        exit(0);
    }
    ...
    create_2Dimage_ID("sim2Ddistrib", xsize2Ddistrib, ysize2Ddistrib, &IDsim2Ddistrib);
    ...
    imageID IDsim0pair0;
    create_3Dimage_ID("sim0pair0", xsize0 * 3, ysize0, NBselected, &IDsim0pair0);
    imageID IDsim0pair1;
    create_3Dimage_ID("sim0pair1", xsize1 * 3, ysize1, NBselected, &IDsim0pair1);
    ...
    for (pair = 0; pair < NBselected; pair++)
    {
        ...
        for (ii = 0; ii < xysize0; ii++)
        {
            data.image[IDsim0diff0].array.F[pair * xysize0 + ii] =
                data.image[IDstream0].array.F[k1 * xysize0 + ii] - data.image[IDstream0].array.F[k2 * xysize0 + ii];
        }
        for (ii = 0; ii < xysize1; ii++)
        {
            data.image[IDsim0diff1].array.F[pair * xysize1 + ii] =
                data.image[IDstream1].array.F[k1 * xysize1 + ii] - data.image[IDstream1].array.F[k2 * xysize1 + ii];
        }

        for (ii = 0; ii < xsize0; ii++)
            for (jj = 0; jj < ysize0; jj++)
            {
                data.image[IDsim0pair0].array.F[pair * ysize0 * xsize0 * 3 + jj * xsize0 * 3 + ii] =
                    data.image[IDstream0].array.F[k1 * xysize0 + jj * xsize0 + ii];
                data.image[IDsim0pair0].array.F[pair * ysize0 * xsize0 * 3 + jj * xsize0 * 3 + ii + xsize0] =
                    data.image[IDstream0].array.F[k2 * xysize0 + jj * xsize0 + ii];
                data.image[IDsim0pair0].array.F[pair * ysize0 * xsize0 * 3 + jj * xsize0 * 3 + ii + xsize0 * 2] =
                    data.image[IDstream0].array.F[k1 * xysize0 + jj * xsize0 + ii] -
                    data.image[IDstream0].array.F[k2 * xysize0 + jj * xsize0 + ii];
            }
        ...
        for (ii = 0; ii < xsize1; ii++)
            ...
            {
                data.image[IDsim0pair1].array.F[pair * ysize1 * xsize1 * 3 + jj * xsize1 * 3 + ii] =
                    data.image[IDstream1].array.F[k1 * xysize1 + jj * xsize1 + ii];
                data.image[IDsim0pair1].array.F[pair * ysize1 * xsize1 * 3 + jj * xsize1 * 3 + ii + xsize1] =
                    data.image[IDstream1].array.F[k2 * xysize1 + jj * xsize1 + ii];
                data.image[IDsim0pair1].array.F[pair * ysize1 * xsize1 * 3 + jj * xsize1 * 3 + ii + xsize1 * 2] =
                    data.image[IDstream1].array.F[k1 * xysize1 + jj * xsize1 + ii] -
                    data.image[IDstream1].array.F[k2 * xysize1 + jj * xsize1 + ii];
            }
    }
    ...
    imageID IDsim1pair0;
    create_3Dimage_ID("sim1pair0", xsize0 * 3, ysize0, NBselected, &IDsim1pair0);
    imageID IDsim1pair1;
    create_3Dimage_ID("sim1pair1", xsize1 * 3, ysize1, NBselected, &IDsim1pair1);
    ...
    for (pair = 0; pair < NBselected; pair++)
    {
        ...
        for (ii = 0; ii < xsize0; ii++)
            for (jj = 0; jj < ysize0; jj++)
            {
                data.image[IDsim1pair0].array.F[pair * ysize0 * xsize0 * 3 + jj * xsize0 * 3 + ii] =
                    data.image[IDstream0].array.F[k1 * xysize0 + jj * xsize0 + ii];
                data.image[IDsim1pair0].array.F[pair * ysize0 * xsize0 * 3 + jj * xsize0 * 3 + ii + xsize0] =
                    data.image[IDstream0].array.F[k2 * xysize0 + jj * xsize0 + ii];
                data.image[IDsim1pair0].array.F[pair * ysize0 * xsize0 * 3 + jj * xsize0 * 3 + ii + xsize0 * 2] =
                    data.image[IDstream0].array.F[k1 * xysize0 + jj * xsize0 + ii] -
                    data.image[IDstream0].array.F[k2 * xysize0 + jj * xsize0 + ii];
            }
        ...
        for (ii = 0; ii < xsize1; ii++)
            for (jj = 0; jj < ysize1; jj++)
            {
                data.image[IDsim1pair1].array.F[pair * ysize1 * xsize1 * 3 + jj * xsize1 * 3 + ii] =
                    data.image[IDstream1].array.F[k1 * xysize1 + jj * xsize1 + ii];
                data.image[IDsim1pair1].array.F[pair * ysize1 * xsize1 * 3 + jj * xsize1 * 3 + ii + xsize1] =
                    data.image[IDstream1].array.F[k2 * xysize1 + jj * xsize1 + ii];
                data.image[IDsim1pair1].array.F[pair * ysize1 * xsize1 * 3 + jj * xsize1 * 3 + ii + xsize1 * 2] =
                    data.image[IDstream1].array.F[k1 * xysize1 + jj * xsize1 + ii] -
                    data.image[IDstream1].array.F[k2 * xysize1 + jj * xsize1 + ii];
            }
    }
...
errno_t AOloopControl_perfTest_SelectWFSframes_from_PSFframes(char *IDnameWFS, char *IDnamePSF, float frac, long x0,
                                                              long x1, long y0, long y1, int EvalMode, float alpha)
{
    imageID IDwfs;
    imageID IDpsf;
