...
errno_t AOloopControl_perfTest_TestDMmodes_Recovery_cli()
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 1) + CLI_checkarg(3, 4) + CLI_checkarg(4, 4) + CLI_checkarg(5, 4) +
        ...
        0)
        {
            AOloopControl_perfTest_TestDMmodes_Recovery(data.cmdargtoken[1].val.string,
...
errno_t AOloopControl_perfTest_mkTestDynamicModeSeq_cli()
{
    if (0 + CLI_checkarg(1, 3) + CLI_checkarg(2, 2) + CLI_checkarg(3, 2) + CLI_checkarg(4, 2) == 0)
        {
            AOloopControl_perfTest_mkTestDynamicModeSeq(data.cmdargtoken[1].val.string,
                                                        data.cmdargtoken[2].val.numl,
                                                        data.cmdargtoken[3].val.numl,
                                                        data.cmdargtoken[4].val.numl);
...
errno_t AOloopControl_perfTest_AnalyzeRM_sensitivity_cli()
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 4) + CLI_checkarg(4, 4) + CLI_checkarg(4, 4) +
        ...
        0)
        {
            AOloopControl_perfTest_AnalyzeRM_sensitivity(data.cmdargtoken[1].val.string,
                                                         data.cmdargtoken[2].val.string,
                                                         data.cmdargtoken[3].val.string,
                                                         data.cmdargtoken[4].val.string,
                                                         data.cmdargtoken[5].val.string,
...
errno_t AOloopControl_perfTest_mkSyncStreamFiles2_cli()
{
    if (0 + CLI_checkarg(1, 5) + CLI_checkarg(2, 5) + CLI_checkarg(3, 5) + CLI_checkarg(4, 1) + CLI_checkarg(5, 1) +
        ...
        0)
        {
            AOloopControl_perfTest_mkSyncStreamFiles2(data.cmdargtoken[1].val.string,
                                                      data.cmdargtoken[2].val.string,
                                                      data.cmdargtoken[3].val.string,
                                                      data.cmdargtoken[4].val.numf,
                                                      data.cmdargtoken[5].val.numf,
...
errno_t AOloopControl_perfTest_ComputeSimilarityMatrix_cli()
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 5) == 0)
        {
            AOloopControl_perfTest_ComputeSimilarityMatrix(data.cmdargtoken[1].val.string,
                                                           data.cmdargtoken[2].val.string);
...
errno_t AOloopControl_perfTest_StatAnalysis_2streams_cli()
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 4) + CLI_checkarg(4, 4) + CLI_checkarg(5, 2) +
        ...
        0)
        {
            AOloopControl_perfTest_StatAnalysis_2streams(data.cmdargtoken[1].val.string,
                                                         data.cmdargtoken[2].val.string,
                                                         data.cmdargtoken[3].val.string,
                                                         data.cmdargtoken[4].val.string,
...
errno_t AOloopControl_perfTest_SelectWFSframes_from_PSFframes_cli()
{
    if (0 + CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3, 1) + CLI_checkarg(4, 2) + CLI_checkarg(5, 2) +
        ...
        0)
        {
            AOloopControl_perfTest_SelectWFSframes_from_PSFframes(data.cmdargtoken[1].val.string,
                                                                  data.cmdargtoken[2].val.string,
                                                                  data.cmdargtoken[3].val.numf,
                                                                  data.cmdargtoken[4].val.numl,
                                                                  data.cmdargtoken[5].val.numl,
...
static errno_t init_module_CLI()
{
   ...
   */
    ...
    RegisterCLIcommand("aoltestmresp",
                       __FILE__,
    ...
    RegisterCLIcommand("aolmktestmseq",
                       __FILE__,
...
errno_t AOcontrolLoop_perfTest_TestSystemLatency_FPCONF()
{
    ...
    function_parameter_add_entry(
        &fps, ".OPDamp", "poke amplitude [um]", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, &OPDamp_default, NULL);

    long NBiter_default[4] = {100, 10, 100000, 100};
    // long fpi_NBiter =
    function_parameter_add_entry(
        &fps, ".NBiter", "Number of iteration", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &NBiter_default, NULL);
    ...
    long fpi_wfsdt = 0;
    function_parameter_add_entry(
        &fps, ".status.wfsdt", "WFS frame interval", FPTYPE_FLOAT64, FPFLAG_DEFAULT_OUTPUT, NULL, &fpi_wfsdt);

    // long fpi_twaitus      =
    function_parameter_add_entry(
        &fps, ".status.twaitus", "initial wait [us]", FPTYPE_INT64, FPFLAG_DEFAULT_OUTPUT, NULL, NULL);

    // long fpi_refdtoffset  =
    function_parameter_add_entry(
        &fps, ".status.refdtoffset", "baseline time offset to poke", FPTYPE_FLOAT64, FPFLAG_DEFAULT_OUTPUT, NULL, NULL);

    // long fpi_dtoffset     =
    function_parameter_add_entry(
        &fps, ".status.dtoffset", "actual time offset to poke", FPTYPE_FLOAT64, FPFLAG_DEFAULT_OUTPUT, NULL, NULL);

    // output
    // long fpi_outframerateHz =
    function_parameter_add_entry(
        &fps, ".out.framerateHz", "WFS frame rate [Hz]", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, NULL, NULL);

    // long fpi_outHardLatencyfr =
    function_parameter_add_entry(
        &fps, ".out.latencyfr", "hardware latency [frame]", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, NULL, NULL);
...
errno_t AOcontrolLoop_perfTest_TestSystemLatency_RUN()
{
    ...
    *wfsdt = (tdouble_end - tdouble_start) / (wfscntend - wfscntstart);
    ...
    if (wfscntend - wfscntstart < 5)
        {
            snprintf(
                msgstring, stringmaxlen, "Number of frames %ld too small -> cannot proceed", wfscntend - wfscntstart);
            processinfo_error(processinfo, msgstring);
            free(latencyarray);
            free(latencysteparray);
            return RETURN_FAILURE;
  ...
  */
    ...
    *latencyfr = latencystepave;
    ...
    snprintf(
        msgstring, stringmaxlen, "Measured %8.3f ms @ %.3f Hz", latencyave * 1000.0, 1.0 * (wfscntend - wfscntstart));
    processinfo_WriteMessage(processinfo, msgstring);
    ...
    if ((index < 0) || (index > (long)AOconf[LOOPNUMBER].AOpmodecoeffs.NBDMmodes - 1))
    ...
    else
        {
            ...
            for (unsigned int i = 0; i < AOconf[LOOPNUMBER].DMctrl.sizeDM; i++)
                {
                    arrayf[i] = ampl * data.image[aoloopcontrol_var.aoconfID_DMmodes]
            ...
            for (ii = 0; ii < wfsxysize; ii++)
                {
                    ...
                    if (data.image[IDwfsmask].array.F[ii] > 0.1)
                        if (data.image[IDwfsref].array.F[ii] >
                            ...
                            {
                                SNR1 = data.image[IDwfsresp].array.F[mode * wfsxysize + ii] /
...
errno_t AOloopControl_perfTest_mkSyncStreamFiles2(
    char *datadir, char *stream0, char *stream1, double tstart, double tend, double dt, double dtlag)
{
    int stringmaxlen = 500;
    ...
    for (stream = 0; stream < NBstream; stream++)
        {
            ...
            if (stream == 0)
                ...
                {
                    dtoffset = +dtlag; // stream 1 is lagging behind by dtlag, so we bring
                                       // it back in time
                                    ...
                                    if (datfileOK != 0)
                                        {
                                            ...
                                            if ((datfile[NBdatFiles].tstart < tend) &&
                                                (datfile[NBdatFiles].tend > tstart) && (datfile[NBdatFiles].cnt > 0))
                                                {
                                                    NBdatFiles++;
                                                }
                                        }
            ...
            for (i = 0; i < NBdatFiles; i++)
                {
                    printf("FILE [%ld]: %20s       %20.9f -> %20.9f   [%10ld]  %10.3f "
                           "Hz\n",
                           i,
                           datfile[i].name,
                           datfile[i].tstart,
            ...
            if (fabs(exptmedian) < 0.0000001)
                {
                    printf("Median Exp Time = 0 , exiting the process: check your data "
                           "and times");
                    exit(0);
                }
    ...
    fprintf(fp, "#\n");
    fprintf(
        fp, "# stream0 median exp time : %6.3f frame -> %8.3f Hz\n", medianexptimearray[0], medianexptimearray[0] / dt);
    fprintf(
        fp, "# stream1 median exp time : %6.3f frame -> %8.3f Hz\n", medianexptimearray[1], medianexptimearray[1] / dt);
    fprintf(fp,
                    ...
                    if (((long)k1 - (long)k2) > dtmin)
                        {
                            if (paircnt > NBpairMax - 1)
                                {
                                    printf("[%s] [%s] [%d]  ERROR: paircnt (%llu) >= NBpairMax "
                                           "(%llu)\n",
                                           __FILE__,
                                           __FUNCTION__,
                                           __LINE__,
                ...
                for (jj = 0; jj < ysize0; jj++)
                    {
                        ...
                        data.image[IDsim0pair0]
                            .array.F[pair * ysize0 * xsize0 * 3 + jj * xsize0 * 3 + ii + xsize0 * 2] =
                            data.image[IDstream0].array.F[k1 * xysize0 + jj * xsize0 + ii] -
                            data.image[IDstream0].array.F[k2 * xysize0 + jj * xsize0 + ii];
                    }
                ...
                for (jj = 0; jj < ysize1; jj++)
                    {
                        ...
                        data.image[IDsim0pair1]
                            .array.F[pair * ysize1 * xsize1 * 3 + jj * xsize1 * 3 + ii + xsize1 * 2] =
                            data.image[IDstream1].array.F[k1 * xysize1 + jj * xsize1 + ii] -
                            data.image[IDstream1].array.F[k2 * xysize1 + jj * xsize1 + ii];
                    }
        }
...
errno_t AOloopControl_perfTest_SelectWFSframes_from_PSFframes(
    char *IDnameWFS, char *IDnamePSF, float frac, long x0, long x1, long y0, long y1, int EvalMode, float alpha)
{
    imageID IDwfs;
    imageID IDpsf;
