                                               ...
                                               __attribute__((unused)) long ksize)
{
  ...
  */
    ...
    printw("    DMprimWrite = %d   Predictive control state: %d        ARPF "
    ...
    printw(" TIMIMNG :  lfr = %9.3f Hz    hw lat = %5.3f fr   comp lat = %5.3f "
    ...
    printw("BLOCK  #modes [ min - max ]    gain   limit   multf  |       dmC  "
            ...
            if (AOconf[loop].aorun.ARPFon == 1)
                {
                    ...
                    ratio =
                        AOconf[loop].AOpmodecoeffs.blockave_PFresrms[k] / AOconf[loop].AOpmodecoeffs.blockave_OLrms[k];
                    color = 0;
                    if (ratio > 1.0)
                        {
                            color = 2;
...
errno_t AOloopControl_perfTest_loopMonitor(long loop, double frequ, long nbcol)
{
    ...
    ksize = data.image[IDmodevalave].md[0].size[1]; // number of averaging line, each line is 2x averaged of
                                                    // previous line
    ...
    while (!kbdhit())
        {
            ...
            AOloopControl_perfTest_printloopstatus(
                loop, nbcol, IDmodeval_dm, IDmodeval, IDmodevalave, IDmodevalrms, ksize);
...
errno_t AOloopControl_perfTest_statusStats(int updateconf, long NBsample)
{
    ...
    for (st = 0; st < statusmax; st++)
        {
            printf("STATUS %2d     %5.2f %%    [   %6ld  /  %6ld  ]   [ %9.3f us] "
                   "%s\n",
                   st,
    ...
    for (st = 0; st < statusmax; st++)
        if (strlen(statusMdef[st]) > 0)
            {
                printf("STATUSM  %2d     %5.2f %%    [   %6ld  /  %6ld  ]   [ "
                       "%9.3f us] %s\n",
                       st,
    ...
    for (st = 0; st < statusmax; st++)
        if (strlen(statusM1def[st]) > 0)
            {
                printf("STATUSM1 %2d     %5.2f %%    [   %6ld  /  %6ld  ]   [ "
                       "%9.3f us] %s\n",
                       st,
