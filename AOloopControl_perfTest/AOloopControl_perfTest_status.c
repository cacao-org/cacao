...
errno_t AOloopControl_perfTest_printloopstatus(long loop, long nbcol, __attribute__((unused)) imageID IDmodeval_dm,
    ...
    */
    ...
    if (AOconf[loop].aorun.ARPFon == 1)
    {
        if (aoloopcontrol_var.aoconfID_modeARPFgainAuto == -1)
        {
            ...
            create_image_ID(imname, 2, sizeout, _DATATYPE_FLOAT, 1, 0, 0,
    ...
    printw("    Gain = %5.3f   maxlim = %5.3f     GPU = %d    kmax=%ld\n", AOconf[loop].aorun.gain,
           AOconf[loop].aorun.maxlimit, AOconf[loop].AOcompute.GPU0, kmax);

    printw("    DMprimWrite = %d   Predictive control state: %d        ARPF gain = %5.3f   AUTOTUNE LIM = %d (perc = "
           "%.2f %%  delta = %.3f nm mcoeff=%4.2f) GAIN = %d\n",
           AOconf[loop].aorun.DMprimaryWriteON, AOconf[loop].aorun.ARPFon, AOconf[loop].aorun.ARPFgain,
           AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_ON, AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_perc,
           1000.0 * AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_delta, AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_mcoeff,
           AOconf[loop].AOAutoTune.AUTOTUNE_GAINS_ON);

    printw(" TIMIMNG :  lfr = %9.3f Hz    hw lat = %5.3f fr   comp lat = %5.3f fr  wfs extr lat = %5.3f fr\n",
           AOconf[loop].AOtiminginfo.loopfrequ, AOconf[loop].AOtiminginfo.hardwlatency_frame,
           AOconf[loop].AOtiminginfo.complatency_frame, AOconf[loop].AOtiminginfo.wfsmextrlatency_frame);
    printw("loop iteration CNT : %lld   ", AOconf[loop].aorun.cnt);
    printw("\n");

    printw("=========== %6ld modes, %3ld blocks ================|------------ Telemetry [nm] ----------------|    |    "
           " LIMITS         |",
           AOconf[loop].AOpmodecoeffs.NBDMmodes, AOconf[loop].AOpmodecoeffs.DMmodesNBblock);
    if (AOconf[loop].aorun.ARPFon == 1)
    {
    ...
    printw("BLOCK  #modes [ min - max ]    gain   limit   multf  |       dmC  Input[OL] ->    WFS[CL]  Ratio  |    | "
           "hits/step    perc  |");
    if (AOconf[loop].aorun.ARPFon == 1)
    {
    ...
    for (unsigned int k = 0; k < AOconf[loop].AOpmodecoeffs.DMmodesNBblock; k++)
    {
        ...
        printw("    %4ld [ %4ld - %4ld ]   %5.3f  %7.5f  %5.3f", AOconf[loop].AOpmodecoeffs.NBmodes_block[k], kmin,
               AOconf[loop].AOpmodecoeffs.indexmaxMB[k] - 1, data.image[aoloopcontrol_var.aoconfID_gainb].array.F[k],
               data.image[aoloopcontrol_var.aoconfID_limitb].array.F[k],
               data.image[aoloopcontrol_var.aoconfID_multfb].array.F[k]);

        printw("  |  %8.2f  %8.2f  ->  %8.2f", 1000.0 * (AOconf[loop].AOpmodecoeffs.blockave_Crms[k]),
        ...
        printw("| %2ld | %9.3f  %6.2f\% |", k, AOconf[loop].AOpmodecoeffs.blockave_limFrac[k],
    ...
    printw("  |  %8.2f  %8.2f  ->  %8.2f", 1000.0 * AOconf[loop].AOpmodecoeffs.ALLave_Crms,
           1000.0 * AOconf[loop].AOpmodecoeffs.ALLave_OLrms, 1000.0 * AOconf[loop].AOpmodecoeffs.ALLave_WFSrms);

    attron(A_BOLD);
    printw("   %5.3f  ", AOconf[loop].AOpmodecoeffs.ALLave_WFSrms / AOconf[loop].AOpmodecoeffs.ALLave_OLrms);
    attroff(A_BOLD);

    printw("| %2ld | %9.3f  %6.2f\% |\n", 0, AOconf[loop].AOpmodecoeffs.ALLave_limFrac,
           100.0 * AOconf[loop].AOpmodecoeffs.ALLave_limFrac / AOconf[loop].AOpmodecoeffs.NBDMmodes);
...
errno_t AOloopControl_perfTest_loopMonitor(long loop, double frequ, long nbcol)
{
    ...
    ksize =
        data.image[IDmodevalave].md[0].size[1]; // number of averaging line, each line is 2x averaged of previous line
...
errno_t AOloopControl_perfTest_statusStats(int updateconf, long NBsample)
{
    ...
    if (updateconf == 1)
    {
        AOconf[LOOPNUMBER].AOtiminginfo.hardwlatency_frame =
            AOconf[LOOPNUMBER].AOtiminginfo.hardwlatency * AOconf[LOOPNUMBER].AOtiminginfo.loopfrequ;
    ...
    for (st = 0; st < statusmax; st++)
    {
        printf("STATUS %2d     %5.2f %%    [   %6ld  /  %6ld  ]   [ %9.3f us] %s\n", st,
               100.0 * statuscnt[st] / NBsample, statuscnt[st], NBsample, loopiterus * statuscnt[st] / NBsample,
               statusdef[st]);
    }
    ...
    for (st = 0; st < statusmax; st++)
        if (strlen(statusMdef[st]) > 0)
        {
            printf("STATUSM  %2d     %5.2f %%    [   %6ld  /  %6ld  ]   [ %9.3f us] %s\n", st,
                   100.0 * statusMcnt[st] / NBsample, statusMcnt[st], NBsample, loopiterus * statusMcnt[st] / NBsample,
                   statusMdef[st]);
        }
    ...
    for (st = 0; st < statusmax; st++)
        if (strlen(statusM1def[st]) > 0)
        {
            printf("STATUSM1 %2d     %5.2f %%    [   %6ld  /  %6ld  ]   [ %9.3f us] %s\n", st,
                   100.0 * statusM1cnt[st] / NBsample, statusM1cnt[st], NBsample,
                   loopiterus * statusM1cnt[st] / NBsample, statusM1def[st]);
        }
...
errno_t AOloopControl_perfTest_showparams(long loop)
{
    ...
    printf("Global gain = %f   maxlim = %f\n  multcoeff = %f  GPU = %d\n", AOconf[loop].aorun.gain,
           AOconf[loop].aorun.maxlimit, AOconf[loop].aorun.mult, AOconf[loop].AOcompute.GPU0);
    printf("    Predictive control state: %d        ARPF gain = %5.3f   AUTOTUNE: lim %d gain %d\n",
           AOconf[loop].aorun.ARPFon, AOconf[loop].aorun.ARPFgain, AOconf[loop].AOAutoTune.AUTOTUNE_LIMITS_ON,
           AOconf[loop].AOAutoTune.AUTOTUNE_GAINS_ON);
