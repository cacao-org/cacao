...
)
{
    ...
    if(create == 1)
    {
        ...
        result = lseek(SM_fd, sizeof(FPAOLOOPCONTROL_CONF) * NB_FPAOloopcontrol - 1,
                       SEEK_SET);
        if(result == -1)
        {
            close(SM_fd);
    ...
    FPAOconf = (FPAOLOOPCONTROL_CONF *) mmap(0,
               sizeof(FPAOLOOPCONTROL_CONF) * NB_FPAOloopcontrol, PROT_READ | PROT_WRITE,
               MAP_SHARED, SM_fd, 0);
...
)
{
    ...
    FPAOconf[loop].hardwlatency_frame = FPAOconf[loop].hardwlatency *
    ...
    if((fp = fopen("./conf/conf_loopfrequ.txt", "r")) == NULL)
    {
        ...
        fprintf(fplog,
                "WARNING: file ./conf/conf_loopfrequ.txt missing. Using default loop speed\n");
    ...
    FPaoconfID_wfsdark = AOloopControl_IOtools_2Dloadcreate_shmim(name, fname,
                         FPAOconf[loop].sizexWFS, FPAOconf[loop].sizeyWFS, 0.0);
    ...
    FPaoconfID_imWFS0 = AOloopControl_IOtools_2Dloadcreate_shmim(name, " ",
                        FPAOconf[loop].sizexWFS, FPAOconf[loop].sizeyWFS, 0.0);
    COREMOD_MEMORY_image_set_createsem(name, 10);

    sprintf(name, "FPaol%ld_imWFS1", loop);
    FPaoconfID_imWFS1 = AOloopControl_IOtools_2Dloadcreate_shmim(name, " ",
                        FPAOconf[loop].sizexWFS, FPAOconf[loop].sizeyWFS, 0.0);
    COREMOD_MEMORY_image_set_createsem(name, 10);
...
int FPAOloopControl_Read_cam_frame(long loop, int semindex)
{
    ...
    if(FPcamReadInit == 0)
    {
        ...
        printf("INITIALIZING SEMAPHORE %d   %s   (%d)\n", semindex,
               data.image[FPaoconfID_wfsim].md[0].name, semval);
        for(i = 0; i < semval; i++)
        {
    ...
    {
        while(FPAOconf[loop].WFScnt == (long long)
                data.image[FPaoconfID_wfsim].md[0].cnt0) // test if new frame exists
        {
            usleep(5);
        }
    }
...
long FPAO_Measure_WFSrespC(long loop, long delayfr, long delayRM1us, long NBave,
                           long NBexcl, char *IDpokeC_name, char *IDoutC_name, int FPAOinitMode,
                           long NBcycle)
{
    long *sizearray;
    long IDoutC;
    ...
    while((iter < NBiter) && (data.signal_USR1 == 0))
    {
        ...
        usleep(delayRM1us);
        data.image[FPaoconfID_dmRM].md[0].write = 1;
        memcpy(data.image[FPaoconfID_dmRM].array.F, arrayf,
               sizeof(float)*FPAOconf[loop].sizeDM);
        data.image[FPaoconfID_dmRM].md[0].cnt0++;
        data.image[FPaoconfID_dmRM].md[0].write = 0;
        FPAOconf[loop].DMupdatecnt ++;
    ...
    for(PokeIndex = 0; PokeIndex < NBpoke; PokeIndex++)
        for(ii = 0; ii < FPAOconf[loop].sizeWFS; ii++)
        {
            data.image[IDoutC].array.F[PokeIndex * FPAOconf[loop].sizeWFS + ii] /= NBave *
                    iter;
        }
...
long FPAOloopControl_MeasureResp_level1(float ampl, long delayfr,
                                        long delayRM1us, long NBave, long NBexcl, int FPAOinitMode, long NBiter)
{
    ...
    for(ii = 0; ii < FPAOconf[loop].sizexDM; ii++)
        ...
        {
            data.image[IDpokeC].array.F[poke * FPAOconf[loop].sizeDM + jj *
                                        FPAOconf[loop].sizexDM + ii] = 0.0;
        }
    ...
    for(ii = 0; ii < FPAOconf[loop].sizexDM; ii++)
        ...
        {
            data.image[IDpokeC].array.F[poke * FPAOconf[loop].sizeDM + jj *
                                        FPAOconf[loop].sizexDM + ii] = ampl * (ii % 2);
        }
    ...
    for(ii = 0; ii < FPAOconf[loop].sizexDM; ii++)
        ...
        {
            data.image[IDpokeC].array.F[poke * FPAOconf[loop].sizeDM + jj *
                                        FPAOconf[loop].sizexDM + ii] = ampl * (jj % 2);
        }
...
//
long FPAOloopControl_MakeLinComb_seq(char *IDpC_name, long xsize0, long ysize0,
                                     long NBmaster0, long N, char *IDout_name)
{
    long IDout;
    ...
    for(kk = 0; kk < kksize; kk++)
    {
        ...
        for(k = 0; k < NBmaster; k++)
        {
            ...
            for(ii = 0; ii < xysize; ii++)
            {
                data.image[IDout].array.F[xysize * kk + ii] += N1array[narray[k]] *
