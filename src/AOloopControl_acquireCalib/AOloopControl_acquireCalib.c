...
int_fast8_t AOloopControl_acquireCalib_RespMatrix_Fast_cli()
{
    if(CLI_checkarg(1, 4) + CLI_checkarg(2, 4) + CLI_checkarg(3,
            4) + CLI_checkarg(4, 2) + CLI_checkarg(5, 1) + CLI_checkarg(6,
                    1) + CLI_checkarg(7, 1) + CLI_checkarg(8, 3) == 0)
    {
        AOloopControl_acquireCalib_RespMatrix_Fast(data.cmdargtoken[1].val.string,
                ...
                data.cmdargtoken[4].val.numl, data.cmdargtoken[5].val.numf,
...
int_fast8_t AOloopControl_acquireCalib_Measure_WFSrespC_cli()
{
    if(CLI_checkarg(1, 2) + CLI_checkarg(2, 2) + CLI_checkarg(3,
            2) + CLI_checkarg(4, 2) + CLI_checkarg(5, 4) + CLI_checkarg(6,
                    5) + CLI_checkarg(7, 2) + CLI_checkarg(8, 2) + CLI_checkarg(9,
                            2) + CLI_checkarg(10, 2) == 0)
    {
        AOloopControl_acquireCalib_Measure_WFSrespC(LOOPNUMBER,
                data.cmdargtoken[1].val.numl, data.cmdargtoken[2].val.numl,
                ...
                data.cmdargtoken[9].val.numl, (uint32_t) data.cmdargtoken[10].val.numl);
        return 0;
    }
    else
    {
...
{
    if(CLI_checkarg(1, 1) + CLI_checkarg(2, 2) + CLI_checkarg(3,
            2) + CLI_checkarg(4, 2) + CLI_checkarg(5, 2) + CLI_checkarg(6,
                    4) + CLI_checkarg(7, 5) + CLI_checkarg(8, 5) + CLI_checkarg(9,
                            2) + CLI_checkarg(10, 2) + CLI_checkarg(11, 2) + CLI_checkarg(12, 2) == 0)
    {
        AOloopControl_acquireCalib_Measure_WFS_linResponse(LOOPNUMBER,
                ...
                data.cmdargtoken[7].val.string, data.cmdargtoken[8].val.string,
...
int_fast8_t AOloopControl_acquireCalib_Measure_zonalRM_cli()
{
    if(CLI_checkarg(1, 1) + CLI_checkarg(2, 2) + CLI_checkarg(3,
            2) + CLI_checkarg(4, 2) + CLI_checkarg(5, 2) + CLI_checkarg(6,
                    3) + CLI_checkarg(7, 3) + CLI_checkarg(8, 3) + CLI_checkarg(9,
                            3) + CLI_checkarg(10, 2) + CLI_checkarg(11, 2) + CLI_checkarg(12,
                                    2) + CLI_checkarg(13, 2) == 0)
    {
        AOloopControl_acquireCalib_Measure_zonalRM(LOOPNUMBER,
                ...
                data.cmdargtoken[5].val.numl, data.cmdargtoken[6].val.string,
                data.cmdargtoken[7].val.string, data.cmdargtoken[8].val.string,
                data.cmdargtoken[9].val.string, data.cmdargtoken[10].val.numl,
                data.cmdargtoken[11].val.numl, data.cmdargtoken[12].val.numl,
...
{
    if(CLI_checkarg(1, 2) + CLI_checkarg(2, 1) + CLI_checkarg(3,
...
{
    if(CLI_checkarg(1, 2) + CLI_checkarg(2, 2) + CLI_checkarg(3,
            2) + CLI_checkarg(4, 3) + CLI_checkarg(5, 2) + CLI_checkarg(6, 2) == 0)
    {
        AOloopControl_acquireCalib_RMseries_deinterlace(data.cmdargtoken[1].val.numl,
                ...
                data.cmdargtoken[4].val.string, data.cmdargtoken[5].val.numl,
...
int_fast8_t init_AOloopControl_acquireCalib()
{
    ...
    RegisterCLIcommand("aolacqresp", __FILE__,
    ...
    RegisterCLIcommand("aolmRMfast", __FILE__,
    ...
    RegisterCLIcommand("aolmeasWFSrespC", __FILE__,
    ...
    RegisterCLIcommand("aolmeaslWFSrespC", __FILE__,
    ...
    RegisterCLIcommand("aolmeaszrm", __FILE__,
...
)
{
    ...
    for(pm = 0; pm < NBpokemap; pm++)
    {
        ...
        for(axis = 0; axis < NBmode; axis++)
        {
            ...
            {
                data.image[IDpokemapC].array.F[xsize * ysize * pm + ii] += vectarray[pm * NBmode
                        + axis] * data.image[IDmodeC].array.F[axis * xsize * ysize + ii];
            }
        }
...
)
{
    ...
    // SETUP FPS
    // ===========================
    FUNCTION_PARAMETER_STRUCT fps = function_parameter_FPCONFsetup(fpsname, CMDmode,
                                    &loopstatus);
    ...
    while(loopstatus == 1)
    {
        if(function_parameter_FPCONFloopstep(&fps, CMDmode,
                                             &loopstatus) == 1)  // Apply logic if update is needed
        {
            // here goes the logic
...
)
{
...
#ifdef AOLOOPCONTROL_ACQUIRECALIB_LOGDEBUG
    snprintf(data.execSRCmessage, STRINGMAXLEN_FUNCTIONARGS,
             "%ld %ld %ld %ld %ld %s %s %d %d %ld %d",
             loop,
             delayfr,
             delayRM1us,
             NBave,
    ...
    char pinfomsg[200];
    sprintf(pinfomsg, "delay=%ld+%ldus ave=%ld excl=%ld cyc=%ld", delayfr,
            delayRM1us, NBave, NBexcl, NBcycle);
    ...
    {
        sprintf(pinfomsg, "Loaded stream %s [%d %d %d]", IDpokeC_name,
    ...
    while(loopOK == 1)
    {
        ...
        sprintf(pinfomsg, "NBpoke=%ld # %3ld/%3ld (%6ld/%6ld)", NBpoke, iter, NBiter,
                imcnt, imcntmax);
        processinfo_WriteMessage(processinfo, pinfomsg);
        ...
        if(SAVE_RMACQU_ALL == 1)   // Save all intermediate result
        {
            ...
            for(AveStep = 0; AveStep < NBave;
                    AveStep++)   // save to disk IDoutCstep[AveStep]
            {
                char imname[100];
                sprintf(imname, "imoutStep%03d", AveStep);

                sprintf(tmpfname, "!tmpRMacqu/%s.tstep%03d.iter%03ld.fits", IDoutC_name,
                        AveStep, iter);
                //list_image_ID();
    ...
    {
        fprintf(fp,
                "%6ld %3ld    %1d %1d     %6ld  %6ld     %4ld %4ld   %4ld %4ld     %3ld %3ld %3ld\n",
                imcnt,
                array_iter[imcnt],
                array_poke[imcnt],
                array_accum[imcnt],
...
long AOloopControl_acquireCalib_Measure_zonalRM(long loop, double ampl,
        long delayfr, long delayRM1us, long NBave, long NBexcl, const char *zrespm_name,
        const char *WFSref0_name, const char *WFSmap_name, const char *DMmap_name,
        long mode, int normalize, int AOinitMode, long NBcycle)
{
    long ID_WFSmap, ID_WFSref0, ID_WFSref2, ID_DMmap, IDmapcube, IDzrespm,
         IDzrespmn, ID_WFSref0n,  ID_WFSref2n;
    long act, j, ii, kk;
    double value;
    float *arrayf;
    char fname[200];
    ...
    if(IDpokeC == -1)
    {
        ...
        for(act = 0; act < AOconf[loop].DMctrl.sizexDM * AOconf[loop].DMctrl.sizeyDM;
                act++)
        {
            for(ii = 0; ii < AOconf[loop].DMctrl.sizexDM * AOconf[loop].DMctrl.sizeyDM;
                    ii++)
            {
...
int_fast8_t AOloopControl_acquireCalib_Measure_Resp_Matrix(long loop,
        long NbAve, float amp, long nbloop, long fDelay, long NBiter)
{
    long NBloops;
    long kloop;
    ...
    for(iter = 0; iter < NBiter; iter++)
    {
        ...
        else
        {
            ...
            normcoeff = 0.0;
            normcoeffcnt = 0.0;
            for(k1 = AOconf[loop].AOpmodecoeffs.NBDMmodes / 2;
...
long AOloopControl_acquireCalib_RespMatrix_Fast(const char *DMmodes_name,
        const char *dmRM_name, const char *imWFS_name, long semtrig, float HardwareLag,
        float loopfrequ, float ampl, const char *outname)
{
    long IDout;
    long IDmodes;
...
)
{
    ...
    float *coeffarray = (float *) malloc(sizeof(float) * zsizeout * NBRM *
                                         refstart);
