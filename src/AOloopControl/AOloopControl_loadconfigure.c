...
)
{
        ...
        if(aoloopcontrol_var.aoconfID_DMmodes == -1) // If not, check file
        {
            ...
            case _DATATYPE_FLOAT :
                memcpy(data.image[aoloopcontrol_var.aoconfID_DMmodes].array.F,
                       ...
                       sizeof(float)*AOconf[loop].DMctrl.sizexDM*AOconf[loop].DMctrl.sizeyDM*AOconf[loop].AOpmodecoeffs.NBDMmodes);
                break;
            case _DATATYPE_DOUBLE :
                for(unsigned long ii=0; ii < AOconf[loop].DMctrl.sizexDM*AOconf[loop].DMctrl.sizeyDM*AOconf[loop].AOpmodecoeffs.NBDMmodes; ii++)
    ...
    if(level>=10)
    {
        ...
        for(kk=0; kk<AOconf[loop].AOpmodecoeffs.DMmodesNBblock; kk++)
        {
            ...
            if(kk==0) {
                for(unsigned long ii=0; ii<AOconf[loop].WFSim.sizexWFS*AOconf[loop].WFSim.sizeyWFS*AOconf[loop].DMctrl.sizexDM*AOconf[loop].DMctrl.sizeyDM; ii++) {
