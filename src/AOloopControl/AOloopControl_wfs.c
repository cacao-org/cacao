...
)
{
    ...
    while(data.signal_USR1 == 0)
    {
        ...
        printf("WFS zero point offset update  # %8llu       (%s -> %s)  ", zpcnt,
               data.image[IDzpdm].name, data.image[IDwfszp].name);
        fflush(stdout);
...
# endif
            for(act = 0; act < dmxysize; act++)
                ...
                {
                    data.image[IDtmp].array.F[elem] += data.image[IDzpdm].array.F[act] *
        ...
        memcpy(data.image[IDwfszp].array.F, data.image[IDtmp].array.F,
...
)
{
    ...
    for(;;)
    {
        ...
        {
            // copy wfsref0 to tmp
            memcpy(data.image[IDtmp].array.F, data.image[IDwfsref0].array.F,
                   sizeof(float)*wfsxysize);
            ...
            memcpy(data.image[IDwfsref].array.F, data.image[IDtmp].array.F,
                   sizeof(float)*wfsxysize);
...
{
    cblas_sgemv(CblasRowMajor, CblasNoTrans, m, n, 1.0, cm_array, n, imarray, 1,
                0.0, outvect, 1);
...
)
{
    ...
    for(;;)
    {
        ...
        {
            while(cnt ==
                    data.image[aoloopcontrol_var.aoconfID_contrM].md[0].cnt0) // test if new frame exists
            {
                usleep(5);
            }
        ...
        for(ii = 0; ii < wfsxysize; ii++)
        {
            data.image[IDout].array.F[ii] =
                ...
                data.image[IDtot].array.F[0] - data.image[IDwfsref].array.F[ii];
        }
        ...
        for(ii = 0; ii < wfsxysize; ii++)
        {
            data.image[IDoutave].array.F[ii] = (1.0 - data.image[IDalpha].array.F[0]) *
                                               data.image[IDoutave].array.F[ii] + data.image[IDalpha].array.F[0] *
                                               data.image[IDout].array.F[ii];
        }
        ...
        for(ii = 0; ii < wfsxysize; ii++)
        {
            data.image[IDoutmave].array.F[ii] = data.image[IDoutave].array.F[ii] *
                                                data.image[IDwfsmask].array.F[ii];
        }
        ...
        for(ii = 0; ii < wfsxysize; ii++)
        {
            data.image[IDoutrms].array.F[ii] = (1.0 - data.image[IDalpha].array.F[0]) *
                                               data.image[IDoutrms].array.F[ii] + data.image[IDalpha].array.F[0] *
                                               (data.image[IDout].array.F[ii] - data.image[IDoutave].array.F[ii]) *
                                               (data.image[IDout].array.F[ii] - data.image[IDoutave].array.F[ii]);
        }
