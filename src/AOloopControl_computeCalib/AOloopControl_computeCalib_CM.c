...
)
{
    ...
    for(uint32_t k = 0; k < m; k++)
    {
        for(uint64_t ii = 0; ii < n; ii++)
        {
            gsl_matrix_set(matrix_D, ii, k,
                           data.image[ID_Rmatrix].array.F[k * n + ii]*CPAcoeff[k]);
        }
    }
    /* compute DtraD */
                ...
                for(uint32_t kk1 = 0; kk1 < m; kk1++)
                {
                    for(uint64_t ii = 0; ii < xsize_modes * ysize_modes; ii++)
                        data.image[IDeigenmodes].array.F[kk * xsize_modes * ysize_modes + ii] +=
                            data.image[ID_VTmatrix].array.F[kk1 * m + kk] * data.image[IDmodes].array.F[kk1
                                    * xsize_modes * ysize_modes + ii];
                }
            }
            printf("\n");
        }
    ...
    {
        printf("\r Number of modes removed : %5ld / %5ld  (step %ld)  ", NB_MR,
               NB_MODE_REMOVED1, NB_MODE_REMOVED_STEP);
        fflush(stdout);
...
)
{
    ...
    for(uint32_t mode = 0; mode < NBDMmodes; mode++)
    {
        for(uint64_t act = 0; act < sizeDM; act++)
        {
            ...
            {
                matrix_Mc[act * sizeWFS + wfselem] += matrix_cmp[mode * sizeWFS + wfselem] *
                                                      matrix_DMmodes[mode * sizeDM + act];
            }
        }
    }
    //# ifdef _OPENMP
            ...
            slice++)
    {
        ...
        for(uint64_t act_active = 0; act_active < sizeDM_active; act_active++)
        {
                    ...
                    wfselem_active++)
            {
                ...
                uint64_t wfselem = aoloopcontrol_var.WFS_active_map[slice * sizeWFS +
                                         wfselem_active];
                data.image[IDcmatc_active[slice]].array.F[act_active * sizeWFS_active[slice] +
                        wfselem_active] =
