...
)
{
    ...
    if(COMPUTE_DM_MODES == 1) // DM modes fmodes2b
    {
        ...
        if(MODAL == 0)
        {
            ...
            for(uint32_t k = 0; k < NBZ; k++)
            {
                ...
                for(uint32_t ii = 0; ii < msizex; ii++)
                    for(uint32_t jj = 0; jj < msizey; jj++)
                    {
                        ...
                        r = sqrt(x * x + y * y) / r1;
                        PA = atan2(y, x);
                        data.image[ID].array.F[k * msizex * msizey + jj * msizex + ii] = Zernike_value(
                                    zindex[k], r, PA);
                    }
            }
            ...
            if(MaskMode == 1)
            {
                ...
                for(citer = 0; citer < NBciter; citer++)
                {
                    ...
                    gauss_filter(ID_name, "modeg", 4.0 * pow(1.0 * (NBciter - citer) / NBciter,
                                 0.5), kernsize);
                    ...
                    for(uint32_t k = 0; k < data.image[ID].md[0].size[2]; k++)
                    {
                        for(uint64_t ii = 0; ii < msizex * msizey; ii++)
                            ...
                            {
                                data.image[ID].array.F[k * msizex * msizey + ii] = data.image[IDg].array.F[k *
                                        msizex * msizey + ii];
                            }
                    }
            ...
            for(uint64_t ii = 0; ii < msizex * msizey; ii++)
            {
                data.image[IDmask].array.F[ii] = 1.0 - (1.0 - data.image[IDmaskRM].array.F[ii])
            ...
            for(uint32_t m = 0; m < data.image[ID].md[0].size[2]; m++)
            {
                for(uint64_t ii = 0; ii < msizex * msizey; ii++)
                {
                    data.image[ID].array.F[m * msizex * msizey + ii] = data.image[IDtmp].array.F[m *
                            msizex * msizey + ii] * data.image[IDmask].array.F[ii];
                }
            }
        }
        else
        ...
        for(uint32_t k = 0; k < (uint32_t)(data.image[ID0].md[0].size[2] - 1 + NBZ);
                k++)
        {
            /// Remove excluded modes if they exist
            ...
            if(IDeModes != -1)
            {
                ...
                for(uint64_t ii = 0; ii < msizex * msizey; ii++)
                {
                    data.image[IDtm].array.F[ii] = data.image[ID].array.F[k * msizex * msizey +
                                                   ii];
                }
                ...
                for(uint64_t ii = 0; ii < msizex * msizey; ii++)
                {
                    data.image[ID].array.F[k * msizex * msizey + ii] = data.image[IDtm].array.F[ii]
...
# endif
            ...
            for(m = 0; m < data.image[ID].md[0].size[2]; m++)
            {
                ...
                for(act = 0; act < msizexy; act++)
                {
                    ...
                    {
                        data.image[IDm].array.F[m1 + wfselem] += data.image[ID].array.F[act1] *
        ...
        if((IDRMMmodes != -1) && (IDRMMresp != -1))
        {
            ...
            for(uint32_t m = 0; m < data.image[IDmodes0all].md[0].size[2]; m++)
            {
                ...
                {
                    data.image[ID_imfit].array.F[ii] = data.image[IDmodes0all].array.F[m * msizexy +
                                                       ii];
                }
                ...
                for(uint32_t jj = 0; jj < linfitsize; jj++)
                    for(uint64_t ii = 0; ii < msizex * msizey; ii++)
                    {
                        data.image[IDtmp].array.F[ii] += data.image[IDRMM_coeff].array.F[jj] *
                                                         data.image[IDRMMmodes].array.F[jj * msizex * msizey + ii];
                    }
                ...
                if(LOcoeff > 0.01)
                {
                    ...
                    for(uint32_t jj = 0; jj < linfitsize; jj++)
                        for(uint64_t wfselem = 0; wfselem < wfssize; wfselem++)
                        {
                            data.image[IDwfstmp].array.F[wfselem] += data.image[IDRMM_coeff].array.F[jj] *
                                    data.image[IDRMMresp].array.F[jj * wfssize + wfselem];
                        }

                    for(uint64_t wfselem = 0; wfselem < wfssize; wfselem++)
                    {
                        data.image[IDm].array.F[m * wfssize + wfselem] = LOcoeff *
                                data.image[IDwfstmp].array.F[wfselem] + (1.0 - LOcoeff) *
        ...
        for(uint32_t m = 0; m < data.image[ID].md[0].size[2]; m++)
        {
            ...
            for(uint64_t ii = 0; ii < msizex * msizey; ii++)
                data.image[MBLOCK_ID[mblock1]].array.F[MBLOCK_NBmode[mblock1]*msizex * msizey +
                                                       ii]
            ...
            for(uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                for(uint64_t ii = 0; ii < msizex * msizey; ii++)
                    data.image[MBLOCK_ID[mblock]].array.F[m * msizex * msizey + ii]
                        = data.image[IDextrablock].array.F[m * msizex * msizey + ii] *
                          data.image[IDmaskRM].array.F[ii];
        ...
        for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            ...
            printf("[%5d] STEP3  -  BLOCK %u/%u: keeping %u / %u modes  ( %f %f ) [%u  %u %u]\n",
                   __LINE__, mblock, NBmblock, cnt, data.image[IDSVDcoeff].md[0].size[0], SVDlim00,
                   svdcoeff0, data.image[IDSVDcoeff].md[0].size[0], msizex, msizey);
            fflush(stdout);
            ...
            slen = snprintf(fname1, STRINGMAXLEN_FULLFILENAME,
                            "!./mkmodestmp/fmodes1_%02u.fits", mblock);
            if(slen < 1)
            {
                PRINT_ERROR("snprintf wrote <1 char");
                abort(); // can't handle this error any other way
        ...
        for(uint32_t mblock = 0; mblock < NBmblock; mblock++) // outer block loop
        {
            ...
            for(uint32_t mblock0 = 0; mblock0 < mblock; mblock0++) // inner block loop
            {
                ...
                for(uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                {
                    ...
                    for(uint64_t ii = 0; ii < msizexy; ii++)
                    {
                        data.image[IDSVDmodein].array.F[ii] = data.image[MBLOCK_ID[mblock]].array.F[m *
                                                              msizexy + ii];
                    }
        ...
        for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            ...
            slen = snprintf(fnameSVDcoeff, STRINGMAXLEN_FULLFILENAME,
                            "./mkmodestmp/SVDcoeff01_%02u.txt", mblock);
            if(slen < 1)
            {
                PRINT_ERROR("snprintf wrote <1 char");
                abort(); // can't handle this error any other way
            ...
            for(uint32_t m = 0; m < data.image[IDSVDcoeff].md[0].size[0]; m++)
            {
                fprintf(fpcoeff, "%5u   %12g   %12g  %5u     %10.8f  %10.8f\n",
                        m, data.image[IDSVDcoeff].array.F[m], data.image[IDSVDcoeff].array.F[0], cnt,
                        data.image[IDSVDcoeff].array.F[m] / data.image[IDSVDcoeff].array.F[0],
                        SVDlim01);
            ...
            for(uint32_t m = 0; m < cnt; m++)
            {
                ...
                for(uint64_t ii = 0; ii < msizexy; ii++)
                {
                    value1 += data.image[IDm].array.F[m * msizexy + ii] * data.image[IDm].array.F[m
                              * msizexy + ii] * data.image[IDmaskRM].array.F[ii];
                }
            ...
            slen = snprintf(fname1, STRINGMAXLEN_FULLFILENAME,
                            "!./mkmodestmp/fmodes2b_%02u.fits", mblock);
            if(slen < 1)
            {
                PRINT_ERROR("snprintf wrote <1 char");
                abort(); // can't handle this error any other way
    ...
    else
    {
        ...
        for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            snprintf(fname, STRINGMAXLEN_FULLFILENAME, "./mkmodestmp/fmodes2b_%02u.fits",
                     mblock);
...
# endif
                    for(m = 0; m < MBLOCK_NBmode[mblock]; m++)
                    {
                        for(act = 0; act < msizexy; act++)
                        {
                            for(wfselem = 0; wfselem < wfssize; wfselem++)
                            {
                                data.image[IDwfsMresp].array.F[m * wfssize + wfselem] +=
                                    data.image[MBLOCK_ID[mblock]].array.F[m * msizexy + act] *
                                    data.image[IDzrespM].array.F[act * wfssize + wfselem];
                            }
                        }
                    }
                    ...
                    if((IDRMMmodes != -1) && (IDRMMresp != -1))
                    {
                        ...
                        for(uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                        {
                            for(uint64_t ii = 0; ii < msizexy; ii++)
                            {
                                data.image[ID_imfit].array.F[ii] = data.image[MBLOCK_ID[mblock]].array.F[m *
                                                                   msizexy + ii];
                            }
                            ...
                            if(LOcoeff > 0.01)
                            {
                                ...
                                for(wfselem = 0; wfselem < wfssize; wfselem++)
                                {
                                    data.image[IDwfsMresp].array.F[m * wfssize + wfselem] = LOcoeff *
                                            data.image[IDwfstmp].array.F[wfselem] + (1.0 - LOcoeff) *
                                            data.image[IDwfsMresp].array.F[m * wfssize + wfselem];
                                }
                            }
                    ...
                    slen = snprintf(fname, STRINGMAXLEN_FULLFILENAME,
                                    "!./mkmodestmp/fmodesWFS0_%02u.fits", mblock);
                    if(slen < 1)
                    {
                        PRINT_ERROR("snprintf wrote <1 char");
                        abort(); // can't handle this error any other way
            ...
            for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
            {
                ...
                for(uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                {
                    for(uint64_t ii = 0; ii < wfssize; ii++)
                    {
                        data.image[IDm].array.F[cnt * wfssize + ii] = data.image[IDmwfs].array.F[m *
                                wfssize + ii];
                    }
                    cnt++;
                }
            ...
            for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
            {
                ...
                for(uint32_t mblock0 = 0; mblock0 < mblock; mblock0++)
                {
                    ...
                    for(uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                    {
                        ...
                        {
                            data.image[IDSVDmodein].array.F[ii] = data.image[IDmwfs].array.F[m * wfssize +
                                                                  ii];
                        }
                ...
                if(cnt > 0)
                {
                    ...
                    for(uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                    {
                        if(mok[m] == 1)
                        {
                            printf("[%5d] writing %u / %u  ->  %u / %u        \n", __LINE__, m,
                                   data.image[IDmwfs].md[0].size[2], m1, data.image[IDm].md[0].size[2]);

                            printf("[%5d] max index IDmwfs1 %u  = %ld / %ld    [ %ld %ld %ld ]\n", __LINE__,
                                   m1, (long)(m1 * wfssize + wfssize - 1),
                                   (long)(data.image[IDmwfs1].md[0].size[0]*data.image[IDmwfs1].md[0].size[1]*data.image[IDmwfs1].md[0].size[2]),
                            ...
                            printf("[%5d] max index IDmwfs  %u  = %ld / %ld    [ %ld %ld %ld ]\n", __LINE__,
                                   m, (long)(m * wfssize + wfssize - 1),
                                   (long)(data.image[IDmwfs].md[0].size[0]*data.image[IDmwfs].md[0].size[1]*data.image[IDmwfs].md[0].size[2]),
                            ...
                            printf("[%5d] max index IDmdm1  %u  = %ld / %ld    [ %ld %ld %ld ]\n", __LINE__,
                                   m1, (long)(m1 * msizexy + msizexy - 1),
                                   (long)(data.image[IDmdm1].md[0].size[0]*data.image[IDmdm1].md[0].size[1]*data.image[IDmdm1].md[0].size[2]),
                            ...
                            printf("[%5d] max index IDmdm   %u  = %ld / %ld    [ %ld %ld %ld ]\n", __LINE__,
                                   m, (long)(m * msizexy + msizexy - 1),
                                   (long)(data.image[IDmdm].md[0].size[0]*data.image[IDmdm].md[0].size[1]*data.image[IDmdm].md[0].size[2]),
                                   (long) data.image[IDmdm].md[0].size[0], (long) data.image[IDmdm].md[0].size[1],
                            ...
                            for(uint64_t ii = 0; ii < wfssize; ii++)
                            {
                                data.image[IDmwfs1].array.F[m1 * wfssize + ii] = data.image[IDmwfs].array.F[m *
                                        wfssize + ii];
                            }
                            for(uint64_t ii = 0; ii < msizexy; ii++)
                            {
                                data.image[IDmdm1].array.F[m1 * msizexy + ii] = data.image[IDmdm].array.F[m *
                                        msizexy + ii];
                            }
                            value1 = 0.0;
                            m1++;
                    ...
                    slen = snprintf(fname1, STRINGMAXLEN_FULLFILENAME,
                                    "!./mkmodestmp/fmodesWFS1_%02u.fits", mblock);
                    if(slen < 1)
                    {
                        PRINT_ERROR("snprintf wrote <1 char");
                        abort(); // can't handle this error any other way
                    ...
                    slen = snprintf(fname1, STRINGMAXLEN_FULLFILENAME,
                                    "!./mkmodestmp/fmodes3_%02u.fits", mblock);
                    if(slen < 1)
                    {
                        PRINT_ERROR("snprintf wrote <1 char");
                        abort(); // can't handle this error any other way
            ...
            for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
            {
                if(MBLOCK_NBmode[mblock] > 0)
                {
                    ...
                    for(uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
                    {
                        ...
                        for(uint64_t ii = 0; ii < msizexy; ii++)
                        {
                            data.image[IDmdm1].array.F[cnt * msizexy + ii] = data.image[IDmdm].array.F[m *
                                    msizexy + ii];
                        }
                        cnt++;
                    }
        ...
        if(BlockNB < 0)
        {
            ...
            int slen = snprintf(command, STRINGMAXLEN_COMMAND,
                                "echo \"%u\" > ./%s/param_NBmodeblocks.txt", NBmblock, stagedir);
            if(slen < 1)
            {
                PRINT_ERROR("snprintf wrote <1 char");
                abort(); // can't handle this error any other way
        ...
        for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            ...
            if((BlockNB < 0) || (BlockNB == (int) mblock))
            {
                ...
                int slen = snprintf(command, STRINGMAXLEN_COMMAND,
                                    "echo \"%f\" > ./%s/block%02u_SVDlim.txt", SVDlim,  stagedir, mblock);
                if(slen < 1)
                {
                    PRINT_ERROR("snprintf wrote <1 char");
                    abort(); // can't handle this error any other way
                ...
                slen = snprintf(fnameSVDcoeff, STRINGMAXLEN_FULLFILENAME,
                                "./mkmodestmp/SVDcoeff_%02u.txt", mblock);
                if(slen < 1)
                {
                    PRINT_ERROR("snprintf wrote <1 char");
                    abort(); // can't handle this error any other way
                ...
                {
                    fprintf(fpcoeff, "%5u   %12g   %12g  %5u     %10.8f  %10.8f\n", kk,
                            data.image[IDSVDcoeff].array.F[kk], data.image[IDSVDcoeff].array.F[0], cnt,
                            data.image[IDSVDcoeff].array.F[kk] / data.image[IDSVDcoeff].array.F[0], SVDlim);
                    printf("==== %u %12g %12g  %3u\n", kk, data.image[IDSVDcoeff].array.F[kk],
                           data.image[IDSVDcoeff].array.F[0], cnt);
                    if(data.image[IDSVDcoeff].array.F[kk] > SVDlim *
                ...
                for(uint32_t kk = 0; kk < cnt; kk++) /// eigen mode index
                {
                    for(uint32_t kk1 = 0; kk1 < data.image[IDSVDcoeff].md[0].size[0]; kk1++)
                    {
                        for(uint64_t ii = 0; ii < msizexy; ii++)
                        {
                            data.image[IDmdm1].array.F[kk * msizexy + ii] +=
                                data.image[ID_VTmatrix].array.F[kk1 * data.image[IDSVDcoeff].md[0].size[0] + kk]
                    ...
                    for(uint64_t ii = 0; ii < msizexy; ii++)
                    {
                        value1 += data.image[IDmdm1].array.F[kk * msizexy + ii] *
                                  data.image[IDmaskRM].array.F[ii];
                        value1cnt += data.image[IDmaskRM].array.F[ii];
                    }
                    for(uint64_t ii = 0; ii < msizexy; ii++)
                ...
                slen = snprintf(fname, STRINGMAXLEN_FULLFILENAME,
                                "!./mkmodestmp/fmodes_%02u.fits", mblock);
                if(slen < 1)
                {
                    PRINT_ERROR("snprintf wrote <1 char");
                    abort(); // can't handle this error any other way
                ...
                slen = snprintf(fname, STRINGMAXLEN_FULLFILENAME,
                                "!./mkmodestmp/fmodesWFS_%02u.fits", mblock);
                if(slen < 1)
                {
                    PRINT_ERROR("snprintf wrote <1 char");
                    abort(); // can't handle this error any other way
            ...
            {
                int slen = snprintf(fname, STRINGMAXLEN_FULLFILENAME,
                                    "./mkmodestmp/fmodes_%02u.fits", mblock);
                if(slen < 1)
                {
                    PRINT_ERROR("snprintf wrote <1 char");
            ...
            for(uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
            {
                ...
                for(uint64_t ii = 0; ii < wfssize; ii++)
                {
                    data.image[IDwfs].array.F[cnt * wfssize + ii] =
                        data.image[MBLOCK_IDwfs[mblock]].array.F[m * wfssize + ii];
                }
        ...
        for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            ...
            if((BlockNB < 0) || (BlockNB == (int) mblock))
            {
                if(MBLOCK_NBmode[mblock] > 0)
                {
                    ...
                    slen = snprintf(fname, STRINGMAXLEN_FULLFILENAME,
                                    "!./mkmodestmp/cmat_%02u.fits", mblock);
                    if(slen < 1)
                    {
                        PRINT_ERROR("snprintf wrote <1 char");
                        abort(); // can't handle this error any other way
                    ...
                    slen = snprintf(fname, STRINGMAXLEN_IMGNAME, "!./mkmodestmp/cmatc_%02u.fits",
                                    mblock);
                    if(slen < 1)
                    {
                        PRINT_ERROR("snprintf wrote <1 char");
                    ...
                    slen = snprintf(fname, STRINGMAXLEN_IMGNAME, "!./mkmodestmp/cmatcact_%02u.fits",
                                    mblock);
                    if(slen < 1)
                    {
                        PRINT_ERROR("snprintf wrote <1 char");
            ...
            else
            {
                ...
                slen = snprintf(fname, STRINGMAXLEN_FULLFILENAME, "./mkmodestmp/cmat_%02u.fits",
                                mblock);
                if(slen < 1)
                {
                    PRINT_ERROR("snprintf wrote <1 char");
                ...
                slen = snprintf(fname, STRINGMAXLEN_FULLFILENAME,
                                "./mkmodestmp/cmatc_%02u.fits", mblock);
                if(slen < 1)
                {
                    PRINT_ERROR("snprintf wrote <1 char");
                    abort(); // can't handle this error any other way
                ...
                slen = snprintf(fname, STRINGMAXLEN_FULLFILENAME,
                                "./mkmodestmp/cmatcact_%02u.fits", mblock);
                if(slen < 1)
                {
                    PRINT_ERROR("snprintf wrote <1 char");
                    abort(); // can't handle this error any other way
        ...
        for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            ...
            int slen = snprintf(command, STRINGMAXLEN_COMMAND,
                                "echo \"%u\" > ./%s/block%02u_NBmodes.txt", MBLOCK_NBmode[mblock], stagedir,
                                mblock);
            if(slen < 1)
            {
                PRINT_ERROR("snprintf wrote <1 char");
        ...
        for(uint32_t mblock = 0; mblock < NBmblock; mblock++)
        {
            ...
            for(uint32_t m = 0; m < MBLOCK_NBmode[mblock]; m++)
            {
                for(uint64_t ii = 0; ii < wfssize; ii++)
                {
                    data.image[IDcmatall].array.F[cnt * wfssize + ii] = data.image[IDcmat].array.F[m
                            * wfssize + ii];
                }
                cnt++;
            }
...
)
{
    ...
    else
    {
        ...
        {
            int slen = snprintf(fname, STRINGMAXLEN_FULLFILENAME,
                                "./conf_staged/param_block%02ldend.txt", mblock);
            if(slen < 1)
            {
                PRINT_ERROR("snprintf wrote <1 char");
                abort(); // can't handle this error any other way
    ...
    for(mblock = 1; mblock < NBmblock; mblock++)
    {
        ...
        MBLOCK_blockstart[mblock] =  MBLOCK_blockstart[mblock - 1] +
                                     MBLOCK_NBmode[mblock - 1];
    }
    ...
    for(mblock = 0; mblock < NBmblock; mblock++)
    {
        ...
        if((Cmblock == mblock) || (Cmblock == -1))
        {
            ...
            for(kk = 0; kk < MBLOCK_NBmode[mblock]; kk++)
            {
                ...
                {
                    data.image[ID].array.F[kk * wfssize + ii] = data.image[IDin].array.F[(kk +
                            MBLOCK_blockstart[mblock]) * wfssize + ii];
                }
            }
            ...
            for(kk = 0; kk < MBLOCK_NBmode[mblock]; kk++)
            {
                ...
                data.image[IDmodes].array.F[kk * NBmodes + (kk + MBLOCK_blockstart[mblock])] =
                    1.0;
            }
    ...
    for(mblock = 0; mblock < NBmblock; mblock++)
    {
        if(sprintf(command, "echo \"%ld\" > ./conf_staged/block%02ld_NBmodes.txt",
                   MBLOCK_NBmode[mblock], mblock) < 1)
        {
            PRINT_ERROR("sprintf wrote <1 char");
        }
