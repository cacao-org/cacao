...
errno_t AOloopControl_acquireCalib_mkRandomLinPokeSequence_cli()
{
    if (CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_LONG) + CLI_checkarg(3, CLIARG_STR) +
        ...
        0)
        {
            AOloopControl_acquireCalib_mkRandomLinPokeSequence(data.cmdargtoken[1].val.string,
...
errno_t AOloopControl_acquireCalib_RespMatrix_Fast_cli()
{
    if (CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_IMG) + CLI_checkarg(3, CLIARG_IMG) +
        ...
        0)
        {
            AOloopControl_acquireCalib_RespMatrix_Fast(data.cmdargtoken[1].val.string,
...
errno_t AOloopControl_acquireCalib_RMseries_deinterlace_cli()
{
    if (CLI_checkarg(1, CLIARG_LONG) + CLI_checkarg(2, CLIARG_LONG) + CLI_checkarg(3, CLIARG_LONG) +
        ...
        0)
        {
            AOloopControl_acquireCalib_RMseries_deinterlace(data.cmdargtoken[1].val.numl,
                                                            data.cmdargtoken[2].val.numl,
                                                            data.cmdargtoken[3].val.numl,
                                                            data.cmdargtoken[4].val.string,
                                                            data.cmdargtoken[5].val.numl,
...
static errno_t init_module_CLI()
{
   ...
   */
    ...
    RegisterCLIcommand("aolacqmkrlps",
                       __FILE__,
    ...
    RegisterCLIcommand("aolmeasWFSrespC",
                       __FILE__,
    ...
    RegisterCLIcommand("aolmeaslWFSrespC",
                       __FILE__,
    ...
    RegisterCLIcommand("aolRMdeinterlace",
                       __FILE__,
                                                    ...
                                                    const char *outdir)
{
    ...
    snprintf(
        pinfomsg, stringmaxlen, "delay=%ld+%ldus ave=%u excl=%u cyc=%u", delayfr, delayRM1us, NBave, NBexcl, NBcycle);

    PROCESSINFO *processinfo;
    int loopOK = 1;

    processinfo = processinfo_setup(pinfoname,  // short name for the processinfo instance, no spaces, no dot,
                    ...
                    if (kk1 == (int)NBave)
                        {
                            ...
                            if ((PokeIndex1Mapped > (uint32_t)(NBpoke - 1)))
                                {
                                    printf("ERROR: PokeIndex1Mapped = %u is outside range 0 - "
                                           "%u\n",
                                           PokeIndex1Mapped,
                                           NBpoke);
                                    exit(0);
                            ...
                            if (kk < NBave) // Capture signal
                                {
                                    ...
                                    memcpy(ptr, data.image[ID_wfsim].array.F,
                                           sizeof(float) * sizeWFS); // NEW
                                    array_accum[imcnt] = 1;
                                }
                            kk1++;
                            if (kk1 == (int)NBave) // wait for NBexcl excluded frames. We poke
                                                   // after delayRM1us
                                {
                                    kk1 = -NBexcl;
                                    PokeIndex1++;
...
errno_t AOcontrolLoop_acquireCalib_Measure_WFS_linResponse_FPCONF()
{
    ...
    function_parameter_add_entry(
        &fps, ".AOloopindex", "loop index", FPTYPE_INT64, FPFLAG_DEFAULT_INPUT, &AOloopindex_default, &fpi_AOloopindex);

    double ampl_default[4] = {0.01, 0.000001, 1.0, 0.01};
    long fpi_ampl = 0;
    function_parameter_add_entry(
        &fps, ".ampl", "RM poke amplitude", FPTYPE_FLOAT64, FPFLAG_DEFAULT_INPUT, &ampl_default, &fpi_ampl);
    ...
    long fpi_MaskMode = 0;
    function_parameter_add_entry(
        &fps, ".MaskMode", "Mask mode, DM and WFS", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, pNull, &fpi_MaskMode);
    ...
    function_parameter_add_entry(
        &fps, ".fn_pokeC", "Poke sequence cube", FPTYPE_FITSFILENAME, FPFLAG, pNull, &fpi_filename_pokeC);
  ...
  */
    ...
    function_parameter_add_entry(
        &fps, ".out.dirname", "output directory", FPTYPE_DIRNAME, FPFLAG_DEFAULT_INPUT, pNull, &fpi_out_dirname);
    (void)fpi_out_dirname;

    long fpi_out_label = 0;
    function_parameter_add_entry(
        &fps, ".out.label", "output label", FPTYPE_STRING, FPFLAG_DEFAULT_INPUT, pNull, &fpi_out_label);

    long fpi_out_timestring = 0;
    function_parameter_add_entry(
        &fps, ".out.timestring", "output timestring", FPTYPE_STRING, FPFLAG_DEFAULT_INPUT, pNull, &fpi_out_timestring);
    ...
    function_parameter_add_entry(
        &fps, ".normalize", "Normalize WFS frames", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, pNull, &fpi_normalize);

    long fpi_Hpokemode = 0;
    function_parameter_add_entry(
        &fps, ".Hpoke", "Hadamard Poke mode", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, pNull, &fpi_Hpokemode);

    long fpi_autoTiming = 0;
    function_parameter_add_entry(
        &fps, ".autoTiming", "Auto Timing", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, pNull, &fpi_autoTiming);

    // Compute Actions
    long fpi_comp_RM_DMmask = 0;
    function_parameter_add_entry(
        &fps, ".compDMmask", "(re-)compute RM DM mask", FPTYPE_ONOFF, FPFLAG_DEFAULT_INPUT, pNull, &fpi_comp_RM_DMmask);
                            ...
                            if (fps.parray[fpi_FPS_mlat].info.fps.FPSNBparamMAX > 0)
                                {
                                    ...
                                    double framerateHz =
                                        functionparameter_GetParamValue_FLOAT32(&FPS_mlat, ".out.framerateHz");
                                    ...
                                    int RMdelayfr =
                                        ((int)(latfr - 0.5 - 0.5 * fps.parray[fpi_NBexcl].val.i64[0] + 10.0)) + 1 - 10;
...
errno_t AOcontrolLoop_acquireCalib_Measure_WFS_linResponse_RUN()
{
    ...
    strncpy(
        wfsref_filename, functionparameter_GetParamPtr_STRING(&fps, ".out.fn_wfsref"), FUNCTION_PARAMETER_STRMAXLEN);
    ...
    strncpy(
        execmkLODMmodes, functionparameter_GetParamPtr_STRING(&fps, ".exec.mkLODMmodes"), FUNCTION_PARAMETER_STRMAXLEN);
    ...
    while (FileOK == 1) // keep processing until there is no more file to process
        {
            ...
            if (IterNumber == -1) // Process the global average
            ...
            else
                {
                    ...
                    WRITE_FULLFILENAME(
                        tmpfname, "%s/acquA/wfsresp.tstep%03d.iter%03d.fits", fps.md->datadir, AveStep, IterNumber);
                    load_fits(tmpfname, wfsresp2aname, 1, &IDwfsresp2a);

                    WRITE_FULLFILENAME(
                        tmpfname, "%s/acquB/wfsresp.tstep%03d.iter%03d.fits", fps.md->datadir, AveStep, IterNumber);
                                    ...
                                    for (uint64_t pix = 0; pix < wfsxysize; pix++)
                                        {
                                            ...
                                            data.image[IDrespC].array.F[wfsxysize * pokearray[poke] + pix] +=
                                                0.5 * valA;
                                            data.image[IDrespC_A].array.F[wfsxysize * pokearray[poke] + pix] +=
                                                1.0 * valA;
                                            ...
                                            data.image[IDrespC].array.F[wfsxysize * pokearray[poke] + pix] +=
                                                0.5 * valB;
                                            data.image[IDrespC_B].array.F[wfsxysize * pokearray[poke] + pix] +=
                                                1.0 * valB;
                                        }
...
errno_t AOloopControl_acquireCalib_Measure_WFS_linResponse(
    ...
    __attribute__((unused)) long NBinnerCycle /// Number of inner cycles (how many consecutive times
                                                   ...
                                                   uint32_t NBcycle)
{
            ...
            for (uint64_t act = 0; act < AOconf[loop].DMctrl.sizexDM * AOconf[loop].DMctrl.sizeyDM; act++)
                {
                    for (uint64_t ii = 0; ii < AOconf[loop].DMctrl.sizexDM * AOconf[loop].DMctrl.sizeyDM; ii++)
                        {
                            data.image[IDpokeC]
                                .array.F[act * AOconf[loop].DMctrl.sizexDM * AOconf[loop].DMctrl.sizeyDM + ii] = 0.0;
                        }
                    data.image[IDpokeC].array.F[act * AOconf[loop].DMctrl.sizexDM * AOconf[loop].DMctrl.sizeyDM + act] =
                        1.0;
                }
    ...
    create_image_ID("zrespm", 3, sizearray, _DATATYPE_FLOAT, 0, 5, 0,
    ...
    create_image_ID("zrespfp", 3, sizearray, _DATATYPE_FLOAT, 0, 5, 0,
                    &IDzrespfp); // positive poke image
    create_image_ID("zrespfm", 3, sizearray, _DATATYPE_FLOAT, 0, 5, 0,
                    ...
                    for (uint64_t ii = 0; ii < AOconf[loop].WFSim.sizeWFS; ii++)
                        {
                            ...
                            data.image[ID_WFSref2].array.F[ii] +=
                                data.image[IDpos].array.F[ii] * data.image[IDpos].array.F[ii];
                        }
                    ...
                    for (uint64_t ii = 0; ii < AOconf[loop].WFSim.sizeWFS; ii++)
                        {
                            ...
                            data.image[ID_WFSref2].array.F[ii] +=
                                data.image[IDneg].array.F[ii] * data.image[IDneg].array.F[ii];
                        }
                            ...
                            for (poke = 0; poke < NBpoke; poke++)
                                {
                                    ...
                                    for (uint64_t ii = 0; ii < AOconf[loop].WFSim.sizeWFS; ii++)
                                        {
                                            tmpv =
                                                data.image[IDzrespmn].array.F[poke * AOconf[loop].WFSim.sizeWFS + ii];
                                            rms += tmpv * tmpv;
                                        }
...
errno_t AOloopControl_acquireCalib_Measure_Resp_Matrix(
    long loop, uint32_t NbAve, float amp, long nbloop, long fDelay, uint64_t NBiter)
{
    long NBloops;
    long kloop;
    ...
    printf("ACQUIRE RESPONSE MATRIX - loop = %ld, NbAve = %u, amp = %f, nbloop "
    ...
    // AOconf[loop].WFSim.sizexWFS, AOconf[loop].WFSim.sizeyWFS);
    create_2Dimage_ID(
        "RMwfs0", AOconf[loop].WFSim.sizexWFS, AOconf[loop].WFSim.sizeyWFS, &(aoloopcontrol_var.aoconfID_imWFS0));

    create_2Dimage_ID(
        "RMwfs1", AOconf[loop].WFSim.sizexWFS, AOconf[loop].WFSim.sizeyWFS, &(aoloopcontrol_var.aoconfID_imWFS1));

    create_2Dimage_ID(
        "RMwfs2", AOconf[loop].WFSim.sizexWFS, AOconf[loop].WFSim.sizeyWFS, &(aoloopcontrol_var.aoconfID_imWFS1));
    ...
    for (iter = 0; iter < NBiter; iter++)
        {
            if (file_exists("stopRM.txt"))
            ...
            else
                {
                    ...
                    for (uint64_t ii = 0; ii < AOconf[loop].WFSim.sizeWFS; ii++)
                        {
                            data.image[IDrefcumul].array.F[ii] = (1.0 - gain) * data.image[IDrefcumul].array.F[ii] +
                            ...
                            for (ii = 0; ii < xysize; ii++)
                                {
                                    data.image[IDref].array.F[ii] +=
                                        data.image[IDRMarray[rmCindex]].array.F[imindex * xysize + ii];
                                }
                            cntref++;
                        }
                }
