...
static FPS_APP_INFO FPS_app_info = {
    ...
    .description_long = "Acquire WFS linear calibration data by systematically applying DM pokes "
                        "and recording WFS responses. Builds the response matrix."
};
...
#define FPS_PARAMS(X)                                                                              \
    ...
    X(".dmstream", dmstream, FPTYPE_STREAMNAME, 1, (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT),      \
      "DM stream")                                                                                 \
    X(".ampl", &pokeampl, FPTYPE_FLOAT32, 1, (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT),            \
      "RM poke amplitude")                                                                         \
    X(".timing.FPS_mlat", &FPS_mlat, FPTYPE_FPSNAME, 1, FPFLAG_DEFAULT_INPUT, "hardware latency")  \
    X(".timing.upmlat", &update_mlat, FPTYPE_ONOFF, 1, FPFLAG_DEFAULT_INPUT,                       \
      "update latency from FPS")                                                                   \
    X(".timing.WFSfrequ", &WFSfrequ, FPTYPE_FLOAT32, 1, (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT), \
      "WFS frame rate [Hz]")                                                                       \
    X(".timing.hardwlatfr", &hardwlatfr, FPTYPE_FLOAT32, 1,                                        \
      (FPFLAG_DEFAULT_INPUT | FPFLAG_CLI_INPUT), "hardware latency [fr]")                          \
    X(".timing.autoTiming", &autotiming, FPTYPE_ONOFF, 1, FPFLAG_DEFAULT_INPUT, "Auto Timing")     \
    X(".timing.delayfr", &delayfr, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT,                         \
      "frame delay, whole part")                                                                   \
    X(".timing.delayRM1us", &delayRM1us, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT,                   \
      "Sub-frame delay [us]")                                                                      \
    X(".timing.NBave", &NBave, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT,                             \
      "Number of frames averaged for a single poke measurement")                                   \
    X(".timing.NBexcl", &NBexcl, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT,                           \
      "Number of frames excluded")                                                                 \
    X(".timing.NBcycle", &NBcycle, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT,                         \
      "Number of measurement cycles to be repeated")                                               \
    X(".timing.NBinnerCycle", &NBinnerCycle, FPTYPE_UINT32, 1, FPFLAG_DEFAULT_INPUT,               \
      "Number of inner cycles")                                                                    \
    X(".RMDMmask.FPS_DMcomb", &FPS_DMcomb, FPTYPE_FPSNAME, 1, FPFLAG_DEFAULT_INPUT,                \
      "DM control process")                                                                        \
    ...
    X(".RMDMmask.upmlat", &update_RMDMmask, FPTYPE_ONOFF, 1, FPFLAG_DEFAULT_INPUT,                 \
      "update RMDMmask from FPS")                                                                  \
    X(".MaskMode", &MaskMode, FPTYPE_ONOFF, 1, FPFLAG_DEFAULT_INPUT, "Mask mode, DM and WFS")      \
    X(".DMmask.RMp0", &maskDMp0, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT,                          \
      "DM mask, point0 percentile point")                                                          \
    X(".DMmask.RMc0", &maskDMc0, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT,                          \
      "DM mask, point0 coefficient")                                                               \
    X(".DMmask.RMp1", &maskDMp1, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT,                          \
      "DM mask, point1 percentile point")                                                          \
    X(".DMmask.RMc1", &maskDMc1, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT,                          \
      "DM mask, point1 coefficient")                                                               \
    X(".DMmask.proxrad", &DMproxrad, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT,                      \
      "DM actuator proximity radius")                                                              \
    X(".WFSmask.RMp0", &maskWFSp0, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT,                        \
      "WFS mask, point0 percentile point")                                                         \
    X(".WFSmask.RMc0", &maskWFSc0, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT,                        \
      "WFS mask, point0 coefficient")                                                              \
    X(".WFSmask.RMp1", &maskWFSp1, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT,                        \
      "WFS mask, point1 percentile point")                                                         \
    X(".WFSmask.RMc1", &maskWFSc1, FPTYPE_FLOAT32, 1, FPFLAG_DEFAULT_INPUT,                        \
      "WFS mask, point1 coefficient")                                                              \
    X(".fn_pokeC", fn_pokeC, FPTYPE_FITSFILENAME, 1, FPFLAG_DEFAULT_INPUT, "Poke sequence cube")   \
    X(".fn_RMDMmask", fn_RMDMmask, FPTYPE_FITSFILENAME, 1, FPFLAG_DEFAULT_INPUT,                   \
    ...
    X(".compPokeMat", &compPokeMat, FPTYPE_ONOFF, 1, FPFLAG_DEFAULT_INPUT,                         \
      "(re)compute poke matrix")                                                                   \
    X(".exec.RMdecode", exec_post_RMdecode, FPTYPE_FILENAME, 1,                                    \
      FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, "RM decode script")                         \
    X(".exec.mkDMWFSmasks", exec_post_mkDMWFSmasks, FPTYPE_FILENAME, 1,                            \
      FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, "Make DM and WFS masks")                    \
    X(".exec.mkDMslaveact", exec_post_mkDMslaveact, FPTYPE_FILENAME, 1,                            \
      FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, "Make DM slaved actuators")                 \
    X(".exec.mkLODMmodes", exec_post_mkLODMmodes, FPTYPE_FILENAME, 1,                              \
      FPFLAG_DEFAULT_INPUT | FPFLAG_FILE_RUN_REQUIRED, "Make DM low order modes")
...
static errno_t customCONFsetup()
{
    if (milk_data.fpsptr != NULL)
    {
        ...
        if (fpi > -1)
        {
            milk_data.fpsptr->parray[fpi].fpflag |= FPFLAG_STREAM_RUN_REQUIRED;
        }
...
static errno_t customCONFcheck()
{
    if (milk_data.fpsptr != NULL)
    {
        ...
        long fpi_update_mlat = functionparameter_GetParamIndex(milk_data.fpsptr, ".timing.upmlat");
        long fpi_autotiming =
            functionparameter_GetParamIndex(milk_data.fpsptr, ".timing.autoTiming");
        ...
        if (FPS_mlat.SMfd < 1)
        {
            ...
            if (fpi_FPS_mlat > -1)
            {
                functionparameter_ConnectExternalFPS(milk_data.fpsptr, fpi_FPS_mlat, &FPS_mlat);
            }
        }
        ...
        if (FPS_DMcomb.SMfd < 1)
        {
            ...
            if (fpi_FPS_DMcomb > -1)
            {
                functionparameter_ConnectExternalFPS(milk_data.fpsptr, fpi_FPS_DMcomb, &FPS_DMcomb);
            }
        }
            ...
            (milk_data.fpsptr->parray[fpi_update_mlat].fpflag & FPFLAG_ONOFF))
        {
            ...
            if (FPS_mlat.SMfd > 0)
            {
                float WFSfrequ = functionparameter_GetParamValue_FLOAT32(&FPS_mlat, ".framerateHz");

                float latencyfr = functionparameter_GetParamValue_FLOAT32(&FPS_mlat, ".latencyfr");

                functionparameter_SetParamValue_FLOAT32(milk_data.fpsptr, ".timing.WFSfrequ",
                                                        WFSfrequ);
                functionparameter_SetParamValue_FLOAT32(milk_data.fpsptr, ".timing.hardwlatfr",
                                                        latencyfr);
            }
            milk_data.fpsptr->parray[fpi_update_mlat].fpflag &= ~FPFLAG_ONOFF;
        }
        ...
        //
        if (fpi_autotiming > -1 &&
            (milk_data.fpsptr->parray[fpi_autotiming].fpflag & FPFLAG_ONOFF)) // ON state
        {
            ...
            int val_RMdelayfr = ((int) ((*hardwlatfr) - 0.5 - 0.5 * (*NBexcl) + 10.0)) + 1 - 10;

            int val_delayRM1us = (int) ((1.0 * val_RMdelayfr - RMdelay) / (*WFSfrequ) * 1000000.0);
        ...
        imageID IDdmRM = image_ID(dmstream, dcimg, dcnimg);
        if (IDdmRM != -1)
        {
            ...
            long fpi_update_RMDMmask =
                functionparameter_GetParamIndex(milk_data.fpsptr, ".RMDMmask.upmlat");
            if (fpi_update_RMDMmask > -1 &&
                (milk_data.fpsptr->parray[fpi_update_RMDMmask].fpflag & FPFLAG_ONOFF))
            {
                ...
                {
                    DMMODEin = functionparameter_GetParamValue_UINT32(&FPS_DMcomb, ".DMmode");
                }
                ...
                //
                {
                    ...
                    if (ID_RMDMmask == -1)
                    {
                        ...
                        if (*DMMODE == 0)
                        {
                            ...
                            // make centered disk
                            //
                            make_disk("RMDMmask", DMxsize, DMysize, *RMDMmaskCx, *RMDMmaskCy,
                                      *RMDMmaskR);
                            //0.5 * (DMxsize) + 0.6);
                        }
                        else
                        {
                            ...
                            //
                            imageID ID;
                            create_2Dimage_ID("RMDMmask", DMxsize, DMysize, &ID);
                    ...
                    WRITE_FULLFILENAME(fname_RMDMmask, "./%s/RMDMmask.fits",
                                       milk_data.fpsptr->md->datadir);
                    functionparameter_SetParamValue_STRING(milk_data.fpsptr, ".fn_RMDMmask",
                                                           fname_RMDMmask);
                }
                ...
                {
                    float WFSfrequ =
                        functionparameter_GetParamValue_FLOAT32(&FPS_mlat, ".framerateHz");

                    float latencyfr =
                        functionparameter_GetParamValue_FLOAT32(&FPS_mlat, ".latencyfr");
            ...
            long fpi_compPokeMat =
                functionparameter_GetParamIndex(milk_data.fpsptr, ".compPokeMat");
            if (fpi_compPokeMat > -1 &&
                (milk_data.fpsptr->parray[fpi_compPokeMat].fpflag & FPFLAG_ONOFF))
            {
                ...
                {
                    long fpi_Hpokemode =
                        functionparameter_GetParamIndex(milk_data.fpsptr, ".Hpoke");
                    if (fpi_Hpokemode > -1 &&
                        (milk_data.fpsptr->parray[fpi_Hpokemode].fpflag & FPFLAG_ONOFF))
                    {
...

                        {
                            ...
                            WRITE_FULLFILENAME(fname_Hpoke, "./%s/Hpoke.fits",
                                               milk_data.fpsptr->md->datadir);
                            functionparameter_SetParamValue_STRING(milk_data.fpsptr, ".fn_pokeC",
                                                                   fname_Hpoke);
                        }
                    }
                    else
                    {
                        ...
                        mkSimpleZpokeM(DMxsize, DMysize, "Spoke");
...

                        {
                            ...
                            WRITE_FULLFILENAME(fname_Spoke, "./%s/Spoke.fits",
                                               milk_data.fpsptr->md->datadir);
                            functionparameter_SetParamValue_STRING(milk_data.fpsptr, ".fn_pokeC",
                                                                   fname_Spoke);
                        }
