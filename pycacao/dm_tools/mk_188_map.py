...
def mk_coords_bim188():
    ...
    ring_offsets = [
        ...
        10.5,
        11,
    ]  # Angular offset of ring origin in unit of actuators.
...
def influ_functions_of_188_onto_50x50(
    actuator_pos: np.ndarray, size_buff: int = 50, influ_radius: float = 1 / 12.0
):
    ...
    influ_radius_n = influ_radius * size_buff / 2.0
    ...
    for ii in range(n_actu):
        ...
        actu_influ_nxn[ii] = np.exp(
            -((row_map - r_pos) ** 2 + (col_map - c_pos) ** 2) / 2 / influ_radius_n**2
        )
...
def make_neighbors_from_actupos(pos: np.ndarray) -> _typethis:
    ...
    rel_matrix = (
        (pos_x[None, :] - pos_x[:, None]) ** 2 + (pos_y[None, :] - pos_y[:, None]) ** 2
    ) ** 0.5
    # Remove diagonal
...
if __name__ == "__main__":
    ...
    bim_respM = project_zonal_on_modal(modes=influs, resp_matrix=sim_respM_2500).astype(
        np.float32
    )
