...
class CacaoConf:
    ...
    def from_pwd_tree(self, where: str = "."):
        ...
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True, executable="bash")
        ...
        obj = CacaoConf(
            PWD=abs_path,
            LOOPNAME=env_dict["CACAO_LOOPNAME"],
    ...
    def get_dm_pup_params(self) -> Tuple[float, float, float]:
        if "CACAO_DM_beam_xcent" in self.full_env:
            ...
            return (
                float(self.full_env["CACAO_DM_beam_xcent"]),
            ...
            return (self.DMxsize / 2.0, self.DMysize / 2.0, self.DMxsize * 0.45)
